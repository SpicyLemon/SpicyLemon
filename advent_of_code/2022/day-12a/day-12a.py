...
class Point(object):
    ...
    def process(self, grid):
        neighbors = []
        of_interest = self.x == 149 and self.y == 21
        if self.x > 0:
        ...
        if self.y < len(grid)-1:
            neighbors.append(grid[self.y+1][self.x])
        if of_interest:
            stdout(f'{self.str} neighbors: ' + points_str(neighbors))
        to_add = []
        for n in neighbors:
            if n.visited:
                if of_interest:
                    stdout(f'{self.str} -> {n.str}: already visited.')
                continue
            d = n.val - self.val
            if d > _MAX_D_VAL:
                if of_interest:
                    stdout(f'{self.str} -> {n.str}: height diff too great: {d} vs {_MAX_D_VAL}.')
                continue
            if self.update_path_to(n):
                if of_interest:
                    stdout(f'{self.str} -> {n.str}: path updated.')
                to_add.append(n)
                continue
            if of_interest:
                stdout(f'{self.str} -> {n.str}: boring.')
        return to_add
...
def solve(params) -> str:
    ...
    if params.verbose:
        stdout('path:\n' + points_str(path))
        stdout('map:\n' + highlight_map(puzzle.grid, path, puzzle.end))

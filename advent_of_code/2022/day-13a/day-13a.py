...
class Packet(object):
    def __init__(self, line):
        printd(f'Creating packet from "{line}"')
        self.elements = []
        if line == '[]':
            printd('  Empty, returning.')
            return
        if not '[' in line[1:-1]:
            for e in line[1:-1].split(','):
                printd(f'  Adding int {e}')
                self.elements.append(int(e))
            return
        parts = ['']
        depth = 0
        for c in line[1:-1]:
            if c == '[':
                ...
                parts[-1] += c
        printd(f'  Parts: {parts}')
        for p in parts:
            if '[' in p:
                printd(f'  Adding sub-packet: {p}')
                self.elements.append(Packet(p))
            else:
                printd(f'  Adding parts int {p}')
                self.elements.append(int(p))
    ...
    def compare(left, right) -> bool:
        printd(f'Comparing {left} vs {right}')
        for i in range(0, len(left.elements)):
            if i >= len(right.elements):
                printd('  out of elements on the right.')
                return 1
            printd(f'  element {left.elements[i]} vs {right.elements[i]}')
            left_is_int = isinstance(left.elements[i], int)
            right_is_int = isinstance(right.elements[i], int)
            if left_is_int and right_is_int:
                if left.elements[i] < right.elements[i]:
                    printd('    left < right, returning -1')
                    return -1
                if left.elements[i] > right.elements[i]:
                    printd('    left > right, returning 1')
                    return 1
            elif left_is_int and not right_is_int:
                printd('    converting left to packet')
                new_left = Packet(f'[{left.elements[i]}]')
                rv = new_left.compare(right.elements[i])
                if rv != 0:
                    return rv
            elif not left_is_int and right_is_int:
                printd('    converting right to packet')
                new_right = Packet(f'[{right.elements[i]}]')
                rv = left.elements[i].compare(new_right)
                if rv != 0:
                    return rv
            else:
                printd('    both are packets')
                rv = left.elements[i].compare(right.elements[i])
                if rv != 0:
                    return rv
        if len(left) < len(right):
            printd('  left is out of elements, returning 1')
            return -1
        printd('  uhhhh, yeah. they are the same, returning 0')
        return 0
...
class Pair(object):
    def __init__(self):
        self.left = None
        self.right = None
    def with_packet(self, line):
        if self.left == None:
            self.left = Packet(line)
        elif self.right == None:
            self.right = Packet(line)
        else:
            raise ValueError(f'Pair already has two packets. Cannot add "{line}"')
    def __str__(self):
...
def solve(params) -> str:
    ...
    in_order = []
    out_of_order = []
    for i in range(0, len(puzzle.pairs)):
        printd(f'Comparing pair {i+1}')
        if puzzle.pairs[i].is_in_order():
            printd('  -> in order')
            in_order.append(i+1)
        else:
            printd('  -> out of order')
            out_of_order.append(i+1)
    stdout(f'In order: {in_order}')
    stdout(f'Out of order: {out_of_order}')
    answer = 0
    for i in in_order:
        answer += i

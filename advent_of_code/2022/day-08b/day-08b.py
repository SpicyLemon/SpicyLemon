    ...
    def set_scores(self):
        ...
        for y in range(0, ylen):
            for x in range(0, xlen):
                if y == 0 or x == 0 or y == ylen-1 or x == xlen-1:
                    self.grid[y][x].height = 0
                    continue
                height = self.grid[y][x].height
                # Look up
                up = 0
                for yy in range(y-1, -1, -1):
                    up += 1
                    if self.grid[yy][x].height >= height:
                        break
                # Look down
                down = 0
                for yy in range(y+1, ylen):
                    down += 1
                    if self.grid[yy][x].height >= height:
                        break
                # Look left
                ...
                for xx in range(x-1, -1, -1):
                    left += 1
                    if self.grid[y][xx].height >= height:
                        break
                # Look right
                right = 0
                for xx in range(x+1, xlen):
                    right += 1
                    if self.grid[y][xx].height >= height:
                        break
                self.grid[y][x].score = up * down * left * right
...
def solve(params) -> str:
    ...
    puzzle.set_scores()
    answer = 0
    for y in range(0, len(puzzle.grid)):
        for x in range(0, len(puzzle.grid[y])):
            if puzzle.grid[y][x].score > answer:
                answer = puzzle.grid[y][x].score

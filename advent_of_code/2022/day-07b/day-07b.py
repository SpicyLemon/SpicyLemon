...
_NEEDED_FREE = 30000000

################################################################################
##############################  Puzzle Solution  ###############################
################################################################################

def to_path(paths) -> str:
    if len(paths) == 0:
        return '/'
    if len(paths) == 1:
        return paths[0]
...
class Dir(object):
    ...
    def __str__(self) -> str:
        lines = [ f'{self.path} (dir, size={self.size})' ]
        for d in self.dirs.values():
            lines.append(str(d))
        for name, size in self.files.items():
            lines.append(f'{self.path}/{name} (file, size={size})')
        return '\n'.join(lines)
    def dir_sizes(self) -> {}:
        rv = { self.path: self.size }
        for p, d in self.dirs.items():
            for p2, s in d.dir_sizes().items():
                rv[p2] = s
        return rv
...
class Terminal(object):
    ...
    def ensure_dir(self, path):
        if path not in self.dirs:
            self.dirs[path] = Dir(path)
    def execute(self, cmd):
        printd('executing ' + cmd.cmd)
        if cmd.cmd[0:2] == 'cd':
            next_path = cmd.cmd[3:]
            printd(f'  next_path = {next_path}')
            if next_path == '/':
                self.pwd = [ '/' ]
            elif next_path == '..':
                self.pwd.pop()
            else:
                self.pwd.append(next_path)
                self.ensure_dir(to_path(self.pwd))
            printd(f'  pwd = {self.pwd}')
        elif cmd.cmd == 'ls':
            pwd = to_path(self.pwd)
            printd(f' pwd = {pwd}')
            curdir = self.dirs[pwd]
            for line in cmd.out:
                printd('  processing output: ' + line)
                v1, name = line.split(' ')
                if v1 == 'dir':
                    newdir = to_path((pwd, name))
                    self.ensure_dir(newdir)
                    curdir.dirs[name] = self.dirs[newdir]
                else:
                    self.dirs[to_path(self.pwd)].files[name] = int(v1)
        else:
...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            if line != '':
                if line[0] == '$':
                    self.cmds.append(Cmd(line[2:]))
                else:
                    self.cmds[-1].out.append(line)
...
def solve(params) -> str:
    ...
    term = Terminal()
    for cmd in puzzle.cmds:
        term.execute(cmd)
    printd('Files:\n' + str(term.dirs['/']))
    sizes = term.dirs['/'].dir_sizes()
    printd('sizes:\n' + str(sizes))
    currently_used = sizes['/']
    currently_free = _TOTAL_SPACE - currently_used
    min_to_delete = _NEEDED_FREE - currently_free
    stdout(f'   Total space: {_TOTAL_SPACE}')
    stdout(f'Currently used: {currently_used}')
    stdout(f'Currently free: {currently_free}')
    stdout(f'   Needed free: {_NEEDED_FREE}')
    stdout(f' min to delete: {min_to_delete}')
    answer = _TOTAL_SPACE
    for d, s in sizes.items():
        if s > min_to_delete and s < answer:
            answer = s
            stdout(f'{d} = {s} = new answer')

    ...
    def can_be_beacon(self, p) -> bool:
        for s in self.sensors:
            ...
            if d <= s.radius:
                return False
        return True
    def count_not(self, y) -> int:
        rv = 0
        test = Point(self.min.x-1, y)
        while not self.can_be_beacon(test):
            printd(f'In range: {test}')
            rv += 1
            test.x -= 1
        for x in range(self.min.x, self.max.x+1):
            test = Point(x, y)
            if not self.can_be_beacon(test):
                printd(f'In range: {test}')
                rv += 1
        test = Point(self.max.x+1, y)
        while not self.can_be_beacon(test):
            printd(f'In range: {test}')
            rv += 1
            test.x += 1

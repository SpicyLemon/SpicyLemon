...
class Stacks(object):
    ...
    def __str__(self):
        lines = []
        for i in range(1,10):
            lines.append(f'{i}: {self.stacks[i]}')
        return '\n'.join(lines)
    def __getitem__(self, x):
        return self.stacks[x]

class Instruction(object):
    def __init__(self, count, f, t):
        self.count = count
        self.out = f
        self.to = t
    def __str__(self) -> str:
        return f'move {self.count} from {self.out} to {self.to}'
    def execute(self, stacks):
        printd(f"Before '{self}':\n{stacks}")
        n = []
        for i in range(1, self.count+1):
            printd(f'Moving {stacks[self.out].peek()}')
            n.append(stacks[self.out].pop())
        n.reverse()
        stacks[self.to].entries.extend(n)
        printd(f"After '{self}':\n{stacks}")
...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        stacknum_re = re.compile('^[\W\d]+$')
        for line in lines:
            if stacknum_re.match(line):
                continue
            if line == '':
                for i in range(1, 10):
            ...
            if instr:
                ...
                if m:
                    self.instructions.append(Instruction(int(m.group(1)), int(m.group(2)), int(m.group(3))))
                continue
            ele = line + ' '
            ele = re.sub('    ', '_ ', ele)
            ele = re.sub('[\[\]]', '', ele)
...
def solve(params) -> str:
    ...
    for instr in puzzle.instructions:
        instr.execute(puzzle.stacks)
    answer = ''
    printd('Final:\n' + str(puzzle))
    for i in range(1, 10):
        answer = answer + str(puzzle.stacks[i].peek())

    ...
    def follow(self, p2):
        ...
        if dx == 0:
            ...
            if dx > 0:
                ...
                self.move('L')
    def visits_str(self):
        return ' '.join(sorted(self.visits.keys()))
    def path_str(self):
...
class Grid(object):
    ...
    def move(self, step):
        for i in range(0, step.amount):
            self.points[0].move(step.direction)
            self.points[0].record()
            for i in range(1, len(self.points)):
                self.points[i].follow(self.points[i-1])
                self.points[i].record()
...
def solve(params) -> str:
    ...
    grid = Grid(params.count)
    for step in puzzle.steps:
        grid.move(step)
    answer = len(grid.points[-1].visits)
    if _debug:
        for i in range(0, len(grid.points)):
            stdout(f'{i}: {grid.points[i].path_str()}')

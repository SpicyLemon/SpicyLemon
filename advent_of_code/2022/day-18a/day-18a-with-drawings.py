...
class Space(object):
        ...
        for z in range(0, self.maximums.z+1):
            ...
            for y in range(0, self.maximums.y+1):
                ...
                for x in range(0, self.maximums.x+1):
                    self.cells[-1][-1].append(False)
        for p in points:
            self.cells[p.z][p.y][p.x] = True
    ...
    def __str__(self) -> str:
        ...
        for z in range(0, self.maximums.z+1):
            ...
            lines.append(gh)
            for y in range(0, self.maximums.y+1):
                lines.append(f'{y:>2}' + ''.join(list(map(lambda v : '#' if v else ' ', self.cells[z][y]))) + f'{y}')
            lines.append(gf)
...
def surface_area(points, space) -> int:
    rv = 0
    for p in points:
        if p.x == 0 or not space[p.z][p.y][p.x-1]:
            rv += 1
        if p.x == space.maximums.x or not space[p.z][p.y][p.x+1]:
            rv += 1
        if p.y == 0 or not space[p.z][p.y-1][p.x]:
            rv += 1
        if p.y == space.maximums.y or not space[p.z][p.y+1][p.x]:
            rv += 1
        if p.z == 0 or not space[p.z-1][p.y][p.x]:
            rv += 1
        if p.z == space.maximums.z or not space[p.z+1][p.y][p.x]:
            rv += 1
    return rv

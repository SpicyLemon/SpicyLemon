...
class Puzzle(object):
    ...
    def __str__(self) -> str:
        ...
        return '\n'.join(map(str, self.blueprints))

_NUM_RE = re.compile('\d+')

class Blueprint(object):
    def __init__(self, line):
        nums = list(map(int, _NUM_RE.findall(line)))
        if len(nums) != 7:
            raise ValueError(f'Could not parse blueprint line: "{line}".')
        self.num = nums[0]
        self.ore_ore = nums[1]
        self.clay_ore = nums[2]
        self.obsidian_ore = nums[3]
        self.obsidian_clay = nums[4]
        self.geode_ore = nums[5]
        self.geode_obsidian = nums[6]
        self.max_ore = max(self.ore_ore, self.clay_ore, self.obsidian_ore, self.geode_ore)
    def __str__(self) -> str:
        return f'[{self.num}]: {self.ore_ore} ore; {self.clay_ore} ore; {self.obsidian_ore} ore + {self.obsidian_clay} clay; {self.geode_ore} ore + {self.geode_obsidian} obsidian'
...
class State(object):
    ...
    def __str__(self) -> str:
        rv = f'[{self.minute}]: '
        rv += f'({self.ore},{self.clay},{self.obsidian},{self.geodes})-'
        rv += f'({self.ore_bots},{self.clay_bots},{self.obsidian_bots},{self.geode_bots})'
        return rv
    @property
    def key(self) -> str:
        return f'{self.minute}-({self.ore},{self.clay},{self.obsidian},{self.geodes})-({self.ore_bots},{self.clay_bots},{self.obsidian_bots},{self.geode_bots})'

_MAX_GEODE_LAG = 1

def get_max_geodes(bp, max_minutes):
    starter = State()
    best = starter
    queue = deque()
    ...
    while queue:
        cur = queue.popleft()
        if _debug: stdout(f'Cur: "{cur.key}" {cur}')

        if cur.key in visited:
            #if _debug: stdout(f'  Already visited "{cur.key}"')
            continue
        visited.add(cur.key)

        if cur.geodes > best.geodes:
            best = cur

        if cur.minute >= max_minutes:
            #printd('  At time limit.')
            continue

        # Start processing this state for next steps.
        cur.minute += 1

        can_build_ore = cur.ore >= bp.ore_ore
        can_build_clay = cur.ore >= bp.clay_ore
        can_build_obsidian = cur.ore >= bp.obsidian_ore and cur.clay >= bp.obsidian_clay
        can_build_geode = cur.ore >= bp.geode_ore and cur.obsidian >= bp.geode_obsidian

        cur.ore += cur.ore_bots
        cur.clay += cur.clay_bots
        cur.obsidian += cur.obsidian_bots
        cur.geodes += cur.geode_bots

        # If this state is too far behind on geodes, just ignore it.
        if cur.geodes + _MAX_GEODE_LAG < best.geodes:
            continue

        # If we can build a geode bot, only explore that option.
        if can_build_geode:
            cur.geode_bots += 1
            cur.ore -= bp.geode_ore
            cur.obsidian -= bp.geode_obsidian
            queue.append(cur)
            continue

        # Otherwise, check each bot type, but don't make more bots of a type than the max needed amount of that type.
        if can_build_ore and cur.ore_bots < bp.max_ore:
            new_state = cur.copy()
            new_state.ore_bots += 1
            new_state.ore -= bp.ore_ore
            queue.append(new_state)
        if can_build_clay and cur.clay_bots < bp.obsidian_clay:
            new_state = cur.copy()
            new_state.clay_bots += 1
            new_state.ore -= bp.clay_ore
            queue.append(new_state)
        if can_build_obsidian and cur.obsidian_bots < bp.geode_obsidian:
            new_state = cur.copy()
        ...
        # And check out just doing nothing.
        queue.append(cur)
...
def solve(params) -> str:
    ...
    if len(params.custom) == 0:
        for bp in puzzle.blueprints:
            if params.verbose:
                stdout(f'Solving: {bp} with {params.count} minutes.')
            res = get_max_geodes(bp, params.count)
            answer *= res.geodes
            if params.verbose:
                stdout(f'Blueprint: {bp}\nBest: {res}\nAnswer Update: {res.geodes} -> {answer}\n')
    else:
        for i in map(int, params.custom):
            bp = puzzle.blueprints[i-1]
            stdout(f'Solving: {bp} with {params.count} minutes.')
            res = get_max_geodes(bp, params.count)
            score = bp.num * res.geodes
            answer += score
            stdout(f'Blueprint: {bp}\nBest: {res}\nAnswer Update: {bp.num} * {res.geodes} = {score} -> {answer}\n')
    # Wrong answer 2: 14496 too high

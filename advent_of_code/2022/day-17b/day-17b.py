...
_DEFAULT_COUNT = 100000

# In the example input, there are 40 jets and 5 different rocks.
# In the actual input, there are 10091 jets and 5 different rocks.
# I'd bet that the rock patterns tessalate after a while.
# The tessalation would start after p rocks.
# It would last n rocks, and it would appear t times.
# We can define p such that the last rock is the last rock in a tesselated copy.
# 1000000000000 = p + n*t
# n is most likely a function of the number of jets and probably the number 5.
# p is then 1000000000000 % n
# and t is (1000000000000 - p) / n
# Then, I need to find the height after p blocks (b), and then again after p+n blocks (m).
# The answer is then b + t*(m-b).
...
class InfList(object):
    ...
    def is_at_start(self) -> bool:
        return self.n == -1 or self.n == len(self.vals) - 1
...
class Cavern(object):
    ...
    def extend_space(self):
        while len(self.space) < 1000:
            self.space.append(list('.' * 7))
    def shift_space(self):
        tops = []
        for x in range(0, len(self.space[0])):
            tops.append(len(self.space)-1)
            while tops[-1] > 0 and self.space[tops[-1]][x] == '.':
                tops[-1] -= 1
        cruft = min(tops)
        printd(f'Removing {cruft} lines from the bottom. Tops = {tops}')
        del self.space[0:cruft]
        self.bottom_at += cruft
        self.extend_space()
    @property
    def height(self) -> int:
        return self.bottom_at + self.internal_height
    @property
    def internal_height(self) -> int:
        rv = 0
        empty = '.' * len(self.space[0])
        while ''.join(self.space[rv]) != empty:
            rv += 1
        return rv
    def __str__(self):
        lines = []
        ...
        for y in range(0, self.internal_height):
                lines.append('|'+''.join(self.space[y])+'|')
        empty = '|'+('.' * len(self.space[0]))+'|'
        for i in range(0, 4):
            lines.append(empty)
        return '\n'.join(reversed(lines))
    def drop(self, rock, jets):
        ...
        start_height = self.internal_height
        rock_bottom = start_height + 3
        rock_left = 2
        ...
        while True:
            ...
            if push == '<':
                ...
                if rock_left == 0:
                    ...
                    for y in range(0, len(rock.left)):
                        ...
                        space_y = rock_bottom + len(rock) - y - 1
            ...
            if jets.is_at_start():
                self.wrap_heights.append(self.height)
                self.wrap_rocks.append(len(self.heights))
            add_rock = False
            if rock_bottom == 0:
                ...
                for x in range(0, len(rock.bottom)):
                    space_x = rock_left + x
                    space_y = rock_bottom + rock.bottom[x] - 1
            ...
            if add_rock:
                ...
                for y in range(0, len(rock)):
                    for x in range(0, len(rock[y])):
                        if rock[len(rock)-y-1][x] != '.':
                            space_x = rock_left + x
                            space_y = rock_bottom + y
                            printd(f'  Add Rock: ({space_x},{space_y}), ({x},{y}) = {c}')
                            self.space[space_y][space_x] = c
                break
            rock_bottom -= 1
        self.heights.append(self.height)
        if len(self.space) - start_height < 10:
            self.shift_space()
    def drop_to_repeat(self, rocks, jets, force_stop):
        i = 0
        while i < force_stop:
            i += 1
            self.drop(next(rocks), jets)
            if len(self.wrap_heights) >= 3 and i >= self.wrap_rocks[-1] + (self.wrap_rocks[-1] - self.wrap_rocks[-2]) - 1:
                return
        raise ValueError(f'No pattern found after {force_stop} rock drops.')
...
def solve(params) -> str:
    ...
    puzzle.cavern.drop_to_repeat(puzzle.rocks, puzzle.jets, params.count)
    goal = 1000000000000
    if params.verbose:
        stdout(f'Wrap rocks: {puzzle.cavern.wrap_rocks}')
        stdout(f'Wrap heights: {puzzle.cavern.wrap_heights}')
    d_rocks = puzzle.cavern.wrap_rocks[-1] - puzzle.cavern.wrap_rocks[-2]
    if params.verbose: stdout(f'd_rocks = {d_rocks} = {puzzle.cavern.wrap_rocks[-1]} - {puzzle.cavern.wrap_rocks[-2]}')
    d_last_rock = (goal - puzzle.cavern.wrap_rocks[0] - 1) % d_rocks
    if params.verbose: stdout(f'd_last_rock = {d_last_rock} = ({goal} - {puzzle.cavern.wrap_rocks[0]} - 1) % {d_rocks}')
    times = int((goal - puzzle.cavern.wrap_rocks[0] - d_last_rock) / d_rocks)
    if params.verbose: stdout(f'times = {times} = ({goal} - {puzzle.cavern.wrap_rocks[0]} - {d_last_rock}) / {d_rocks}')
    roi = puzzle.cavern.wrap_rocks[-1] + d_last_rock
    if params.verbose: stdout(f'roi = {roi} = {puzzle.cavern.wrap_rocks[-1]} + {d_last_rock}')
    d_last_height = puzzle.cavern.heights[roi] - puzzle.cavern.wrap_heights[-1]
    if params.verbose: stdout(f'd_last_height = {d_last_height} = {puzzle.cavern.heights[roi]} - {puzzle.cavern.wrap_heights[-1]}')
    answer = puzzle.cavern.wrap_heights[0] + times * (puzzle.cavern.wrap_heights[-1] - puzzle.cavern.wrap_heights[-2]) + d_last_height
    if params.verbose: stdout(f'answer = {answer} = {puzzle.cavern.wrap_heights[0]} + {times} * ({puzzle.cavern.wrap_heights[-1]} - {puzzle.cavern.wrap_heights[-2]}) + {d_last_height}')

...
class Cave(object):
    def __init__(self, paths):
        ...
        for x in range(self.min.y, self.max.y):
            ...
            for y in range(self.min.x, self.max.x):
                self.cave[-1].append('.')
        self.cave_max_y = len(self.cave)-1
        self.neg_cave = []
        for y in range(0, self.cave_max_y+1):
            self.neg_cave.append([])
        self.source = Point(500-self.min.x,0-self.min.y)
        self.set(self.source.x, self.source.y, '+')
        for path in paths:
            p1 = path[0]
        ...
        for x in range(0, len(self.cave[0])):
            self.cave[self.cave_max_y][x] = '_'
    def __str__(self) -> str:
        lines = []
    ...
    def get(self, x, y) -> str:
        if y >= self.cave_max_y:
            return '-'
        if x >= len(self.cave[y]):
            return '.'
        if x >= 0:
            return self.cave[y][x]
        nx = x * -1 - 1
        if nx >= len(self.neg_cave[y]):
            return '.'
        return self.neg_cave[y][nx]
    def set(self, x, y, val):
        printd(f'Setting: ({x},{y}) = {val}')
        if y > self.cave_max_y:
            raise ValueError(f'y {y} is too far down. Max is {self.cave_max.y}.')
        if x >= 0:
            while x >= len(self.cave[y]):
                self.cave[y].append('.')
            self.cave[y][x] = val
        else:
            nx = x * -1 -1
            while nx >= len(self.neg_cave[y]):
                self.neg_cave[y].append('.')
            self.neg_cave[y][nx] = val

    def drop(self, i) -> bool:
        cur = Point(self.source.x, self.source.y)
        printd(f'Starting drop {i}:')
        while True:
            printd(f'  at: {cur}')
            if self.get(cur.x, cur.y+1) == '.':
                printd('    straight')
                self.time += 1
                cur.y += 1
            elif self.get(cur.x-1, cur.y+1) == '.':
                printd('    left')
                self.time += 1
                cur.y += 1
                cur.x -= 1
                ...
                printd('    settles')
                self.set(cur.x, cur.y, 'o')
                return cur.x != self.source.x or cur.y != self.source.y
...
def solve(params) -> str:
    ...
    while cave.drop(answer+1):
        answer += 1
        if params.count > 0 and answer >= params.count:
            break
    answer += 1
    stdout('Cave:\n'+str(cave))

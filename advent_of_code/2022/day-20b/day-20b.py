...
class Node(object):
    ...
    def context(self, n = 0) -> str:
        rv = str(self.value)
        left = self.left
        right = self.right
        for _ in range(0, n):
            rv = f'{left.value}, {rv}, {right.value}'
            left = left.left
            right = right.right
        return rv
...
class EncryptedCoords(object):
    def __init__(self, nums):
        ...
        if len(nums) == 0:
            return
        self.entries.append(Node(self.nums[0]*self.key))
        for i in range(1, len(self.nums)):
            self.entries.append(self.entries[-1].add_right(self.nums[i]*self.key))
        mod_val = len(self.nums)-1
        for node in self.entries:
            if node.value == 0:
                self.zero = node
            elif node.value > 0:
                node.to_move = node.value % mod_val
                if abs(node.to_move - mod_val) < node.to_move:
                    node.to_move = node.to_move - mod_val
            elif node.to_move < 0:
                node.to_move = -1 * ((-1 * node.to_move) % mod_val)
                if mod_val + node.to_move < abs(node.to_move):
                    node.to_move = mod_val + node.to_move
    ...
    def calc_key(self, verbose = False) -> int:
        rv = 0
        cur = self.zero
        for _ in range(0, 3):
            for _ in range(0, 1000):
                cur = cur.right
            if verbose:
                stdout(f'key value: {cur.value}: {cur.context(3)}')
            rv += cur.value
        return rv
    def mix(self):
        for node in self.entries:
            if node.to_move > 0:
                for i in range(0, node.to_move):
                    node.move_right()
            elif node.to_move < 0:
                for i in range(0, node.to_move, -1):
                    node.move_left()
...
def solve(params) -> str:
    ...
    if params.verbose:
        stdout(f'Coords:\n{coords}')
    for i in range(1, 11):
        stdout(f'Mixing step {i}')
        coords.mix()
    stdout('Done Mixing.')
    answer = coords.calc_key(params.verbose)

...
class Node(object):
    ...
    def distance(self) -> int:
        return self.pressure * 100 - self.minute
    @property
    def minute(self) -> int:
        return len(self.path_to)
    def __str__(self) -> str:
        return f'{self.minute}: {self.action} {self.name} = {self.pressure}'
    def long_str(self) -> str:
        ...
        return '\n'.join(lines)
    def key(self) -> str:
        return f'{self.name}-{list(self.valves_open.keys())}'
    def spawn(self):
        rv = Node()
    ...
    def stomp(self, worse):
        worse.action = self.action
        worse.path = []
        worse.path.extend(self.path)
        worse.pressure = self.pressure
        worse.valves_open = {}
        for key, value in self.valves_open.items():
            worse.valves_open[key] = value
    def spawn_open(self, rate):
        rv = self.spawn()
        rv.name = self.name
        rv.action = 'open'
        rv.valves_open[rv.name] = rv.minute
        rv.pressure += rate * (_TIME_LIMIT - self.minute - 1)
        return rv
    def spawn_move(self, to):
        rv = self.spawn()
        rv.name = to
        rv.action = 'move'
        return rv
    def spawn_all(self, valves):
        ...
        if self.minute >= _TIME_LIMIT:
            return rv
        valve = valves[self.name]
        if self.name not in self.valves_open and valve.rate > 0:
            rv.append(self.spawn_open(valve.rate))
        for name in valve.connects_to:
            rv.append(self.spawn_move(name))
        return rv
...
class Solver(object):
    ...
    def solve(self):
        ...
        while len(self.queue) > 0:
            ...
            if show: stdout(f'Current State: Checked: {i}, {self}')
            i += 1
            node = self.queue.next()
            node.visited = True
            if node.pressure > max_pressure:
                if show: stdout(f'  New top score! {node}')
                max_pressure = n.pressure
                max_node = n
            nexts = node.spawn_all(self.valves)
            if show:
                s = f'  Considering: {node}, nexts:'
                for n in nexts:
                    s += ' ' + n.key()
                stdout(s)
            if len(nexts) > 0:
                for n in nexts:
                    ...
                    if k not in self.nodes:
                        if show: stdout(f'    Next: {n}, is new')
                        self.queue.add(n)
                        self.nodes[k] = n
                        continue
                    n2 = self.nodes[k]
                    if n2.visited:
                        if show: stdout(f'    Next: {n} already visited.')
                    elif n.distance > n2.distance:
                        if show: stdout(f'    Next: {n} is better than {n2}, stomping')
                        n.stomp(n2)
                    else:
                        if show: stdout(f'    Next: {n} is worse than {n2}')
            else:

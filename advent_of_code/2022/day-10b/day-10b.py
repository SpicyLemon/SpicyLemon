...
class CPU(object):
    ...
    def signal_strength(self) -> int:
        rv = 0
        for signal in self.of_interest:
            rv += signal[0] * signal[1]
        return rv

    def execute(self, inst):
        printd('executing ' + str(inst))
        if inst[0] == 'noop':
            self.nextCycle()
        elif inst[0] == 'addx':
            self.nextCycle()
    ...
    def nextCycle(self):
        self.cycle += 1
        if (self.cycle + 40) % 40 == 20:
            self.of_interest.append((self.cycle, self.x))
        if len(self.crt[-1]) == 40:
            self.crt.append('')
        pixel = '.'
        if abs(self.x - len(self.crt[-1])) <= 1:
            pixel = '#'
        self.crt[-1] += pixel
...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            if line != '':
                if line[0:4] == 'addx':
                    cmd, val = line.split(' ')
                    self.instructions.append((cmd, int(val)))
                else:
    ...
    def __str__(self) -> str:
        ...
        for inst in self.instructions:
            lines.append('%4s %4d' % (inst[0], inst[1]))
        return '\n'.join(lines)
...
def solve(params) -> str:
    ...
    cpu = CPU()
    for inst in puzzle.instructions:
        cpu.execute(inst)
    stdout(f'last cycle: {cpu.cycle}')

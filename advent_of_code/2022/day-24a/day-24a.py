        ...
        for y in range(0, self.max_y+1):
            for x in range(0, self.max_x+1):
                if self.lefts[x][y] != _O:
                    grid[y][x] = _L if grid[y][x] == _O else '3' if grid[y][x] == '2' else '4' if grid[y][x] == '3' else '2'
                if self.rights[self.max_x - x][y] != _O:
                    grid[y][x] = _R if grid[y][x] == _O else '3' if grid[y][x] == '2' else '4' if grid[y][x] == '3' else '2'
                if self.ups[y][x] != _O:
                    grid[y][x] = _U if grid[y][x] == _O else '3' if grid[y][x] == '2' else '4' if grid[y][x] == '3' else '2'
                if self.downs[self.max_y - y][x] != _O:
                    grid[y][x] = _D if grid[y][x] == _O else '3' if grid[y][x] == '2' else '4' if grid[y][x] == '3' else '2'
        return grid
    def __str__(self) -> str:
...
class State(object):
    def __init__(self, field = None, loc = None, target = None):
        self.field = field
        self.loc = loc
        self.target = target
        self.path_to = []
    ...
    def distance(self) -> int:
        return self.minute * 1000 + distance(self.loc, self.target)
    @property
    def minute(self) -> int:
        return len(self.path_to)
    @property
    def key(self) -> str:
        return f'{self.minute}-{self.loc}'
    def next_state(self, field, loc):
        rv = State(field, loc, self.target)
    ...
    def __str__(self) -> str:
        ...
        return grid_str_with_edges(grid)

def find_path(puzzle):
    starter = State(Field(puzzle.grid), puzzle.start, puzzle.end)
    queue = PQ()
    queue.add(starter)
    ...
    while len(queue) > 0:
        ...
        if _debug and i % 10000 == 0:
            stdout(f'Current: {cur.key} {cur.distance}. Known states: {len(states)}, Queue: {len(queue)}.\n{cur}')
        #if cur.minute > 5: return []
        if cur.loc.x == puzzle.end.x and cur.loc.y == puzzle.end.y-1:
            rv = cur.path_to
            rv.append(cur.loc)
            rv.append(puzzle.end)
            return rv

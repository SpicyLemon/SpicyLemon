...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            if line != '':
                e1, e2 = line.split(',')
                self.elf_pairs.append((Elf(e1), Elf(e2)))
...
def is_full_overlap(pair) -> bool:
    if pair[0].min >= pair[1].min and pair[0].max <= pair[1].max:
        return True
    if pair[1].min >= pair[0].min and pair[1].max <= pair[0].max:
        return True
    return False

def is_any_overlap(pair) -> bool:
    if pair[0].max < pair[1].min or pair[1].max < pair[0].min:
        return False
    return True

...
def add_digits(digits1, digits2):
    v1 = list(reversed(digits1))
    v2 = list(reversed(digits2))
    if len(v2) > len(v1):
        v1, v2 = v2, v1
    carry = 0
    rv = []
    for i in range(0, len(v1)):
        d = v1[i] + (0 if i >= len(v2) else v2[i]) + carry
        if d > 2:
            carry = 1
            d = d - 5
        elif d < -2:
    ...
    if carry != 0:
        rv.append(carry)
    rv.reverse()
    printd(f'{digits1} + {digits2} = {rv}')
    return rv

def sum_snafu(nums):
    rv = [0]
    for num in nums:
        rv = add_digits(rv, to_digits(num))
    return to_snafu(rv)
...
def solve(params) -> str:
    ...
    answer = ''.join(sum_snafu(puzzle.numbers))

...
class Puzzle(object):
    ...
    def __str__(self) -> str:
        ...
        lines = []
        for line in lines:
            lines.append(''.join(self.numbers))
        return '\n'.join(lines)

def convert(num) -> int:
    digits = []
    for digit in num:
        if digit == '1' or digit == '2' or digit == '0':
    ...
    for i in range(0, len(digits)):
        rv += pow(5, i) * digits[len(digits)-i-1]
    if _debug:
        n = ''.join(num)
        d = ''.join(list(map(str, digits)))
        stdout(f'{n:>20} -> {d:>20} = {rv:>20}')
    return rv

def convert_and_sum(numbers) -> int:
    rv = 0
    for num in numbers:
        rv += convert(num)
    return rv
...
def to_snafu(num) -> str:
    ...
    maxes = [2]
    to_max = ['2']
    while len(pows) == 0 or maxes[-1] <= num:
        pows.append(pow(5, len(pows)))
        to_max.append('2')
        maxes.append(convert(to_max))
    pows.reverse()
    maxes.reverse()
    digits = []
    left = num
    for d in range(0, len(pows)-1):
        for n in [2, 1, 0, -1, -2]:
            if pows[d] * n - left <= maxes[d+1]:
                printd(f'{pows[d]} * {n} - {left} <= {maxes[d+1]}')
                digits.append(n)
                left = left - pows[d] * n
                printd(f'Left: {left}, Digits: {digits}')
                break
            else:
                printd(f'{pows[d]} * {n} - {left} > {maxes[d+1]}')
    digits.append(left)
    printd(f'Digits: {digits}')
    rv = ''
    for d in digits:
        if d == -2:
            rv += '='
...
def solve(params) -> str:
    ...
    num = convert_and_sum(puzzle.numbers)
    stdout(f'Num: {num}')
    answer = to_snafu(num)

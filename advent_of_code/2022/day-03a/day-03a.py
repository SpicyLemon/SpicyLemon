...
class Rucksack(object):
    def __init__(self, line):
        ...
        self.contents = line
        self.compartments = (line[0:comp_len], line[comp_len:])
        self.shared = None
    ...
    def common(self) -> str:
        if self.shared != None:
            return self.shared
        for i in self.compartments[0]:
            for j in self.compartments[1]:
                if i == j:
                    self.shared = i
                    return i
        return None
...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            if line != '':
                self.sacks.append(Rucksack(line))
...
def solve(params) -> str:
    ...
    total = 0
    for sack in puzzle.sacks:
        total += get_priority(sack.common)
    answer = total

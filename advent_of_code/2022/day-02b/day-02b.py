...
def to_throw(val: str) -> Throw:
    ...
    raise ValueError(f'Unknown Throw encoding: [{val}].')

def calc_throw(theirs: Throw, plan: str) -> (Throw, Throw):
    if plan == 'X':
        # Lose
        if theirs == Throw.Rock:
            return (theirs, Throw.Scissors)
        elif theirs == Throw.Paper:
            return (theirs, Throw.Rock)
        else:
            return (theirs, Throw.Paper)
    elif plan == 'Y':
        # Tie
        ...
        if theirs == Throw.Rock:
            ...
            return (theirs, Throw.Rock)
    raise ValueError(f'Unknown plan encoding: [{val}].')

def game_string(game) -> str:
    return str(game[0]) + ' ' + str(game[1])
...
class Puzzle(object):
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            theirs, mine = line.split(' ', 1)
            self.games.append(calc_throw(to_throw(theirs), mine))
...
def score_game(game) -> int:
    ...
    if game[0] == game[1]:
        outcome = 3
    elif game[0].value % 3 == game[1].value - 1:
        outcome = 6
    return outcome + game[1].value
...
def solve(params) -> str:
    ...
    total = 0
    for game in puzzle.games:
        score = score_game(game)
        printd(game_string(game) + f' = {score}')
        total += score
    answer = total

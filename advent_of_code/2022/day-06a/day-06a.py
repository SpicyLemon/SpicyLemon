...
def find_unique(data_stream: str, count: int) -> int:
    for i in range(count, len(data_stream)):
        chars = to_chars(data_stream[i-count:i])
        is_uniq = True
...
def solve(params) -> str:
    ...
    if len(params.custom) > 0:
        data_stream = params.custom[0]
    else:
        puzzle = Puzzle(params.input)
        data_stream = puzzle.data_stream
    answer = find_unique(data_stream, params.count)

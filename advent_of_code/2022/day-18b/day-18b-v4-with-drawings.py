...
class Space(object):
    def __init__(self, points):
        ...
        for z in range(0, self.maximums.z+1):
            for y in range(0, self.maximums.y+1):
                for x in [0, self.maximums.x]:
                    if self.cells[z][y][x] == None:
                        self.cells[z][y][x] = _WATER
            for y in [0, self.maximums.y]:
                for x in range(0, self.maximums.x+1):
                    if self.cells[z][y][x] == None:
                        self.cells[z][y][x] = _WATER
        for z in [0, self.maximums.z]:
            for y in range(0, self.maximums.y+1):
                for x in range(0, self.maximums.x+1):
                    if self.cells[z][y][x] == None:
                        self.cells[z][y][x] = _WATER
        ...
        while made_change:
            made_change = False
            for z  in range(1, self.maximums.z):
                for y in range(1, self.maximums.y):
                    for x in range(1, self.maximums.x):
                        if self.cells[z][y][x] == None:
                            if _WATER in self.get_neighbors(x, y, z):
                                self.cells[z][y][x] = _WATER
                                made_change = True
    def get_neighbors(self, x, y, z):
        rv = []
        if x > 0: rv.append(self.cells[z][y][x-1])
        if x < self.maximums.x: rv.append(self.cells[z][y][x+1])
        if y > 0: rv.append(self.cells[z][y-1][x])
        if y < self.maximums.y: rv.append(self.cells[z][y+1][x])
        if z > 0: rv.append(self.cells[z-1][y][x])
        if z < self.maximums.z: rv.append(self.cells[z+1][y][x])
        return rv
    def __str__(self) -> str:
        lines = []
        gh = grid_header(self.maximums.y)
        gf = grid_footer(self.maximums.y)
        for z in range(0, self.maximums.z+1):
            lines.append(f'z: {z}')
            lines.append(gh)
            for y in range(0, self.maximums.y+1):
                lines.append(f'{y:>2}' + ''.join(list(map(lambda v : ' ' if v == _WATER else '#' if v == _LAVA else '.', self.cells[z][y]))) + f'{y}')
            lines.append(gf)
            if z != self.maximums.z:
                lines.append('')
        return '\n'.join(lines)
...
def grid_header(width) -> str:
    return '\n'.join(grid_header_lines(width))

def grid_footer(width) -> str:
    return '\n'.join(reversed(grid_header_lines(width)))

def surface_area(points, space) -> int:
    rv = 0
    for p in points:
        x, y, z = p.x, p.y, p.z
        if x == 0 or space.cells[z][y][x-1] == _WATER:
            rv += 1
        if x == space.maximums.x or space.cells[z][y][x+1] == _WATER:
            rv += 1
        if y == 0 or space.cells[z][y-1][x] == _WATER:
            rv += 1
        if y == space.maximums.y or space.cells[z][y+1][x] == _WATER:
            rv += 1
        if z == 0 or space.cells[z-1][y][x] == _WATER:
            rv += 1
        if z == space.maximums.z or space.cells[z+1][y][x] == _WATER:
            rv += 1
    return rv

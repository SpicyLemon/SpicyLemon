...
class Node(object):
    ...
    @property
    def distance(self):
        if len(self.path_to) == 0:
            return 0
        return distance(self.point, self.path_to[0])
    @property
    def key(self):
...
def is_edge(point, maximums) -> bool:
    return point.x == 0 or point.y == 0 or point.z == 0 or point.x == maximums.x or point.y == maximums.y or point.z == maximums.z

def copy_append(l, v=None):
    rv = []
    if len(l) > 0:
        rv.extend(l)
    if v != None:
        ...
        for z in range(0, self.maximums.z+1):
            ...
            for y in range(0, self.maximums.y+1):
                ...
                for x in range(0, self.maximums.x+1):
                    self.cells[-1][-1].append(Node(x, y, z))
        for p in points:
            self.cells[p.z][p.y][p.x].type = _LAVA
    def get(self, x, y, z):
        return self.cells[z][y][x]
    def get_neighbors(self, point):
        rv = []
        x, y, z = point.x, point.y, point.z
        if x > 0: rv.append(self.get(x-1, y, z))
        if x < self.maximums.x: rv.append(self.get(x+1, y, z))
        if y > 0: rv.append(self.get(x, y-1, z))
        if y < self.maximums.y: rv.append(self.get(x, y+1, z))
        if z > 0: rv.append(self.get(x, y, z-1))
        if z < self.maximums.z: rv.append(self.get(x, y, z+1))
        return rv
    def is_edge(self, point) -> bool:
        return point.x == 0 or point.y == 0 or point.z == 0 or point.x == self.maximums.x or point.y == self.maximums.y or point.z == self.maximums.z
    def is_water(self, x, y, z) -> bool:
        node = self.get(x, y, z)
        if node.type == _WATER:
            return True
        if node.type == _LAVA or node.type == _INTERNAL:
            return False
        if self.is_edge(node):
            node.type = _WATER
            return True
        printd(f'Finding node type: {node}')
        queue = PQ(node)
        visited = {}
        nodes = {node.key: node}
        all_are = _UNKNOWN
        while len(queue) > 0:
            cur = queue.next()
            printd(f'  Checking node: {cur}')
            visited[cur.key] = cur
            neighbors = self.get_neighbors(cur)
            for n in neighbors:
                if n.key in visited or n.type == _LAVA:
                    continue
                if n.key not in nodes:
                    n.path_to = copy_append(cur.path_to, cur)
                    queue.add(n)
                    nodes[n.key] = n
                if n.type in [_WATER, _INTERNAL]:
                    printd(f'    found {n}')
                    all_are = n.type
                elif self.is_edge(n):
                    printd(f'    found edge: {n}')
                    all_are = _WATER
            if all_are != _UNKNOWN:
                break
        if all_are == _UNKNOWN:
            printd('  Nothing left to check. Must be {_INTERNAL}')
            all_are = _INTERNAL
        for n in nodes.values():
            n.type = all_are
        return self.is_water(x, y, z)

def surface_area(points, space) -> int:
    rv = 0
    for p in points:
        if p.x == 0 or space.is_water(p.x-1, p.y, p.z):
            rv += 1
        if p.x == space.maximums.x or space.is_water(p.x+1, p.y, p.z):
            rv += 1
        if p.y == 0 or space.is_water(p.x, p.y-1, p.z):
            rv += 1
        if p.y == space.maximums.y or space.is_water(p.x, p.y+1, p.z):
            rv += 1
        if p.z == 0 or space.is_water(p.x, p.y, p.z-1):
            rv += 1
        if p.z == space.maximums.z or space.is_water(p.x, p.y, p.z+1):
            rv += 1
    return rv

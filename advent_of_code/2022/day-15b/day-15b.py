...
class Sensor(object):
    def __init__(self, location, beacon):
        ...
        self.beacon = beacon
        self.radius = distance(location, beacon)
    def __str__(self) -> str:
        return f'At: {self.location}, Closest beacon: {self.beacon}, Radius: {self.radius}'
    ...
    def __init__(self, lines):
        ...
        for line in lines:
            if line != '':
                ...
                if m:
                    self.sensors.append(Sensor(Point(int(m.group(1)), int(m.group(2))), Point(int(m.group(3)), int(m.group(4)))))
                else:
...
def rotate(point, center):
    #orig = Point(point.x, point.y)
    if point.x >= center.x and point.y < center.y:
        point.y += 1
        point.x += 1
        #printd(f'Upper right: {orig} -> {point} around {center}, distance = {distance(point, center)}')
    elif point.x > center.x and point.y >= center.y:
        point.y += 1
        point.x -= 1
        #printd(f'Lower right: {orig} -> {point} around {center}, distance = {distance(point, center)}')
    elif point.x <= center.x and point.y > center.y:
        point.y -= 1
        point.x -= 1
        #printd(f'Lower left: {orig} -> {point} around {center}, distance = {distance(point, center)}')
    elif point.x < center.x and point.y <= center.y:
        point.y -= 1
        point.x += 1
        #printd(f'Upper left: {orig} -> {point} around {center}, distance = {distance(point, center)}')
    else:
        raise ValueError(f'WTF: Point: {point}, Center: {center}')
...
class Cave(object):
    def __init__(self, sensors):
        ...
        self.max = None
        if len(sensors) > 0:
            self.min = Point(sensors[0].location.x, sensors[0].location.y)
            self.max = Point(sensors[0].location.x, sensors[0].location.y)
        for s in sensors:
            if s.location.x < self.min.x:
                self.min.x = s.location.x
            if s.location.x > self.max.x:
                self.max.x = s.location.x
            if s.location.y < self.min.y:
                self.min.y = s.location.y
            if s.location.y > self.max.y:
                self.max.y = s.location.y
            if s.beacon.x < self.min.x:
                self.min.x = s.beacon.x
            if s.beacon.x > self.max.x:
                self.max.x = s.beacon.x
            if s.beacon.y < self.min.y:
                self.min.y = s.beacon.y
            if s.beacon.y > self.max.y:
                self.max.y = s.beacon.y
        self.width = self.max.x - self.min.x + 1
        self.height = self.max.y - self.min.y + 1
        #printd(f'Min: {self.min}, Max: {self.max}')
        #printd(f'Width: {self.width}, Height: {self.height}')
    def get(self, p) -> str:
        if p.x < self.min.x or p.x > self.max.x or p.y < self.min.y or p.y > self.max.y:
            return '.'
        return self.grid[p.y-self.min.y][p.x-self.min.x]
    def set(self, p, val):
        if p.x < self.min.x:
            raise ValueError(f'x value {p.x} below min {self.min.x}')
        if p.x > self.max.x:
            raise ValueError(f'x value {p.x} above max {self.max.x}')
        if p.y < self.min.y:
            raise ValueError(f'y value {p.y} above min {self.min.y}')
        if p.y > self.max.y:
            raise ValueError(f'y value {p.y} above max {self.max.y}')
        self.grid[p.y-self.min.y][p.x-self.min.x] = val
    def can_be_missing_beacon(self, p, maximum) -> bool:
        if p.x < 0 or p.y < 0 or p.x > maximum or p.y > maximum:
            printd(f'Out of range: {p}')
            return False
        for s in self.sensors:
            if p.x == s.beacon.x and p.y == s.beacon.y:
                printd(f'Already beacon: {p}')
                return False
            if p.x == s.location.x and p.y == s.location.y:
                printd(f'Is a sensor: {p}')
                return False
            d = distance(s.location, p)
            printd(f'Is {d} away from Sensor {s}')
            if d <= s.radius:
                return False
        stdout(f'Bingo: {p}')
        return True
    def find_frequency(self, maximum, verbose) -> int:
        for sensor in sorted(self.sensors, key=lambda x: x.radius, reverse=False):
            if verbose:
                stdout(f'Orbiting around sensor {sensor}')
            start = Point(sensor.location.x, sensor.location.y-sensor.radius-1)
            #printd(f'  Starting point: {start}')
            test = Point(start.x, start.y)
            while True:
                if self.can_be_missing_beacon(test, maximum):
                    return 4000000 * test.x + test.y
                if _debug:
                    printd(f'  Not {test}, which is {distance(sensor.location, test)} away.')
                rotate(test, sensor.location)
                if test.x == start.x and test.y == start.y:
                    if _debug:
                        printd(f'  Back to start {test}, which is {distance(sensor.location, test)} away.')
                    break
        return -1
    ...
    def __str__(self) -> str:
        ...
        lines.append(f'Width: {self.width}, Height: {self.height}')
...
def solve(params) -> str:
    ...
    if params.verbose:
        stdout('Cave:\n'+str(cave))
        stdout(f'Max: {maximum}')
    answer = cave.find_frequency(maximum, params.verbose)

...
sub mainProgram1 {
    ...
    my $sues = parseSues();
    print 'There are '.(scalar keys %$sues).' Sues to start.'."\n";
    foreach my $param (keys %$params) {
        filterSues($param, $params->{$param}, $sues);
        print 'After filtering on '.$param.' = '.$params->{$param}.' there are '.(scalar keys %$sues).' Sues.'."\n";
    }
    print Dumper($sues)."\n";
}
...
sub mainProgram2 {
    ...
    print 'There are '.(scalar keys %$sues).' Sues to start.'."\n";
    foreach my $param (keys %$params) {
        filterSuesP2($param, $params->{$param}, $sues);
        print 'After filtering on '.$param.' = '.$params->{$param}.' there are '.(scalar keys %$sues).' Sues.'."\n";
    }
    print Dumper($sues)."\n";
}
...
sub filterSuesP2 {
    ...
    my $amount = shift;
    my $sues = shift;
    my @sueNums = keys %$sues;
    foreach my $sueNum (@sueNums) {
        if (exists $sues->{$sueNum}->{$param}) {
            if ($param eq 'cats' || $param eq 'trees') {
                if ($sues->{$sueNum}->{$param} <= $amount) {
                    delete $sues->{$sueNum};
                }
            }
            elsif ($param eq 'pomeranians' || $param eq 'goldfish') {
                if ($sues->{$sueNum}->{$param} >= $amount) {
                    delete $sues->{$sueNum};
                }
            }
...
sub filterSues {
    ...
    my @sueNums = keys %$sues;
    foreach my $sueNum (@sueNums) {
        if (exists $sues->{$sueNum}->{$param} && $sues->{$sueNum}->{$param} != $amount) {
            delete $sues->{$sueNum};
        }
    }
}
    ...
    foreach my $line (split("\n", getParamsStr())) {
        ...
        if (length($line) != 0) {
            if ($line =~ m{^(\w+): (\d+)$}) {
                ...
                die "Could not parse param line: $line";
            }
        }
    }
    return \%rv;
...
sub getSuesString {
    my $sues = shift;
    my @lines = ();
    foreach my $sueNum (sort { $a <=> $b } keys %$sues) {
        push (@lines, sprintf('Sue %3d => %s', $sueNum, getSueString($sues->{$sueNum})))
    }
    return join("\n", @lines);
}

sub getSueString {
    return Data::Dumper->new([$_[0]])->Indent(0)->Terse(1)->Dump();
}
    ...
    foreach my $line (split("\n", getStr())) {
        ...
        if (length($line) != 0) {
            if ($line =~ m{^Sue (\d+): (.*)$}) {
                ...
                die "Could not parse Sue line: $line";
            }
        }
    }
    return \%rv;
...
sub getStr {
   return q^Sue 1: children: 1, cars: 8, vizslas: 7

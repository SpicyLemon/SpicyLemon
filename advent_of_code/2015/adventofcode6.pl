...
sub mainProgram1 {
   my $grid = getNewGrid(1000,1000, 0);
   my $instructions = getInstructions();
   foreach my $inst (@$instructions) {
      processInstruction1($grid, $inst->{action}, $inst->{x1}, $inst->{y1}, $inst->{x2}, $inst->{y2});
   }
   my ($on, $off) = countLights($grid);
   print "On : $on\n";
   print "Off: $off\n";
}
...
sub mainProgram2 {
   ...
   my $instructions = getInstructions();
   foreach my $inst (@$instructions) {
      processInstruction2($grid, $inst->{action}, $inst->{x1}, $inst->{y1}, $inst->{x2}, $inst->{y2});
   }
   my $total_brightness = getTotalBrightness($grid);
   print "Total Brightness: $total_brightness\n";
}


sub processInstruction1 {
   my $grid = shift;
   my $action = shift;
   my $x1 = shift;
   my $y1 = shift;
   ...
   foreach my $x (getRange($x1, $x2)) {
      foreach my $y (getRange($y1, $y2)) {
         $grid->[$x]->[$y] = $action eq 'turn on'  ? 1
                           : $action eq 'turn off' ? 0
                           : $action eq 'toggle'   ? ($grid->[$x]->[$y] + 1) % 2
                           :                         $grid->[$x]->[$y];
      }
   }
}
...
sub processInstruction2 {
   ...
   foreach my $x (getRange($x1, $x2)) {
      foreach my $y (getRange($y1, $y2)) {
         $grid->[$x]->[$y] = $action eq 'turn on'  ? $grid->[$x]->[$y] + 1
                           : $action eq 'turn off' ? $grid->[$x]->[$y] - 1
                           : $action eq 'toggle'   ? $grid->[$x]->[$y] + 2
                           :                         $grid->[$x]->[$y];
         if ($grid->[$x]->[$y] < 0) {
            $grid->[$x]->[$y] = 0;
         }
...
sub getNewGrid {
   ...
   my $val = shift;
   my @retval = ();
   foreach my $x (getRange(0, $x-1)) {
      my @row = ();
      foreach my $y (getRange(0, $y-1)) {
         push (@row, $val);
      }
      push (@retval, \@row);
   }
...
sub getInstructions {
   ...
   foreach my $line (split(/\n/, getString())) {
      if ($line =~ /^(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)$/) {
         ...
         print "error parsing instruction: '$line'\n";
      }
   }
   return \@retval;
}
...
sub getString {
return q^turn on 489,959 through 759,964

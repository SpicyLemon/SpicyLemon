...
func Solve(params *Params) (string, error) {
	...
	for y, row := range input.Grid {
		for x, cell := range row {
			...
			p := NewPoint(x, y)
			count := CountRoleNeighbors(input.Grid, p)
			if count < 4 {
				aRoles = append(aRoles, p)
			}
		}
	}
	if debug {
		Stderrf("%s", CreateIndexedGridString(input.Grid, aRoles, aRoles))
	}
	answer := len(aRoles)
...
func CountRoleNeighbors(grid [][]string, p *Point) int {
	rv := 0
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if dx == 0 && dy == 0 {
			...
			x, y := p.X+dx, p.Y+dy
			if y >= 0 && y < len(grid) && x >= 0 && x < len(grid[y]) && grid[y][x] == Role {
				rv++
			}
		}
	}
...
func (i Input) String() string {
	return CreateIndexedGridString(i.Grid, []XY(nil), nil)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Grid: make([][]string, len(lines))}
	for i, line := range lines {
		rv.Grid[i] = make([]string, len(line))
		for j, r := range line {
			rv.Grid[i][j] = string(r)
		}
	}
	return &rv, nil
}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := 0
	for i, bank := range input.Banks {
		mj := FindMaxJoltage(bank)
		Debugf("%3d: %d <- %s", i, mj, bank)
		answer += mj
	}
	return fmt.Sprintf("%d", answer), nil
}

func FindMaxJoltage(bank Bank) int {
	lastI := 0
	ds := make([]int, 12)
	for k := range ds {
		for i := lastI; i < len(bank)-(11-k); i++ {
			d := bank[i]
			if d > ds[k] {
				ds[k] = d
				lastI = i + 1
	...
	rvStr := strings.Join(MapSlice(ds, strconv.Itoa), "")
	rv, _ := strconv.Atoi(rvStr)
	return rv
}
...
func (i Input) String() string {
	return StringNumberJoin(i.Banks, 1, "\n")
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Banks: make([]Bank, len(lines))}
	for i, line := range lines {
		rv.Banks[i] = ParseBank(line)
	}
	return &rv, nil
}

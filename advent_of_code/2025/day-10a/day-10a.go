...
func Solve(params *Params) (string, error) {
	...
	for i, machine := range input.Machines {
		ans := CountButtonsToTarget(machine)
		answer += ans
...
func CountButtonsToTarget(machine *Machine) int {
	...
	for i := 1; i < 100_000; i++ {
		...
		for _, lights := range lastSet {
			toCheck := machine.ApplyAllButtons(lights)
...
func (b Button) Apply(lights []byte) []byte {
...
func (m *Machine) String() string {
	...
	return fmt.Sprintf("[%s] %s %s", m.Target, StringJoin(m.Buttons, " "), m.Joltage)
}
...
func (m *Machine) ApplyAllButtons(lights []byte) [][]byte {
	rv := make([][]byte, len(m.Buttons))
	for i, button := range m.Buttons {
		rv[i] = button.Apply(lights)
	}
	return rv
}

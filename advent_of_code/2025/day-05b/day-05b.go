...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	slices.SortFunc(input.Fresh, (*IDRange).Compare)
	keepGoing := true
	for keepGoing {
		input.Fresh, keepGoing = ReduceRanges(input.Fresh)
		Debugf("After reduction:\n%s", input)
	}
	answer := 0
	for _, r := range input.Fresh {
		answer += r.Len()
	}
	return fmt.Sprintf("%d", answer), nil
}

func ReduceRanges(ranges []*IDRange) ([]*IDRange, bool) {
	rv := make([]*IDRange, 0, len(ranges))
	for i := 0; i < len(ranges)-1; i++ {
		for j := i + 1; j < len(ranges); j++ {
			if nr := CombineRanges(ranges[i], ranges[j]); nr != nil {
				rv = append(rv, nr)
				for k := j + 1; k < len(ranges); k++ {
					rv = append(rv, ranges[k])
				}
				return rv, true
			}
		}
		rv = append(rv, ranges[i])
	}
	return ranges, false
}

// Combine returns a range that covers both this and the provided ranges if possible, nil otherwise.
func CombineRanges(r1, r2 *IDRange) *IDRange {
	switch {
	...
	case r2.Min <= r1.Min && r1.Max <= r2.Max:
		// e.g. 2-5 and 1-10.
		return r2
	case r1.Min <= r2.Min && r2.Min <= r1.Max && r1.Max <= r2.Max:
		// E.g. 1-5 and 2-6.
		return &IDRange{Min: r1.Min, Max: r2.Max}
	case r2.Min <= r1.Min && r1.Min <= r2.Max && r2.Max <= r1.Max:
		// E.g. 2-6 and 1-5.
		return &IDRange{Min: r2.Min, Max: r1.Max}
	default:
		return nil
	}
}
...
func ParseIDRange(line string) (*IDRange, error) {
	parts := strings.Split(line, "-")
	if len(parts) != 2 {
		return nil, fmt.Errorf("unknown ID Range format: %q", line)
	}
	var err error
	rv := &IDRange{}
	rv.Min, err = strconv.Atoi(parts[0])
	if err != nil {
		return nil, fmt.Errorf("invalid ID Range min %q (from %q): %w", parts[0], line, err)
	}
	rv.Max, err = strconv.Atoi(parts[1])
	if err != nil {
		return nil, fmt.Errorf("invalid ID Range max %q (from %q): %w", parts[1], line, err)
	}
	return rv, nil
}
...
func (r *IDRange) Contains(id int) bool {
	return r != nil && r.Min <= id && r.Max >= id
}
...
func (r *IDRange) Compare(r2 *IDRange) int {
	if r == r2 {
		return 0
	}
	if r == nil {
...
func (i Input) String() string {
	return StringNumberJoin(i.Fresh, 1, "\n") //  + "\n" + "\n" + strings.Join(MapSlice(i.IDs, strconv.Itoa), "\n")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		...
		if inRanges {
			r, err := ParseIDRange(line)
			if err != nil {
				return nil, fmt.Errorf("[%d]: invalid ID Range: %w", i, err)
			}
			rv.Fresh = append(rv.Fresh, r)
			continue
		}

		id, err := strconv.Atoi(line)
		if err != nil {
			return nil, fmt.Errorf("[%d]: invalid ID: %w", i, err)
		}

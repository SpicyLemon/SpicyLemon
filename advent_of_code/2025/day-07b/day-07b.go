...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	beamGrid := DrawBeams(input.Grid)
	if debug {
		Stderrf("With Beams:\n%s", CreateIndexedGridString(beamGrid, []*Point{input.Start}, input.Splitters))
	}
	usedSplitters := CountUsedSplitters(beamGrid)
	if debug {
		Stderrf("With used splitters:\n%s", CreateIndexedGridString(beamGrid, usedSplitters, input.Splitters))
	}
	answer := CountBeams(beamGrid)
	return fmt.Sprintf("%d", answer), nil
}

func CountBeams(grid [][]string) int {
	beamCounts := make([][]int, len(grid))
	for y, row := range grid {
		beamCounts[y] = make([]int, len(row))
		for x, cell := range row {
			...
			// Add the number from the cell above.
			beamCounts[y][x] += beamCounts[y-1][x]
			// If the cell to the left is a splitter, add the number from the beam above it.
			if x > 0 && grid[y][x-1] == Splitter {
				beamCounts[y][x] += beamCounts[y-1][x-1]
			}
			// If the cell to the right is a splitter, add the number from the beam above it.
			if x < len(row)-1 && grid[y][x+1] == Splitter {
				beamCounts[y][x] += beamCounts[y-1][x+1]
			}
		}
	}
	...
	if debug {
		Stderrf("Beam counts:\n%s", CreateIndexedGridStringNums(beamCounts, []XY(nil), nil))
	}

	rv := 0
	lastY := len(grid) - 1
	for _, count := range beamCounts[lastY] {
		rv += count
	}
	return rv
}

func CountUsedSplitters(grid [][]string) []*Point {
	var rv []*Point
	for y := 1; y < len(grid); y++ {
		for x, cell := range grid[y] {
			if cell == Splitter && grid[y-1][x] == Beam {
...
func DrawBeams(grid [][]string) [][]string {
	rv := make([][]string, len(grid))
	for y, row := range grid {
		rv[y] = make([]string, len(row))
		for x, cell := range row {
			switch {
			case y == 0 || cell == Splitter:
				// If we're on the top row, or a splitter, copy what's already in the grid.
				rv[y][x] = grid[y][x]
			case grid[y-1][x] == Start || rv[y-1][x] == Beam:
				// If it's under the start or a beam, it's a beam.
				rv[y][x] = Beam
			case x > 0 && grid[y][x-1] == Splitter && rv[y-1][x-1] == Beam:
				// If it's to the left of a Splitter that is hit by a beam, it's a beam!
				rv[y][x] = Beam
			case x < len(row)-1 && grid[y][x+1] == Splitter && rv[y-1][x+1] == Beam:
				// If it's to the right of a Splitter that is hit by a beam, it's a beam!
				rv[y][x] = Beam
			default:
				// It's empty space.
				rv[y][x] = Space
...
func (i Input) String() string {
	return CreateIndexedGridString(i.Grid, []*Point{i.Start}, i.Splitters)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{}
	rv.Grid = ParseGridOfStrings(lines)
	for y, row := range rv.Grid {
		for x, cell := range row {
			switch cell {
			case Start:
				rv.Start = NewPoint(x, y)
			case Splitter:
				rv.Splitters = append(rv.Splitters, NewPoint(x, y))
			}
		}
	}
	return &rv, nil

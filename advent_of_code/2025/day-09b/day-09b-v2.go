...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	pairs := MakePairs(input.RedTiles)
	if debug {
		Stderrf("Pairs (%d):\n%s", len(pairs), StringNumberJoin(pairs, 1, "\n"))
	} else if verbose {
		Stderrf("Pairs (%d)", len(pairs))
	}
	pairs = FilterForPairsWithoutPoints(pairs, input.RedTiles)
	if debug {
		Stderrf("Empty pairs (%d):\n%s", len(pairs), StringNumberJoin(pairs, 1, "\n"))
	} else if verbose {
		Stderrf("Empty pairs (%d)", len(pairs))
	}
	pairs = FilterForPairsWithoutLines(pairs, input.RedTiles)
	if debug {
		Stderrf("Pairs without intersections (%d):\n%s", len(pairs), StringNumberJoin(pairs, 1, "\n"))
	} else if verbose {
		Stderrf("Pairs without intersections (%d)", len(pairs))
	}
	Verbosef("Largest: %s", pairs[0])
	answer := pairs[0].Area
	return fmt.Sprintf("%d", answer), nil
}

func FilterForPairsWithoutPoints(pairs []*Pair, points []*Point) []*Pair {
	var rv []*Pair
	for i, pair := range pairs {
		...
		for j, point := range points {
			if pair.Contains(point) {
				hasPoint = true
				Debugf("[%d]:%s contains %s:[%d]", i+1, pair, point, j+1)
				break
			}
		}
		if !hasPoint {
			Debugf("[%d]: %s is empty!", i+1, pair)
			rv = append(rv, pair)
		}
	}
	return rv
...
func FilterForPairsWithoutLines(pairs []*Pair, points []*Point) []*Pair {
	var rv []*Pair
	for i, pair := range pairs {
		hasPoint := false
		for j, p1 := range points {
			k := (j + 1) % len(points)
			p2 := points[k]
			if pair.Intersects(p1, p2) {
				hasPoint = true
				Debugf("[%d]:%s is intersected by [%d]%s-%s[%d]", i+1, pair, j+1, p1, p2, k+1)
				break
			}
		}
		if !hasPoint {
			Debugf("[%d]:%s has no intersections", i+1, pair)
			rv = append(rv, pair)
		}
	}
	return rv
...
func FilterForPairsWithoutLinesOld(pairs []*Pair, points []*Point) []*Pair {
	var rv []*Pair
	for i, pair := range pairs {
		hasPoint := false
		for j, p1 := range points {
			k := (j + 1) % len(points)
			p2 := points[k]
			for _, midPoint := range GetLinePoints(p1, p2) {
				if pair.Contains(midPoint) {
					hasPoint = true
					Debugf("[%d]:%s is intersected by [%d]%s-%s[%d]", i+1, pair, j+1, p1, p2, k+1)
					break
				}
...
func GetLinePoints(p1, p2 *Point) []*Point {
	if p1.X == p2.X {
		start, stop := MinMax(p1.Y, p2.Y)
		rv := make([]*Point, 0, stop-start+1)
		for y := start; y <= stop; y++ {
			rv = append(rv, NewPoint(p1.X, y))
		}
		return rv
	}
	if p1.Y == p2.Y {
		start, stop := MinMax(p1.X, p2.X)
		rv := make([]*Point, 0, stop-start+1)
		for x := start; x <= stop; x++ {
			rv = append(rv, NewPoint(x, p1.Y))
		}
		return rv
	}
	panic(fmt.Errorf("points %s and %s are not on a line"))
}

func MakePairs(points []*Point) []*Pair {
	var rv []*Pair
	for i := 0; i < len(points)-1; i++ {
		for j := i + 1; j < len(points); j++ {
			rv = append(rv, NewPair(points[i], points[j]))
		}
	}
	slices.SortFunc(rv, func(l, r *Pair) int {
		return cmp.Compare(r.Area, l.Area)
	})
	return rv
}
...
func NewPair(p1, p2 *Point) *Pair {
	...
	rv.MinX, rv.MaxX = MinMax(p1.X, p2.X)
	rv.MinY, rv.MaxY = MinMax(p1.Y, p2.Y)
	return rv
}
...
func (p *Pair) String() string {
	...
	return fmt.Sprintf("%s--%d--%s", p.P1, p.Area, p.P2)
}

// Contains returns true if the given point is inside (exclusive) of this pair's square.
// It returns false if the point is on an edge or outside this pair's square.
func (p *Pair) Contains(point *Point) bool {
	return p.MinX < point.X && point.X < p.MaxX && p.MinY < point.Y && point.Y < p.MaxY
}

func (p *Pair) Intersects(p1, p2 *Point) bool {
	if p.Contains(p1) || p.Contains(p2) {
		return true
	}
	if p1.X == p2.X {
		if p1.X <= p.MinX || p.MaxX <= p1.X {
			return false
		}
		minY, maxY := MinMax(p1.Y, p2.Y)
		if minY <= p.MinY && p.MaxY <= maxY {
			return true
		}
		return false
	}
	if p1.Y == p2.Y {
		if p1.Y <= p.MinY || p.MaxY <= p1.Y {
			return false
		}
		minX, maxX := MinMax(p1.X, p2.X)
		if minX <= p.MinX && p.MaxX <= maxX {
			return true
		}
		return false
	}
...
func ParsePoint(line string) (*Point, error) {
	parts, err := SplitParseIntsD(line, ",")
	if err != nil {
		return nil, fmt.Errorf("invalid point %q: %w", line, err)
	}
	if len(parts) != 2 {
		return nil, fmt.Errorf("unknown point format %q", line)
	}
	return NewPoint(parts[0], parts[1]), nil
}
...
func (i Input) String() string {
	return StringNumberJoin(i.RedTiles, 1, "\n")
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{RedTiles: make([]*Point, len(lines))}
	var err error
	for i, line := range lines {
		rv.RedTiles[i], err = ParsePoint(line)
		if err != nil {

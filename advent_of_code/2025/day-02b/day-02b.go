...
func Solve(params *Params) (string, error) {
	...
	for i, r := range input.Ranges {
		found := FindAllInvalidIDs(r.Min, r.Max)
		Debugf("[%d]: %s: %d", i, r, found)
...
func FindAllInvalidIDs(start, stop int) []int {
	lenMin := len(strconv.Itoa(stop))
	vals := make(map[int]bool)
	for i := 2; i <= lenMin; i++ {
		for _, val := range FindInvalidIDs(start, stop, i) {
			vals[val] = true
		}
	}
	return slices.Sorted(maps.Keys(vals))
}

func FindInvalidIDs(start, stop, repl int) []int {
	// defer FuncEnding(FuncStarting(start, stop, repl))
	startStr := strconv.Itoa(start)
	curStr := startStr[:len(startStr)/repl]
	cur, _ := strconv.Atoi(curStr)
	var rv []int
	for {
		whole, _ := strconv.Atoi(strings.Repeat(curStr, repl))
		if whole > stop {
			break
		}
		if whole >= start {
...
func ParseIDRange(in string) (*IDRange, error) {
	parts, err := SplitParseIntsD(in, "-")
	if err != nil {
		return nil, fmt.Errorf("invalid id range: %q: %w", in, err)
	}
	if len(parts) != 2 {
		return nil, fmt.Errorf("unknown id range: %q", in)
	}
	return &IDRange{Min: parts[0], Max: parts[1]}, nil
}
...
func (i Input) String() string {
	return StringNumberJoin(i.Ranges, 1, "\n")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		...
		for j, part := range parts {
			...
			r, err := ParseIDRange(part)
			if err != nil {
				return nil, fmt.Errorf("[%d,%d]: %w", i, j, err)
			}
			rv.Ranges = append(rv.Ranges, r)
		}
	}
	return &rv, nil
}

...
func Solve(params *Params) (string, error) {
	...
	for i, machine := range input.Machines {
		ans := CountButtonsToJoltage(machine)
		answer += ans
		Verbosef("[%d]: %d = %s", i, ans, machine)
	}
...
func CountButtonsToJoltageV4(machine *Machine) int {
	defer FuncEnding(FuncStarting(machine))
	lastSet := make(map[string][]int)
	zeros := machine.NewJoltage()
	lastSet[jkey(zeros)] = zeros
	if winner, _ := machine.IsJoltage(zeros); winner {
		return 0
	}

	var ok bool
	for i := 1; i < 100_000; i++ {
		Debugf("[%d]: %d states to apply buttons to", i, len(lastSet))
		nextSet := make(map[string][]int)
		for _, joltage := range lastSet {
			toCheck := machine.ApplyAllButtonsToJoltage(joltage)
			ok, toCheck = machine.HasJoltage(toCheck)
			if ok {
				return i
			}
			for _, nextJoltage := range toCheck {
				nextSet[jkey(nextJoltage)] = nextJoltage
			}
		}
		lastSet = nextSet
	}
...
// CountButtonsToJoltageV3 only adds to the next set if its new.
// This one doesn't take as long as V2, but still takes too long.
func CountButtonsToJoltageV3(machine *Machine) int {
	defer FuncEnding(FuncStarting(machine))
	lastSet := make([][]int, 1)
	lastSet[0] = machine.NewJoltage()
	if winner, _ := machine.IsJoltage(lastSet[0]); winner {
		return 0
	}
	var ok bool
	for i := 1; i < 100_000; i++ {
		...
		for _, joltage := range lastSet {
			...
			nextSet = AppendIfNew(nextSet, toCheck...)
		}
		lastSet = nextSet
	}
	return -999999999999999999
...
// CountButtonsToJoltageV2 reduces the next set after full expansion.
// This one doesn't run out of memory, but takes too long.
func CountButtonsToJoltageV2(machine *Machine) int {
	defer FuncEnding(FuncStarting(machine))
	lastSet := make([][]int, 1)
	lastSet[0] = machine.NewJoltage()
	if winner, _ := machine.IsJoltage(lastSet[0]); winner {
	...
	for i := 1; i < 100_000; i++ {
		...
		for _, joltage := range lastSet {
			...
			nextSet = append(nextSet, toCheck...)
		}
		lastSet = RemoveDuplicateJoltages(nextSet)
	}
	return -999999999999999999
}

// CountButtonsToJoltageV1 doesn't have any set reductions and runs out of memory.
func CountButtonsToJoltageV1(machine *Machine) int {
	defer FuncEnding(FuncStarting(machine))
	lastSet := make([][]int, 1)
	lastSet[0] = machine.NewJoltage()
	if winner, _ := machine.IsJoltage(lastSet[0]); winner {
...
func AppendIfNew(rv [][]int, joltages ...[]int) [][]int {
	for _, joltage := range joltages {
		if !ContainsJoltage(rv, joltage) {
			rv = append(rv, joltage)
		}
	}
	return rv
}

func RemoveDuplicateJoltages(joltages [][]int) [][]int {
	rv := make([][]int, 0, len(joltages))
	for _, joltage := range joltages {
		if !ContainsJoltage(rv, joltage) {
			rv = append(rv, joltage)
		}
...
func ContainsJoltage(joltages [][]int, toCheck []int) bool {
	for _, joltage := range joltages {
		if IsEqualJoltages(toCheck, joltage) {
			return true
		}
	}
	return false
...
func IsEqualJoltages(a, b []int) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
...
func CountButtonsToTarget(machine *Machine) int {
	lastSet := make([][]byte, 1)
	lastSet[0] = machine.NewLights()
	if machine.IsTarget(lastSet[0]) {
		return 0
	}
	for i := 1; i < 100_000; i++ {
		var nextSet [][]byte
		for _, lights := range lastSet {
			toCheck := machine.ApplyAllButtonsToLights(lights)
			if machine.HasTarget(toCheck) {
				return i
			}
			nextSet = append(nextSet, toCheck...)
		}
...
func (b Button) String() string {
	...
	return "(" + strings.Join(MapSlice(b, strconv.Itoa), ",") + ")"
}

func (b Button) ApplyLights(lights []byte) []byte {
	rv := make([]byte, len(lights))
	copy(rv, lights)
	for _, i := range b {
		if lights[i] == On {
			rv[i] = Off
...
func (b Button) ApplyJoltage(joltage []int) []int {
	rv := make([]int, len(joltage))
	copy(rv, joltage)
	for _, i := range b {
		rv[i]++
	}
...
func (m *Machine) String() string {
	...
	return fmt.Sprintf("[%s] %s {%s}", m.Target, StringJoin(m.Buttons, " "), strings.Join(MapSlice(m.Joltage, strconv.Itoa), ","))
}
...
func ParseMachine(line string) (*Machine, error) {
	...
	rv := &Machine{}
	rv.Target = []byte(strings.Trim(parts[0], "[]"))

	var err error
	buttons := parts[1 : len(parts)-1]
	rv.Buttons = make([]Button, len(buttons))
	for i, button := range buttons {
		rv.Buttons[i], err = SplitParseIntsD(strings.Trim(button, "()"), ",")
		if err != nil {
			return nil, fmt.Errorf("invalid button %d %q: %w", i+1, button, err)
		}
	}

	rv.Joltage, err = SplitParseIntsD(strings.Trim(parts[len(parts)-1], "{}"), ",")
	if err != nil {
		return nil, fmt.Errorf("invalid joltage %q: %w", parts[len(parts)-1], err)
	}

	return rv, nil
}

func (m *Machine) NewLights() []byte {
	rv := make([]byte, len(m.Target))
	for i := range rv {
		rv[i] = Off
	}
	return rv
...
func (m *Machine) ApplyAllButtonsToLights(lights []byte) [][]byte {
	rv := make([][]byte, len(m.Buttons))
	for i, button := range m.Buttons {
		rv[i] = button.ApplyLights(lights)
	}
	return rv
}

func (m *Machine) IsTarget(lights []byte) bool {
	return string(m.Target) == string(lights)
}

func (m *Machine) HasTarget(lightSets [][]byte) bool {
	for _, lights := range lightSets {
		if m.IsTarget(lights) {
			return true
		}
	}
	return false
...
func (m *Machine) NewJoltage() []int {
	return make([]int, len(m.Joltage))
}

func (m *Machine) ApplyAllButtonsToJoltage(joltage []int) [][]int {
	rv := make([][]int, len(m.Buttons))
	for i, button := range m.Buttons {
		rv[i] = button.ApplyJoltage(joltage)
	}
	return rv
}

// IsJoltage returns two bools. The first indicates whether the provided joltage is correct.
// The second indicates whether it's worth pressing more buttons on that joltage.
func (m *Machine) IsJoltage(joltage []int) (bool, bool) {
	if len(m.Joltage) != len(joltage) {
		return false, false
	}
	allEqual := true
	for i := range m.Joltage {
		if m.Joltage[i] < joltage[i] {
			return false, false
		}
		if m.Joltage[i] > joltage[i] {
			allEqual = false
...
func (m *Machine) HasJoltage(joltages [][]int) (bool, [][]int) {
	rv := make([][]int, 0, len(joltages))
	for _, joltage := range joltages {
		winner, keep := m.IsJoltage(joltage)
		if winner {
...
func ParseInput(lines []string) (*Input, error) {
	...
	rv := Input{Machines: make([]*Machine, len(lines))}
	var err error
	for i, line := range lines {
		rv.Machines[i], err = ParseMachine(line)
		if err != nil {

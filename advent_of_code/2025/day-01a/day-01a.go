...
func (d *Dial) String() string {
	...
	return fmt.Sprintf("at %2d (of %d)", d.Cur, d.Max)
}

func (d *Dial) IsZero() bool {
	return d != nil && d.Cur == 0
}

func (d *Dial) TakeTurn(turn *Turn) {
	switch turn.Dir {
	case Left:
		d.TurnLeft(turn.Len)
	case Right:
		d.TurnRight(turn.Len)
	default:
		panic(fmt.Errorf("Turn has unknown direction: %s", turn))
	}
}

func (d *Dial) TurnLeft(l int) {
	d.Cur = (1000 + d.Cur - l) % (d.Max + 1)
}

func (d *Dial) TurnRight(l int) {
	d.Cur += l
	if d.Cur > d.Max {
		d.Cur %= (d.Max + 1)
	}
...
func (t *Turn) String() string {
	...
	return fmt.Sprintf("%s %3d", t.Dir, t.Len)
}

func ParseTurn(in string) (*Turn, error) {
	if len(in) == 0 {
		return nil, nil
	}

	rv := &Turn{}
	rv.Dir = Direction(in[:1])
	if rv.Dir != Left && rv.Dir != Right {
		return nil, fmt.Errorf("invalid direction %q (from %q)", in[:1], in)
	}

	var err error
	rv.Len, err = strconv.Atoi(in[1:])
	if err != nil {
		return nil, fmt.Errorf("invalid number %q (from %q): %w", in[1:], in, err)
	}
...
func (i Input) String() string {
	return StringNumberJoin(i.Turns, 1, "\n")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		t, err := ParseTurn(line)
		if err != nil {
			return nil, fmt.Errorf("[%d]: %w", i, err)
		}
		rv.Turns = append(rv.Turns, t)
	}
	return &rv, nil
}

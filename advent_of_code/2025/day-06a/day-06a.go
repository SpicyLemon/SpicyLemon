...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := 0
	for x := 0; x < len(input.Nums[0]); x++ {
		ans := 0
		switch input.Ops[x] {
		case "*":
			ans = 1
			for y := 0; y < len(input.Nums); y++ {
				ans *= input.Nums[y][x]
			}
		case "+":
			for y := 0; y < len(input.Nums); y++ {
		...
		default:
			return "", fmt.Errorf("unknown op [%d] = %q", x, input.Ops[x])
		}
		Debugf("[%d] = %d", x, ans)
		answer += ans
	}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		nums, err := SplitParseInts(line)
		if err == nil {
			rv.Nums = append(rv.Nums, nums)
		} else {
			rv.Ops = strings.Fields(line)
		}
	}
	return &rv, nil
}

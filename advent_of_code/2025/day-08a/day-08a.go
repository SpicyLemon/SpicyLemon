...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	count := params.Count
	if params.InputFile == DEFAULT_INPUT_FILE && count == DEFAULT_COUNT {
		count = 10
	}
	Debugf("Count: %d", count)

	pairs := MakePairs(input.Points)
	if debug {
		Stderrf("Pairs (%d):\n%s", len(pairs[:count]), StringNumberJoin(pairs[:count], 1, "\n"))
	}
	circuits := MakeCircuits(pairs[:count])
	if debug {
		Stderrf("Circuits (%d):\n%s", len(circuits), StringNumberJoin(circuits, 1, "\n"))
	}
	if len(circuits) < 3 {
		return "", fmt.Errorf("only made %d circuit(s)", len(circuits))
	}
	answer := circuits[0].Size() * circuits[1].Size() * circuits[2].Size()
	return fmt.Sprintf("%d", answer), nil
}

func MakeCircuits(pairs []*Pair) []Circuit {
	var rv []Circuit
	for _, pair := range pairs {
		...
		if c1 == nil && c2 == nil {
			rv = append(rv, NewCircuit(pair.P1, pair.P2))
			continue
		}
		if c1 != nil && c2 != nil && i1 == i2 {
			// Both already in the same circuit, nothing more to do.
			continue
		}
		if c1 != nil && c2 == nil {
			// p1 is already in a circuit, but p2 isn't; so just add p2 to this one.
			rv[i1] = append(rv[i1], pair.P2)
			continue
		}
		if c1 == nil && c2 != nil {
			// p2 is already in a circuit, but p1 isn't; sojust add p1 to this one.
			rv[i2] = append(rv[i2], pair.P1)
			continue
		}

		// They're both in different circuits, so we need to join them together.
		nc := c1.Combine(c2)
		// Now, we take c1 and c2 out of rv, and add this one
		newRv := make([]Circuit, 0, len(rv)-1)
		for i, c := range rv {
			if i != i1 && i != i2 {
				newRv = append(newRv, c)
			}
		}
		newRv = append(newRv, nc)
		rv = newRv
	}

	slices.SortFunc(rv, func(a, b Circuit) int {
...
func NewCircuit(points ...*Point) Circuit {
	return points
}

func (c Circuit) String() string {
	return strings.Join(MapSlice(c, (*Point).String), "-")
}

func (c Circuit) Combine(c2 Circuit) Circuit {
	rv := make(Circuit, 0, len(c)+len(c2))
	rv = append(rv, c...)
	rv = append(rv, c2...)
	return rv
}

func (c Circuit) Has(p *Point) bool {
	for _, cp := range c {
...
func MakePairs(points []*Point) []*Pair {
	...
	slices.SortFunc(rv, func(a, b *Pair) int {
...
func (p *Pair) String() string {
	...
	return fmt.Sprintf("%s<-%d->%s", p.P1, p.DistSq, p.P2)
}

func DistanceSquared(p1, p2 *Point) int {
	return squared(p1.X-p2.X) + squared(p1.Y-p2.Y) + squared(p1.Z-p2.Z)
}
...
func ParsePoint(line string) (*Point, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not parse point line %q: %w", line, err)
	}
	if len(parts) != 3 {
		return nil, fmt.Errorf("unknown point format %q", line)
	}
	return NewPoint(parts[0], parts[1], parts[2]), nil
}
...
func (i Input) String() string {
	return StringNumberJoin(i.Points, 1, "\n")
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Points: make([]*Point, len(lines))}
	var err error
...
func AsPoints[S ~[]E, E XY](vals S) []*Point {
	...
	for i, val := range vals {
		rv[i] = NewPoint(val.GetX(), val.GetY(), 0)
	}
	return rv
}

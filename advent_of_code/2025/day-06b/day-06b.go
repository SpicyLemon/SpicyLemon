...
func Solve(params *Params) (string, error) {
	...
	for e, nums := range input.Nums {
		...
		default:
			return "", fmt.Errorf("unknown op [%d] = %q", e, input.Ops[e])
		}
		Debugf("[%d]: %q %d = %d", e, input.Ops[e], nums, ans)
		answer += ans
	}
...
func (i Input) String() string {
	return CreateIndexedGridStringNums(i.Nums, []XY(nil), nil) + "\n" + strings.Join(AddLineNumbers(i.Ops, 0), " ")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	rv := Input{}
	e := 0
	rv.Nums = append(rv.Nums, make([]int, 0, 4))
	...
	for i := len(lines[0]) - 1; i >= 0; i-- {
		...
		if err != nil {
			return nil, fmt.Errorf("could not convert %q to number (i = %d): %w", str, i, err)
		}
		rv.Nums[e] = append(rv.Nums[e], num)
	}
	slices.Reverse(rv.Nums)
	rv.Ops = strings.Fields(lines[numLines])
	return &rv, nil
}

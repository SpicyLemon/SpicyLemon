...
func Solve(params *Params) (string, error) {
	...
	// Go through the points and find the first pair containing that point.
	// The last such pair in the list is the pair that will tie everything together.
	lastPairI := -1
	...
	lastPair := pairs[lastPairI]
	Debugf("Last Pair: %s", lastPair)
	answer := lastPair.P1.X * lastPair.P2.X
...
func (p *Pair) Has(point *Point) bool {
	return p != nil && (EqualPoints(point, p.P1) || EqualPoints(point, p.P2))
}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	dMap := WireDevices(input.Devices)
	you := dMap["you"]
	out := dMap["out"]
	paths := FindPaths(Path{you}, out)
	if debug {
...
func FindPaths(cur Path, to *Device) []Path {
	defer FuncEnding(FuncStarting(cur, to))
	var rv []Path
	from := cur[len(cur)-1]
...
func (p Path) String() string {
	...
	return strings.Join(MapSlice(p, (*Device).GetName), " ")
}
...
func WireDevices(devices []*Device) map[string]*Device {
	rv := make(map[string]*Device)
	for _, d := range devices {
		rv[d.Name] = d
	}
	if rv["out"] == nil {
		rv["out"] = &Device{Name: "out"}
	}
	for _, d := range devices {
		d.DOuts = make([]*Device, len(d.Outs))
...
func (d *Device) String() string {
	...
	return fmt.Sprintf("%s: %s", d.Name, strings.Join(d.Outs, " "))
}
...
func (i Input) String() string {
	return StringNumberJoin(i.Devices, 1, "\n")
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Devices: make([]*Device, len(lines))}
	for i, line := range lines {
		rv.Devices[i] = ParseDevice(line)
	}
	return &rv, nil
}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := FindWay(input.Steps, input.Order, input.Nodes)
...
func (i Input) String() string {
	nodes := make([]string, len(i.Order))
	for j, el := range i.Order {
		nodes[j] = i.Nodes[el].String()
	}
	return fmt.Sprintf("%s\n\n%s\n", i.Steps, strings.Join(AddLineNumbers(nodes, 1), "\n"))
}

var lineRx = regexp.MustCompile(`(...) = \((...), (...)\)`)

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Nodes: make(map[string]*Node)}
	for i, line := range lines {
		...
		parts := lineRx.FindStringSubmatch(line)
		if len(parts) != 4 {
			return nil, fmt.Errorf("failed to parse %q", line)
		}
		node := &Node{Name: parts[1], Left: parts[2], Right: parts[3]}
		rv.Order = append(rv.Order, node.Name)
		rv.Nodes[node.Name] = node
	}
	return &rv, nil
}
...
func FindWay(steps string, names []string, nodes map[string]*Node) int {
	var starts []string
	...
	for i, start := range starts {
		s, offset, period := 0, 0, 0
		cur := start
		Debugf("% 3d: Starting at %s", 0, nodes[cur])
		for period == 0 {
			here := fmt.Sprintf("%s%03d", cur, s)
			for j, past := range paths[i] {
				if past == here {
					offset = j
					period = len(paths[i]) - offset
					break
				}
			}
			paths[i] = append(paths[i], here)
			...
			case 'R':
				cur = nodes[cur].Right
			default:
				panic(fmt.Errorf("unknown step %q", steps[s]))
			}
			Debugf("% 3d: stepping %c to %s", len(paths[i]), steps[s], nodes[cur])
			if cur[2] == 'Z' {
				ends[i] = append(ends[i], len(paths[i]))
			}
			s++
			if s >= len(steps) {
				s = 0
	...
	for i, path := range paths {
		lines[i] = fmt.Sprintf("Start %s: Offset: %d Period: %d, Ends: %v, Path (%d)",
			path[0], offsets[i], periods[i], ends[i], len(path),
		)
		if len(path) <= 20 {
			lines[i] += fmt.Sprintf(": %v", path)
		}
	}
	Stdoutf("After processing:\n%s\n", strings.Join(AddLineNumbers(lines, 1), "\n"))

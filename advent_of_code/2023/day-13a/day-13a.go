...
func (i Input) String() string {
	return fmt.Sprintf("Fields (%d):\n%s", len(i.Fields), StringNumberJoin(i.Fields, 1, "\n"))
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		if len(line) == 0 {
			rv.Fields = append(rv.Fields, nextField)
			nextField = Field{}
			continue
		}
		nextField = append(nextField, []byte(line))
	}
	if len(nextField) > 0 {
		rv.Fields = append(rv.Fields, nextField)
	}
...
func (f Field) String() string {
	return "\n" + CreateIndexedGridStringBz(f, nil, nil)
}

func (f Field) FindReflection() int {
	defer FuncEnding(FuncStarting())

	// First check for horizontal.
	for y := 0; y < len(f)-1; y++ {
		if f.IsVReflection(y) {
			return 100 * (y + 1)
		}
	}

	// Then lets do vertical.
	for x := 0; x < len(f[0])-1; x++ {
		if f.IsHReflection(x) {
			return x + 1
		}
	}
...
func (f Field) IsVReflection(y int) bool {
	for dy := 0; y+dy+1 < len(f) && y-dy >= 0; dy++ {
		if !bytes.Equal(f[y-dy], f[y+dy+1]) {
			return false
		}
	}
	return true
...
func (f Field) IsHReflection(x int) bool {
	for dx := 0; x+dx+1 < len(f[0]) && x-dx >= 0; dx++ {
		for y := range f {
			if f[y][x-dx] != f[y][x+dx+1] {
				return false
			}
		}
	}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	galaxies := GetGalaxies(input.Space, params.Count)
	Debugf("Galaxies (%d): %s", len(galaxies), StringNumberJoin(galaxies, 1, ", "))
	answer := SumAllDistances(galaxies)
	return fmt.Sprintf("%d", answer), nil
}

func SumAllDistances(galaxies []XY) int {
	rv := 0
	for a := 0; a < len(galaxies)-1; a++ {
		for b := a + 1; b < len(galaxies); b++ {
			rv += Distance(galaxies[a], galaxies[b])
		}
	}
	return rv
}
...
func GetGalaxies(space [][]byte, expansion int) []XY {
	var galaxies []XY
	haveX := make(map[int]bool)
	haveY := make(map[int]bool)
	...
	expandX := make([]int, 0, len(space[0])-len(haveX))
	...
	expandY := make([]int, 0, len(space)-len(haveY))
	...
	rv := make([]XY, len(galaxies))
	for i, p := range galaxies {
		x, y := p.GetXY()
		x += CountBetween(0, x, expandX) * (expansion - 1)
		y += CountBetween(0, y, expandY) * (expansion - 1)
		rv[i] = Point{X: x, Y: y}
	}
...
// CountBetween counts the number of vals between the given min and max (exclusive).
// It's assumed that the vals are sorted smallest first.
func CountBetween(min, max int, vals []int) int {
	rv := 0
...
func (i Input) String() string {
	return fmt.Sprintf("Space:\n%s", CreateIndexedGridStringBz(i.Space, nil, nil))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Space: make([][]byte, len(lines))}
	for i, line := range lines {
		rv.Space[i] = []byte(line)
	}
	return &rv, nil
}

...
const DEFAULT_COUNT = 1_000_000_000
...
func Solve(params *Params) (string, error) {
	...
	for i := 0; i < params.Count; i++ {
		input.Field.Spin()
		loads = append(loads, input.Field.GetLoad())
		if debug && params.Count != DEFAULT_COUNT {
			Debugf("After %d cycle(s): Load = %d", i+1, loads[len(loads)-1])
		}
		cycle := FindCycle(loads)
		if len(cycle) > 0 {
			Stdoutf("Cycle found after Spin %d: (%d) %v", i+1, len(cycle), cycle)
			c := (1_000_000_000 - len(loads) - 1 + len(cycle)) % (len(cycle))
			answer = cycle[c]
			break
		}
		if params.Verbose && (i+1)%1_000_000 == 0 {
			Stdoutf("Cycle %d of %d (million) done.", (i+1)/1_000_000, params.Count/1_000_000)
		}
	}
	return fmt.Sprintf("%d", answer), nil
}

func FindCycle(loads []int) []int {
	if len(loads) < 20 {
		return nil
	}
	maxLen := len(loads) / 3
	for l := 5; l <= maxLen; l++ {
		rv := loads[len(loads)-l:]
		if IsCycle(loads, rv) {
			return rv
		}
	}
...
func IsCycle(loads, cycle []int) bool {
	yStart := len(loads) - (len(cycle) * 3)
	if yStart < 0 {
		return false
	}
	for y := yStart; y < len(loads); y++ {
		if loads[y] != cycle[(y-yStart)%len(cycle)] {
			return false
		}
	}
	return true
...
func (i Input) String() string {
	return fmt.Sprintf("Field %s", i.Field)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Field: make(Field, len(lines))}
	for i, line := range lines {
		rv.Field[i] = []byte(line)
	}
	return &rv, nil
}
...
func (f Field) String() string {
	...
	for y, row := range f {
		for x, cell := range row {
			switch cell {
			case Stop:
				stops = append(stops, Point{X: x, Y: y})
			case Rock:
				rocks = append(rocks, Point{X: x, Y: y})
			}
		}
	}
	return fmt.Sprintf("(%d, %d):\n", len(f[0]), len(f)) + CreateIndexedGridStringBz(f, rocks, stops)
}

func (f Field) GetLoad() int {
	rv := 0
	lf := len(f)
	for y, row := range f {
...
func (f Field) Spin() {
	f.TiltNorth()
	// Debugf("After Tilt North:\n%s", f)
	f.TiltWest()
	// Debugf("After Tilt West:\n%s", f)
	f.TiltSouth()
	// Debugf("After Tilt South:\n%s", f)
	f.TiltEast()
}

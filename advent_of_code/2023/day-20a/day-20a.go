...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	moduleMap := WireModules(input.Modules)
	highs, lows := CountPulses(moduleMap, params.Count)
	answer := highs * lows
	Debugf("Totals: %d = %d (highs) * %d (lows)", answer, highs, lows)
	return fmt.Sprintf("%d", answer), nil
}

func CountPulses(moduleMap map[string]*Module, count int) (int, int) {
	defer FuncEnding(FuncStarting())
	var highs, lows []int
	for i := 0; i < count; i++ {
		Debugf("Button Press: %d", i+1)
		pulses := HitButton(moduleMap)
		Debugf("%s", pulses)
		newHighs, newLows := CountHighsLows(pulses)
		highs = append(highs, newHighs)
		lows = append(lows, newLows)
		if IsZeroState(moduleMap) {
			Debugf("Back to zero state after %d button presses", i+1)
			break
		}
	}

	if len(highs) >= count {
		Debugf("No cycle detected")
		return SumInts(highs[:count]), SumInts(lows[:count])
	}

	rvHighs := SumInts(highs) * (count / len(highs))
	rvLows := SumInts(lows) * (count / len(lows))
	leftover := count % len(highs)
	if leftover != 0 {
		rvHighs += SumInts(highs[:leftover])
		rvLows += SumInts(lows[:leftover])
	}

	return rvHighs, rvLows
}

func HitButton(moduleMap map[string]*Module) Pulses {
	defer FuncEnding(FuncStarting())
	ResetCounters(moduleMap)
	var sent, queue Pulses
	queue = append(queue, NewPulse("button", Low, "broadcaster"))
	for len(queue) > 0 {
		pulse := queue[0]
		queue = queue[1:]
...
func ResetCounters(moduleMap map[string]*Module) {
...
func (p Pulse) String() string {
	...
	return fmt.Sprintf("%s -%s-> %s", p.Source, arrow, p.Dest)
}
...
func (m Module) String() string {
	...
	parts[0] = fmt.Sprintf("(%3d,%3d):", m.HighsSent, m.LowsSent)
	if m.Type == FlipFlop || m.Type == Conjunction {
		if m.On {
	...
	parts = append(parts, fmt.Sprintf("%s%s -> %s", t, m.Name, m.SendTo))

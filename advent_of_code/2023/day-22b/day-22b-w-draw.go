...
func Solve(params *Params) (string, error) {
	...
	if debug {
		Stderrf("After Drops:\n%s",
			strings.Join(AddLineNumbers(MapSlice(bricks, (*Brick).StringWithBlocks), 1), "\n"))
	}
	answer := CountDependent(bricks, space, params)
	return fmt.Sprintf("%d", answer), nil
}

func CountDependent(bricks Bricks, brickSpace SpaceMap[*Brick], params *Params) int {
	relations := BuildBrickRelations(bricks, brickSpace)
	space := BuildSpaceRelations(relations)
	DrawSpace(space)
	AssertBricksInSpace(relations, space)
	rv := 0
	for i, brick := range relations {
		Debugf("[%d/%d]: Deleting Brick: %s", i+1, len(relations), brick)
		falling := DeleteBrick(brick, space, params)
		Debugf("[%d/%d]: Deleting Brick: %s drops %d: %s",
			i+1, len(relations), brick.Brick, len(falling), MapSlice(falling, (*BrickRelation).GetName))
		if !debug && params.Verbose && (i+1)%100 == 0 {
			Stderrf("[%d/%d]: Total so far: %d", i+1, len(relations), rv)
		}
		rv += len(falling)
	}
	return rv
...
func AssertBricksInSpace(bricks []*BrickRelation, space SpaceMap[*BrickRelation]) {
	...
	for _, brick := range bricks {
		for _, block := range brick.Brick.Blocks {
			if !brick.HasPoint(block) {
				fail("brick %s has block %s in it's list but shouldn't", brick, block)
			}

			fromSpace := space.Get(block)
			if fromSpace == nil || !brick.Equal(fromSpace) {
				fail("block %s from %s is marked as %s in space", block, brick, fromSpace)
			}
			if fromSpace != brick {
				fail("block %s from %s is equal to %s but is a different address: %p vs %p",
					block, brick, fromSpace, brick, fromSpace)
			}
		}
		for _, above := range brick.Above {
			if brick.Equal(above) {
				fail("brick %s has itself (%s) above it.", brick, above)
			}
			isAbove := false
			for _, block := range above.Brick.Blocks {
				x, y, z := block.GetXYZ()
				if brick.HasPointRaw(x, y, z-1) {
					isAbove = true
					break
				}
			}
			if !isAbove {
				fail("brick %s has %s above it, but it is not actually above it.", brick, above)
			}
		}
		for _, below := range brick.Below {
			if brick.Equal(below) {
				fail("brick %s has itself (%s) below it.", brick, below)
			}
			isBelow := false
			for _, block := range below.Brick.Blocks {
				x, y, z := block.GetXYZ()
				if brick.HasPointRaw(x, y, z+1) {
					isBelow = true
					break
			...
			if !isBelow {
				fail("brick %s has %s below it, but it is not actually below it.", brick, below)
			}
		}
		isOnGround := brick.Brick.P1.Z == 1 || brick.Brick.P2.Z == 1
		if isOnGround && len(brick.Below) != 0 {
			fail("brick %s is on the ground but has bricks below it:\n%s",
				brick, StringNumberJoin(brick.Below, 1, "\n"))
		}
		if !isOnGround && len(brick.Below) == 0 {
			fail("brick %s is not on the ground, but has nothing below it", brick)
		}
	}
	...
	for i := 0; i < len(bricks)-1; i++ {
		for j := i + 1; j < len(bricks); j++ {
			if bricks[i] == bricks[j] {
				fail("bricks[%d] is the same as bricks[%d]: %p %s", i, j, bricks[i], bricks[i])
			}
			if bricks[i].Equal(bricks[j]) {
				fail("brick[%d] %p is equal to bricks[%d] %p: %s", i, bricks[i], j, bricks[j], bricks[i])
			}
		}
	}

	var spaceBricks []*BrickRelation
	for z, zm := range space {
		for y, ym := range zm {
			for x, fromSpace := range ym {
				if !fromSpace.HasPointRaw(x, y, z) {
					fail("point (%d,%d,%d) in space has %s, which does not have that point in space.")
				}
				spaceBricks = AppendBRIfUnique(spaceBricks, fromSpace)
			}
		}
	}

	slices.SortFunc(spaceBricks, CompareBrickRelations)

	if len(spaceBricks) != len(bricks) {
		// We already made sure that all the bricks are in the correct spot in space.
		// So there must be more spaceBricks here.
		l, _ := OrderInts(len(spaceBricks), len(bricks))
		for i := 0; i < l; i++ {
			if !spaceBricks[i].Equal(bricks[i]) {
				fail("spaceBricks[%d] != bricks[%d]: %s vs %s", i, i, spaceBricks[i], bricks[i])
			}
			if spaceBricks[i] != bricks[i] {
				fail("spaceBricks[%d] is not the same object as bricks[%d], %p vs %p", i, i, spaceBricks[i], bricks[i])
			}
		}
		fail("the following bricks are found in space, but not the list of bricks:\n",
			StringNumberJoin(spaceBricks[l:], l, "\n"))
	}

	for i := range spaceBricks {
		if !spaceBricks[i].Equal(bricks[i]) {
			fail("spaceBricks[%d] != bricks[%d]: %s vs %s", i, i, spaceBricks[i], bricks[i])
		}
		if spaceBricks[i] != bricks[i] {
	...
	Stderrf("Bricks and Space asserted.")
}

func CompareBrickRelations(bri, brj *BrickRelation) int {
	return CompareBricks(bri.Brick, brj.Brick)
}

func DeleteBrick(gone *BrickRelation, space SpaceMap[*BrickRelation], params *Params) []*BrickRelation {
	defer FuncEnding(FuncStarting())
	Debugf("Now Gone: %s", gone)
	_ = params
	if len(gone.Above) == 0 {
		Debugf("Nothing above it.")
		return nil
	}
	_, minZ := OrderInts(gone.Brick.P1.Z, gone.Brick.P2.Z)
	zs := IntKeys(space)
	var falling, notFalling []*BrickRelation
	notFalling = append(notFalling, gone)
	for _, z := range zs {
		if z <= minZ {
			continue
		}
		Debugf("z = %d for %s", z, gone.Brick)
		for _, ym := range space[z] {
			for _, brick := range ym {
				if brick == gone || ContainsBR(falling, brick) || ContainsBR(notFalling, brick) {
					continue
				}
				if !brick.IsStable(gone, falling) {
					Debugf("  Now Falling: %s", brick)
					falling = AppendBRIfUnique(falling, brick)
				} else {
					notFalling = AppendBRIfUnique(notFalling, brick)
				}
			}
		}
	}
...
func BuildSpaceRelations(bricks []*BrickRelation) SpaceMap[*BrickRelation] {
	defer FuncEnding(FuncStarting())
	space := NewSpaceMap[*BrickRelation]()
	for _, brick := range bricks {
		for _, block := range brick.Brick.Blocks {
			already := space.Get(block)
			if already != nil {
				panic(fmt.Sprintf("cannot put %s in space because %s is already occupying %s",
					brick.Brick, already.Brick, block))
			}
...
func (b BrickRelation) Equal(b2 *BrickRelation) bool {
	if !b.Brick.Equal(b2.Brick) {
		return false
	}
	if len(b.Above) != len(b2.Above) || len(b.Below) != len(b2.Below) {
		return false
	}
	for i := range b.Above {
		if !b.Above[i].Brick.Equal(b2.Above[i].Brick) {
			return false
		}
	}
	for i := range b.Below {
		if !b.Below[i].Brick.Equal(b2.Below[i].Brick) {
			return false
		}
	}
	return true
...
func (b BrickRelation) HasPoint(p XYZ) bool {
	return b.Brick.HasPoint(p)
}

func (b BrickRelation) HasPointRaw(x, y, z int) bool {
	return b.Brick.HasPointRaw(x, y, z)
}
...
func (b BrickRelation) IsStable(gone *BrickRelation, falling []*BrickRelation) bool {
	if b.Brick.P1.Z == 1 || b.Brick.P2.Z == 1 {
		// Debugf("On the ground: %s", b.Brick)
		return true
	}
	for _, below := range b.Below {
		if gone != below && !ContainsBR(falling, below) {
			// Debugf("Brick %s has %s below it.", b.Brick, below.Brick)
			return true
		}
	}
	return false
...
func AppendBRIfUnique(brs []*BrickRelation, bricks ...*BrickRelation) []*BrickRelation {
	for _, brick := range bricks {
		if !ContainsBR(brs, brick) {
			brs = append(brs, brick)
		}
	}
	return brs
}

func ContainsBR(brs []*BrickRelation, brick *BrickRelation) bool {
	for _, known := range brs {
		if known == brick {
			return true
		}
	}
...
func DrawSpace(space SpaceMap[*BrickRelation]) {
	defer FuncEnding(FuncStarting())
	xs, ys, zs := space.GetDims()

	var zxg, zyg, yxg [][][]byte

	initGrid := func(a, b, c int) [][][]byte {
		rv := make([][][]byte, a)
		for i := range rv {
			rv[i] = make([][]byte, b)
			for j := range rv[i] {
				rv[i][j] = []byte(strings.Repeat(" ", c))
			}
		}
		return rv
	}

	markSpot := func(x, y, z int, val byte) {
		zxg[z][x][y] = val
		zyg[z][y][x] = val
		yxg[y][x][z] = val
	}
	...
	drawGrid := func(props gridProps) []string {
		flat := make([][]string, len(props.grid))
		var color, hl []XY
		for i, row := range props.grid {
			if i == 0 && props.zeroDash {
				flat[i] = strings.Split(strings.Repeat("-", len(row)), "")
				continue
			}
			flat[i] = make([]string, len(row))
			for j, vals := range row {
				if !props.zeroDash {
					vals = vals[1:]
				}
				if !props.getFirst {
				...
				for _, val := range vals {
					if val != ' ' && !bytes.Contains(bz, []byte{val}) {
						bz = append(bz, val)
					}
				}
				...
				flat[i][j] = string(cell)

				if len(bz) > 1 {
					color = append(color, NewPoint(j, i, 0))
				}
				if vals[0] == cell {
					hl = append(hl, NewPoint(j, i, 0))
				}
			}
		}

		lines := CreateIndexedGridString(flat, color, hl, props.vRev, props.hRev)
		width := len(lines[0])
		toPad := width - len(props.title)
		var titleLine string
		switch {
		case toPad < 0:
			titleLine = props.title[:width]
		case toPad > 0:
			lPad := toPad / 2
			rPad := toPad - lPad
			titleLine = strings.Repeat(" ", lPad) + props.title + strings.Repeat(" ", rPad)
		default:
			titleLine = props.title
		}

		return append([]string{titleLine}, lines...)
	}
	...
	zxg = initGrid(zl, xl, yl)
	zyg = initGrid(zl, yl, xl)
	yxg = initGrid(yl, xl, zl)

	zs.Iter(func(z int) {
		ys.Iter(func(y int) {
			xs.Iter(func(x int) {
				brick := space.GetRaw(x, y, z)
				if brick != nil {
					markSpot(x, y, z, brick.Brick.Name)
				}
			})
		})
	})

	zxf := drawGrid(gridProps{
		title:    "Front",
		grid:     zxg,
		getFirst: true,
		vRev:     true,
	...
	middle := make([]string, len(zxf))
	for i := range zxf {
		middle[i] = strings.Join([]string{zyl[i], zxf[i], zyr[i], zxb[i]}, "  ")
	}

	spacer := strings.Repeat(" ", len(zyl[0])+2)
	for i := range yxb {
		yxb[i] = spacer + yxb[i]
		yxt[i] = spacer + yxt[i]
	}
	...
	Stdoutf("Drawing:\n"+
		"\033[93mYellow\033[0m = Block is hiding at least one block from a different brick.\n"+
		"  \033[7mBold\033[0m = Block is in the zero value but not hiding any other bricks.\n"+
		"  \033[93;7mBoth\033[0m = Block is in the zero value and hiding another brick.\n"+
		"Normal = Block is at least one space back and does not block any other bricks.\n"+
		"%s",
		drawing)
}
...
func Drop(startingBricks Bricks) (Bricks, SpaceMap[*Brick]) {
	...
	dropBrick := func(brick *Brick) bool {
	...
	if dropLoops != 1 {
		panic(fmt.Errorf("made %d drop loops", dropLoops))
	}

	slices.SortFunc(bricks, CompareBricks)
	return bricks, space
}

func CompareBricks(bi, bj *Brick) int {
	bizl, bizh := OrderInts(bi.P1.Z, bi.P2.Z)
	bjzl, bjzh := OrderInts(bj.P1.Z, bj.P2.Z)
	if c := CompInts(bizh, bjzh); c != 0 {
		return c
	}
	if c := CompInts(bizl, bjzl); c != 0 {
		return c
	}
	biyl, biyh := OrderInts(bi.P1.Y, bi.P2.Y)
	bjyl, bjyh := OrderInts(bj.P1.Y, bj.P2.Y)
	if c := CompInts(biyh, bjyh); c != 0 {
		return c
	}
	if c := CompInts(biyl, bjyl); c != 0 {
		return c
	}
	bixl, bixh := OrderInts(bi.P1.X, bi.P2.X)
	bjxl, bjxh := OrderInts(bj.P1.X, bj.P2.X)
	if c := CompInts(bixh, bjxh); c != 0 {
		return c
	}
	return CompInts(bixl, bjxl)
}
...
func (b Brick) String() string {
	return fmt.Sprintf("%c=%s~%s", b.Name, b.P1, b.P2)
}

func (b Brick) StringWithBlocks() string {
...
func (b Brick) HasPoint(p XYZ) bool {
	x, y, z := p.GetXYZ()
	return b.HasPointRaw(x, y, z)
}

func (b Brick) HasPointRaw(x, y, z int) bool {
	xMin, xMax := OrderInts(b.P1.X, b.P2.X)
	yMin, yMax := OrderInts(b.P1.Y, b.P2.Y)
	zMin, zMax := OrderInts(b.P1.Z, b.P2.Z)
...
func (b *Brick) Equal(b2 *Brick) bool {
	return b.P1.Equal(b2.P1) && b.P2.Equal(b2.P2)
}
...
func CreateIndexedGridString[S ~[]E, E XY](
	vals [][]string,
	colorPoints S,
	highlightPoints S,
	vRev bool,
	...
	for l := 1; l <= dCount; l++ {
		...
		if hRev {
			bz := []byte(topIndexLines[dCount-l])
			slices.Reverse(bz)
			topIndexLines[dCount-l] = string(bz)
		}
	}
	...
	headerLines := make([]string, len(topIndexLines), len(topIndexLines)+len(vals))
	for i, l := range topIndexLines {
		headerLines[i] = fmt.Sprintf("%s%s", blankLead, l)
	}

	bodyLines := make([]string, len(vals))
	// Add all the line numbers, and cells (with the desired coloring/marking).
	for y, r := range vals {
		lead := fmt.Sprintf(leadFmt, y)
		cells := make([]string, width)
		for x := 0; x < width; x++ {
			var v, colStart, colEnd string
			if x < len(r) {
				v = r[x]
			}
			switch textFmt[y][x] {
			case 0: // default look.
				// Nothing to do.
			case 1: // color only.
				colStart = "\033[93m" // Bright Yellow text.
			case 2: // highlight only
				colStart = "\033[7m" // Foreground<->Background Reversed.
			case 3: // color and highlight
				colStart = "\033[93;7m" // Bright Yellow background after fg<->bg reversed.
			default: // Unknown, make it ugly.
				colStart = "\033[93;41m" // Bright yellow text on a red background.
			}
			if len(colStart) > 0 {
				colEnd = "\033[0m"
			}
			cells[x] = colStart + fmt.Sprintf(cellFmt, v) + colEnd
		}
		if hRev {
			slices.Reverse(cells)
		}
		bodyLines[y] = lead + strings.Join(cells, "")
	}

	if vRev {
		slices.Reverse(bodyLines)
	}

	return append(headerLines, bodyLines...)
}
...
func (c Params) Verbosef(format string, a ...interface{}) {
	if c.Verbose {
		StderrAsf(GetFuncName(1), format, a...)
	}
}

func (c Params) HasCustom(str string) bool {
	for _, cust := range c.Custom {
		if cust == str {
			return true
		}
	}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := 0
	for _, h := range input.Histories {
		answer += GetVal(h)
	}
	return fmt.Sprintf("%d", answer), nil
}

func GetVal(hist History) int {
...
func Predict(hist History) Prediction {
	rv := make(Prediction, 1, 10)
	rv[0] = hist

	prev := hist
	Debugf("Start: %s", prev)
	for !prev.IsZero() {
		next := make(History, prev.Length()-1)
...
func (p Prediction) String() string {
	return StringNumberJoin(p, 0, "\n")
}

func (p Prediction) Length() int {
	return len(p)
}

func (p Prediction) GetTopRight() int {
	return p[0].Last()
}

func (p Prediction) GetTopLeft() int {
	return p[0].First()
}
...
func (i Input) String() string {
	return fmt.Sprintf("Histories (%d):\n%s", len(i.Histories), StringNumberJoin(i.Histories, 1, "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Histories: make([]History, len(lines))}
	var err error
	for i, line := range lines {
		rv.Histories[i], err = SplitParseInts(line, " ")
		if err != nil {
...
func AddNew(h History, plus int) History {
	return append(h, h.Last()+plus)
}

func PreNew(h History, plus int) History {
	return append(History{h.First() - plus}, h...)
}

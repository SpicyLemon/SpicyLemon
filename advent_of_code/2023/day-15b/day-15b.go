...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	boxes := ProcessSteps(input.Steps)
	Debugf("Boxes:\n%s", boxes)
	answer := GetFocusingPower(boxes)
	return fmt.Sprintf("%d", answer), nil
}

func ProcessSteps(steps []Step) Boxes {
	boxes := make(Boxes)
	for _, step := range steps {
		box := step.GetBox()
		switch step.Op {
		case Add:
			boxes[box] = AddLens(boxes[box], step.Lens)
		case Rem:
			boxes[box] = RemLens(boxes[box], step.Lens)
		}
	}
	return boxes.Clean()
}

func GetFocusingPower(boxes Boxes) int {
	rv := 0
	for boxNum, box := range boxes {
		for i, lens := range box {
			nv := (boxNum + 1) * (i + 1) * lens.Focus
			Debugf("%s = %d = %d (box %d) * %d (slot %d) * %d (lens focus)",
				lens, nv, boxNum+1, boxNum, i+1, i, lens.Focus)
			rv += nv
		}
	}
	return rv
...
func (b Box) String() string {
	...
	for _, lens := range b {
		if !lens.Removed {
			lenses = append(lenses, lens.String())
		}
	}
...
func (b Boxes) String() string {
	...
	for i := 0; i <= 255; i++ {
		...
		if found {
			str := box.String()
			if len(str) > 0 {
				lines = append(lines, fmt.Sprintf("Box % 3d: %s", i, str))
			}
		}
	}
...
func FindLens(lenses Box, label string) *Lens {
	for _, lens := range lenses {
		if !lens.Removed && lens.Label == label {
			return lens
		}
	}
	return nil
...
func RemLens(lenses Box, toRemove Lens) []*Lens {
	lens := FindLens(lenses, toRemove.Label)
	if lens != nil {
		lens.Removed = true
	}
	return lenses
...
func AddLens(lenses []*Lens, toAdd Lens) []*Lens {
	lens := FindLens(lenses, toAdd.Label)
	if lens != nil {
		lens.Focus = toAdd.Focus
		return lenses
	}
	return append(lenses, toAdd.Copy())
}
...
func (i Input) String() string {
	return fmt.Sprintf("Steps (%d):\n%s", len(i.Steps), StringNumberJoin(i.Steps, 1, "\n"))
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		...
		for _, step := range steps {
			rv.Steps = append(rv.Steps, NewStep(step))
		}
	}
	return &rv, nil
}
...
func (l Lens) String() string {
	if l.Removed {
		return fmt.Sprintf("X[%s %d]X", l.Label, l.Focus)
	}
	return fmt.Sprintf("[%s %d]", l.Label, l.Focus)
}

func (l Lens) Copy() *Lens {
	return &Lens{Label: l.Label, Focus: l.Focus, Removed: l.Removed}
}
...
func NewStep(inst string) Step {
	rv := Step{Inst: inst}
	if strings.HasSuffix(inst, "-") {
		rv.Op = Rem
		rv.Lens = Lens{Label: strings.TrimSuffix(inst, string(Rem))}
	} else {
		rv.Op = Add
		parts := strings.Split(inst, string(Add))
		if len(parts) != 2 {
			panic(fmt.Errorf("unknown instruction: %q", inst))
		}
		focus, err := strconv.Atoi(parts[1])
		if err != nil {
			panic(fmt.Errorf("unknown focal length from instruction %q, %q: %w", inst, parts[1], err))
		}
		rv.Lens = Lens{Label: parts[0], Focus: focus}
	}
	return rv
}
...
func (s Step) String() string {
	...
	return fmt.Sprintf("%q => %s %s", s.Inst, op, s.Lens)
}
...
func (s Step) GetHash() int {
	return hashStr(s.Inst)
}

func (s Step) GetBox() int {
	return hashStr(s.Lens.Label)
}

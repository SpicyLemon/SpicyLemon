...
func Solve(params *Params) (string, error) {
	...
	if params.Verbose {
		Stdoutf("Graph:\n%s", graph)
		DrawGraph(input.Cells, graph)
	}
	start := graph.GetStart()
	answer := FindMaxPath(start, 0, nil)
	return fmt.Sprintf("%d", answer), nil
}

func FindMaxPath(cur *GraphNode, length int, past []*GraphNode) int {
	...
	for _, edge := range cur.Edges {
		...
		res := FindMaxPath(edge.To, length+edge.Length, past)
		if res > rv {
			rv = res
		}
	}
...
func DrawGraph(cells [][]byte, graph Graph) {
	...
	Stdoutf("Cells:\n%s", CreateIndexedGridStringBz(cells2, nil, nodes))
}
...
func BuildGraph(cells [][]byte, params *Params) Graph {
	...
	for _, vector := range vectors {
		end, length := FollowPath(cells, vector)
		if end == nil {
			continue
		}
		endNode := rv.Get(end)
...
func FollowPath(cells [][]byte, vector *Vector) (*Point, int) {
	x, y := vector.GetXY()
	dir := vector.Dir
	length := 0

...
func Solve(params *Params) (string, error) {
	...
	if debug {
		paths := make([]XY, 0, len(path1)+len(path2))
		paths = append(paths, path1...)
		paths = append(paths, path2...)
		Stdoutf("Paths:\n%s", input.DrawPath(paths, Last(path1), Last(path2)))
	}
	answer := len(path1) - 1
...
func (i Input) DrawPath(path []XY, highlights ...XY) string {
	return CreateIndexedGridString(i.Map, path, append([]XY{i.Start}, highlights...))
}
...
func CreateIndexedGridString(vals [][]byte, colorPoints []XY, highlightPoints []XY) string {
	...
	for y, r := range vals {
		...
		for x := 0; x < width; x++ {
			...
			cell := fmt.Sprintf(cellFmt, string(v))
			switch textFmt[y][x] {
			case 0: // default look.
				rv.WriteString(cell)
			case 1: // color only.

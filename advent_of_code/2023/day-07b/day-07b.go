...
func Solve(params *Params) (string, error) {
	...
	sort.Slice(input.Hands, func(i, j int) bool {
		return input.Hands[i].Strength < input.Hands[j].Strength
	})
	Debugf("Sorted Input:\n%s", input)
	answer := 0
...
func (i Input) String() string {
	return fmt.Sprintf("Hands (%d):\n%s\n", len(i.Hands), StringNumberJoin(i.Hands, 1, "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Hands: make([]*Hand, len(lines))}
	for i, line := range lines {
		parts := strings.Split(line, " ")
		bid, err := strconv.Atoi(strings.TrimSpace(parts[1]))
		if err != nil {
			return nil, fmt.Errorf("failed to parse hand bid %q from line %q: %w", parts[1], line, err)
		}
		rv.Hands[i] = &Hand{Cards: parts[0], Bid: bid}
	}
	return &rv, nil
}
...
func (h Hand) String() string {
	if h.Strength == 0 {
		return fmt.Sprintf("%s% 5d", h.Cards, h.Bid)
	}
	return fmt.Sprintf("%s% 5d => %-15s % 8d", h.Cards, h.Bid, h.HandType, h.Strength)
}

func (h *Hand) SetStrength() {
	h.HandType = GetHandType(h.Cards)
	h.Strength = int(h.HandType)
	for _, card := range h.Cards {
		h.Strength = h.Strength*13 + CardVal(card)
	}
}

func CardVal(card rune) int {
	switch card {
	case 'J':
		return 0
	...
	case 'A':
		return 12
	default:
		panic(fmt.Errorf("unknown card %q", card))
	}
}
...
func (t HandType) String() string {
	...
	case FiveOfAKind:
		return "6: FiveOfAKind"
	default:
		return fmt.Sprintf("%d: UNKNOWN", t)
	}
}
...
func GetHandType(cards string) HandType {
	...
	// jCount cannot be 3 here. If it is 3, then count+3 must be either 4 or 5 and caught above.
	// if jCount is 2 here, we know we neither have a pair nor three of a kind (without the J).
	// If we had three of a kind, count+2 = 5, if a pair, count+2 = 4, both caught above.
	// So if jCount is 2, we know we end up with three of a kind.
	if jCount == 2 {
		return ThreeOfAKind
	}
	// jCount can be a most 1 from here down.
	if haveThree {
		// If we had a three of a kind, we know there aren't any J because count+1 = 4, caught earlier.
		// So we just check it normally.
		if pairs > 0 {
			return FullHouse
		}
		return ThreeOfAKind

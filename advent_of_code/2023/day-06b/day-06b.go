...
func (i Input) String() string {
	return fmt.Sprintf("Races (%d):\n%s\n", len(i.Races),
		strings.Join(AddLineNumbers(MapSlice(i.Races, Race.String), 1), "\n"))
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "Time:") {
			timeStr := strings.ReplaceAll(strings.TrimPrefix(line, "Time:"), " ", "")
			time, err = strconv.Atoi(timeStr)
			if err != nil {
				return nil, fmt.Errorf("failed to parse times %q: %w", timeStr, err)
			}
		}
		if strings.HasPrefix(line, "Distance:") {
			distStr := strings.ReplaceAll(strings.TrimPrefix(line, "Distance:"), " ", "")
			dist, err = strconv.Atoi(distStr)
			if err != nil {
				return nil, fmt.Errorf("failed to parse distances %q: %w", distStr, err)
			}
		}
	}
	rv.Races = []Race{{Time: time, Dist: dist}}
	return &rv, nil
}

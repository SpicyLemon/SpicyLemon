...
func (s *Solver) Run(params *Params) {
	...
	for !s.IsDone() {
		i++
		s.CalculateNext()
		Debugf("Calculated %d moves. Solver: %s", i, s)
		if params.Verbose && i%250_000 == 0 && !debug {
			Stderrf("Calculated %d moves. Solver: %s", i, s)
...
func ThresholdCost(node *PQNode) int {
	return node.MinCost * 9 / 10
}
...
func (s *Solver) CalculateNext() {
	...
	if ThresholdCost(keyNode) > s.KnownMins[keyNode.Y][keyNode.X] {
		Debugf("Key Node's MinCost %d too much over: %d", keyNode.MinCost, s.KnownMins[keyNode.Y][keyNode.X])
		return
	}
	...
	for _, move := range nextMoves {
		...
		if ThresholdCost(move) > s.KnownMins[move.Y][move.X] {
			Debugf("  MinCost: %d too much over: %d", move.MinCost, s.KnownMins[move.Y][move.X])
			continue
		}
...
func (s Solver) GetNextMoves(cur *PQNode) []*PQNode {
	...
	threePrev := cur.Trace(3)
	Debugf("Three Prev: %s", threePrev)
	mustTurn := false
	if len(threePrev) >= 3 {
		dx := Abs(cur.X - threePrev[2].X)
		dy := Abs(cur.Y - threePrev[2].Y)
		Debugf("Last 3: dx: %d, dy: %d", dx, dy)
...
func SamePrevs(n1, n2 *PQNode) bool {
	return n1 != nil && n2 != nil &&
		SamePoints(n1, n2) &&
		SamePoints(n1.Prev, n2.Prev) &&
		SamePoints(n1.Prev2, n2.Prev2) &&
		SamePoints(n1.Prev3, n2.Prev3)
}

...
func Solve(params *Params) (string, error) {
	...
	goal := Draw{Red: 12, Green: 13, Blue: 14}
	Debugf("Parsed Input:\n%s", input)
	for i, game := range input {
		input[i].Maxs = GetMaxs(game.Draws)
	}
	Debugf("Maxs calculated:\n%s", input)
	results := GetWins(goal, input)
	Debugf("Results calculated:\n%+v", results)
	answer := SumWins(results)
...
func (i Input) String() string {
	games := make([]string, len(i))
	for j, g := range i {
		games[j] = g.String()
	}
	return fmt.Sprintf("Games (%d):\n%s\n", len(games), strings.Join(AddLineNumbers(games, 1), "\n"))
}

func ParseInput(lines []string) (Input, error) {
	defer FuncEnding(FuncStarting())
	rv := make(Input, len(lines))
	var err error
	for i, line := range lines {
		rv[i], err = ParseGame(line)
		if err != nil {
			return nil, err
		}
	}
...
func (g Game) String() string {
	draws := make([]string, len(g.Draws))
	for i, draw := range g.Draws {
		draws[i] = draw.String()
	}
	return fmt.Sprintf("Game %d: %s: [%s]", g.N, strings.Join(draws, "; "), g.Maxs.String())
}

func ParseGame(game string) (Game, error) {
	parts := strings.Split(game, ":")
	if len(parts) != 2 {
		return Game{}, fmt.Errorf("cannot parse game %q", game)
	}

	gnum, err := strconv.Atoi(strings.TrimPrefix(strings.TrimSpace(parts[0]), "Game "))
	if err != nil {
		return Game{}, fmt.Errorf("invalid game number %q: %w", parts[0], err)
	}

	rv := Game{N: gnum}
	rv.Draws, err = ParseDraws(parts[1])
	return rv, err
}

func GetWins(goal Draw, games []Game) map[int]bool {
	rv := make(map[int]bool, len(games))
	for _, game := range games {
		rv[game.N] = IsWin(goal, game)
	}
	return rv
}

func SumWins(results map[int]bool) int {
	rv := 0
	for g, w := range results {
		if w {
			rv += g
		}
	}
...
func IsWin(goal Draw, game Game) bool {
	return goal.Red >= game.Maxs.Red && goal.Blue >= game.Maxs.Blue && goal.Green >= game.Maxs.Green
}
...
func (d Draw) String() string {
	return fmt.Sprintf("%d red, %d blue, %d green", d.Red, d.Blue, d.Green)
}

func ParseDraw(draw string) (Draw, error) {
	parts := strings.Split(draw, ",")
	rv := Draw{}
	for _, part := range parts {
		pieces := strings.Split(strings.TrimSpace(part), " ")
		if len(pieces) != 2 {
			return rv, fmt.Errorf("unknown piece format %q", part)
		}
		val, err := strconv.Atoi(pieces[0])
		if err != nil {
			return rv, fmt.Errorf("uknown piece number %q: %w", part, err)
		}
		switch pieces[1] {
		case "red":
			rv.Red = val
		...
		case "green":
			rv.Green = val
		default:
			return rv, fmt.Errorf("unknown piece color %q", part)
		}
	}
	return rv, nil
}

func ParseDraws(draws string) ([]Draw, error) {
	parts := strings.Split(draws, ";")
	var err error
	rv := make([]Draw, len(parts))
	for i, draw := range parts {
		rv[i], err = ParseDraw(draw)
		if err != nil {
			return rv, fmt.Errorf("unknown draw %q: %w", draw, err)
		}
	}
	return rv, nil
}

func GetMaxs(draws []Draw) Draw {
	rv := Draw{}
	for _, draw := range draws {
		if draw.Red > rv.Red {
			rv.Red = draw.Red
		}
		if draw.Blue > rv.Blue {
			rv.Blue = draw.Blue
		}
		if draw.Green > rv.Green {
			rv.Green = draw.Green
		}
	}
	return rv

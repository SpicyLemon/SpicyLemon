...
func (i Input) String() string {
	return fmt.Sprintf("Lines (%d):", len(i.Lines)) + strings.Join(AddLineNumbers(i.Lines, 1), "\n")
}
...
func CalVal(line string) int {
	...
	rv, err := strconv.Atoi(string(first) + string(last))
	if err != nil {
		panic(err)
	}
	return rv

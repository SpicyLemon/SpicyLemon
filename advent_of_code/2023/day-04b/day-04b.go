...
func Solve(params *Params) (string, error) {
	...
	for i, card := range input.Cards {
		winners := card.CountWinners()
		for j := 1; j <= winners; j++ {
			input.Cards[i+j].Count += card.Count
		}
		answer += card.Count
	}
...
func (i Input) String() string {
	cards := make([]string, len(i.Cards))
	for n, card := range i.Cards {
		cards[n] = card.String()
	}
	return fmt.Sprintf("Cards (%d):\n%s\n", len(i.Cards), strings.Join(AddLineNumbers(cards, 1), "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Cards: make([]Card, 0, len(lines))}
	var errs []error
	for _, line := range lines {
		cn := strings.Split(line, ":")
		if len(cn) != 2 {
			errs = append(errs, fmt.Errorf("could not parse line %q: expected exactly 1 colon", line))
			continue
		}
		lr := strings.Split(cn[1], "|")
		if len(lr) != 2 {
			errs = append(errs, fmt.Errorf("could not parse line %q: expected exactly 1 pipe", line))
			continue
		}

		n, err := strconv.Atoi(strings.TrimSpace(strings.TrimPrefix(cn[0], "Card")))
		if err != nil {
			errs = append(errs, fmt.Errorf("could not parse line %q: unexpected card format", line))
			continue
		}

		card := Card{N: n, Count: 1}
		card.Left, err = SplitParseInts(lr[0], " ")
		if err != nil {
			errs = append(errs, fmt.Errorf("could not parse line %q: invalid left numbers: %w", line, err))
			continue
		}
		card.Right, err = SplitParseInts(lr[1], " ")
		if err != nil {
			errs = append(errs, fmt.Errorf("could not parse line %q: invalid right numers: %w", line, err))
			continue
		}
...
func (c Card) String() string {
	return fmt.Sprintf("Card % 3d: %v | %v", c.N, c.Left, c.Right)
}

func (c Card) CountWinners() int {
	return len(FindSubset(c.Left, c.Right))
}

func FindSubset(main, toFind []int) []int {
	mainMap := make(map[int]bool)
	for _, m := range main {
		mainMap[m] = true
	}

	rv := make([]int, 0, len(main))

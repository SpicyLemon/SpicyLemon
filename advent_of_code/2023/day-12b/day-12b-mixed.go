...
func Solve(params *Params) (string, error) {
	...
	// Debugf("Parsed Input:\n%s", input)
	input.Expand()
	// Debugf("Expanded Input:\n%s", input)
	answer := 0
	if params.Count > 0 {
		spring := input.Springs[params.Count-1]
		nv := CountOptions(spring)
		Stdoutf("% 4d: %s = %d", params.Count-1, spring, nv)
		answer += nv
	} else {
		for i, spring := range input.Springs {
			if params.Verbose {
				Stdoutf("% 4d: %s", i, spring)
			}
			nv := CountOptions(spring)
			if params.Verbose {
				Stdoutf("% 4d: %s = %d", i, spring, nv)
			}
			answer += nv
		}
	}
...
func CountOptions(s *Spring) int {
	defer FuncEnding(FuncStarting())

	if bytes.Contains(s.Record, []byte{Operational}) {
		return CountOptionsIter(s)
	}
	return CountOptionsRec(s.Record, s.Groups)
}

var memoizedCountOptionsRec = make(map[string]int)

func CountOptionsRec(target []byte, groups []int) int {
	defer FuncEnding(FuncStarting())
	key := fmt.Sprintf("%s %v", target, groups)
	rv, known := memoizedCountOptionsRec[key]
	if known {
		Debugf("Target: %s, Groups: %v, Already known: %d", string(target), groups, rv)
		return rv
	}

	Debugf("Target: %s, Groups: %v", string(target), groups)
	sum := SumInts(groups)
	...
	rv = 0
	for i := 0; i <= extra; i++ {
		check := MakeCheck(i, groups, tarLen)
		if CanBe(check, target, sum) {
			if len(groups) == 1 {
				...
				if len(groups) == 2 && IsAllBz(newTar, Unknown) {
					nv := len(newTar) - groups[1] + 1
					Debugf("%d %s %d/%d: Check: %s => %s Shortcut = %d", tarLen, target, i, extra, check, newTar, nv)
					rv += nv
				} else {
					Debugf("%d %s %d/%d: Check: %s => Recurse", tarLen, target, i, extra, check)
					rv += CountOptionsRec(newTar, groups[1:])
					Debugf("%d %s %d/%d: Groups: %v  Options: %d", tarLen, target, i, extra, groups, rv)
				}
			}
		} else {
			Debugf("%d %s %d/%d: Check: %s => nope", tarLen, target, i, extra, check)
		}
	}

	memoizedCountOptionsRec[key] = rv
	Debugf("Target: %s  Groups: %v  Options: %d", target, groups, rv)
	return rv
}

func CanBe(check, target []byte, minBroke int) bool {
	if len(check) > len(target) {
		return false
	}
	broke := 0
	for i, b := range check {
		if target[i] != b && target[i] != Unknown {
			return false
		}
		if b == Damaged {
			broke++
...
func MakeCheck(opers int, groups []int, tarLen int) []byte {
	rvLen := opers + groups[0] + 1
	if len(groups) == 1 {
		rvLen = tarLen
	}
	rv := bytes.Repeat([]byte{Operational}, rvLen)
...
func CountOptionsIter(s *Spring) int {
	defer FuncEnding(FuncStarting())

	Debugf("Spring: %s", s)
	groups := SplitRecord(s.Record)
	if debug {
		Debugf("Groups (%d): %s", len(groups), strings.Join(MapSlice(groups, BzStringer), "  "))
	}

	optRanges := make([][]*OptRange, len(groups))
	...
	opts := []*OptRange{{Count: 1, Groups: nil}}

	for _, ors := range optRanges {
		var newOpts []*OptRange
		for _, optRange := range ors {
			for _, opt := range opts {
				newOpt := CombineOptRanges(opt, optRange)
				if newOpt.CanBe(s.Groups) {
	...
	if debug {
		Debugf("Opts (%d):\n%s", len(opts), StringNumberJoin(opts, 0, "\n"))
	}

	rv := 0
	for _, opt := range opts {
		if EqualInts(opt.Groups, s.Groups) {
			rv += opt.Count
		}
	}
...
func (o OptRange) String() string {
	return fmt.Sprintf("%d x %s", o.Count, strings.Join(MapSlice(o.Groups, IntStringer), ","))
}

func (o OptRange) CanBe(groups []int) bool {
	if len(o.Groups) > len(groups) {
		return false
	}
	return EqualInts(o.Groups, groups[:len(o.Groups)])
}

func (o OptRange) Copy() *OptRange {
	return &OptRange{
		Count:  o.Count,
		Groups: CopyInts(o.Groups),
	}
}

func (o *OptRange) Concat(other *OptRange) {
	o.Count *= other.Count
	o.Groups = append(o.Groups, other.Groups...)
}

func CombineOptRanges(ranges ...*OptRange) *OptRange {
	rv := ranges[0].Copy()
...
func SplitRecord(rec []byte) [][]byte {
	var rv [][]byte
	inGroup := false
	for _, b := range rec {
		switch b {
		case Damaged, Unknown:
			...
			inGroup = true
			rv[len(rv)-1] = append(rv[len(rv)-1], b)
		case Operational:
			inGroup = false
		}
	}
...
var memoizedOptRanges = make(map[string][]*OptRange)

func BuildOptRanges(record []byte) []*OptRange {
	rv, known := memoizedOptRanges[string(record)]
	if known {
		return rv
	}
	rvMap := make(map[string]*OptRange)
	countGroups := func(g []int) {
		key := strings.Join(MapSlice(g, IntStringer), ",")
		if _, found := rvMap[key]; !found {
			rvMap[key] = &OptRange{Groups: g}
			rv = append(rv, rvMap[key])
		}
		rvMap[key].Count++
	}

	opts := BuildOptions(record)
	for _, opt := range opts {
		g := GetGroups(opt)
		countGroups(g)
	}

	memoizedOptRanges[string(record)] = rv

	return rv
}

func BuildOptions(record []byte) [][]byte {
	rv := make([][]byte, 0, Pow2(bytes.Count(record, []byte{Unknown})))

	recLen := len(record)
	maxR := len(record) - 1
	switch record[maxR] {
	case Operational, Damaged:
		rv = append(rv, make([]byte, recLen))
		rv[0][maxR] = record[maxR]
	case Unknown:
		rv = append(rv, make([]byte, recLen), make([]byte, recLen))
		rv[0][maxR] = Operational
		rv[1][maxR] = Damaged
	}
...
func (i Input) String() string {
	return fmt.Sprintf("Springs (%d):\n%s", len(i.Springs), StringNumberJoin(i.Springs, 1, "\n"))
}

func (i *Input) Expand() {
	for _, s := range i.Springs {
		rec := string(s.Record)
		s.Record = []byte(rec + "?" + rec + "?" + rec + "?" + rec + "?" + rec)
		s.Groups = RepeatInts(s.Groups, 5)
	}
}

func RepeatInts(ints []int, count int) []int {
	rv := make([]int, 0, len(ints)*count)
	for i := 0; i < count; i++ {
		rv = append(rv, ints...)
	}
	return rv
...
func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Springs: make([]*Spring, len(lines))}
	for i, line := range lines {
		parts := strings.Split(line, " ")
		if len(parts) != 2 {
			return nil, fmt.Errorf("unable to parse line %q: expected exactly one space", line)
		}
		groups, err := SplitParseInts(parts[1], ",")
		if err != nil {
			return nil, fmt.Errorf("unable to parse line %q: invalid group numbers %q: %w", line, parts[1], err)
		}
		rv.Springs[i] = &Spring{
			Record: []byte(parts[0]),
			Groups: groups,
...
func (s Spring) String() string {
	return fmt.Sprintf("%s %s", string(s.Record), strings.Join(MapSlice(s.Groups, IntStringer), ","))
}

func IntStringer(i int) string {
...
func IsAllBz(bz []byte, b byte) bool {
	for _, a := range bz {
		if a != b {
			return false
		}

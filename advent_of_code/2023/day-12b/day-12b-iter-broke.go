...
func Solve(params *Params) (string, error) {
	...
	if params.Count > 0 {
		...
		Debugf("% 4d: %s = %d", params.Count-1, spring, nv)
		answer += nv
	} else {
		for i, spring := range input.Springs {
			nv := CountOptions(spring)
			Debugf("% 4d: %s = %d", i, spring, nv)
			answer += nv
		}
	}
...
func CountOptions(s *Spring) int {
	...
	for i, g := range groups {
		optRanges[i] = BuildGroupOptRanges(g)
		if debug {
...
func (o OptRange) CanBe(target []int) bool {
	if len(o.Groups) > len(target) {
		return false
	}
	return EqualInts(o.Groups, target[:len(o.Groups)])
}
...
func BuildGroupOptRanges(group []byte) []*OptRange {
	parts := SplitGroup(group)
	if debug {
		Debugf("%s parts (%d): %s", group, len(parts), strings.Join(MapSlice(parts, BzStringer), " "))
	}
	optRangeOpts := make([][]*OptRange, len(parts))
	for i, part := range parts {
		optRangeOpts[i] = BuildOptRanges(part)
	}

	rv := []*OptRange{{Count: 1, Groups: nil}}
	for _, opts := range optRangeOpts {
		var newRv []*OptRange
		for _, opt := range opts {
			for _, v := range rv {
				newRv = append(newRv, CombineOptRanges(v, opt))
			}
		}
		rv = newRv
	}
...
func SplitGroup(group []byte) [][]byte {
	var rv [][]byte
	for i, b := range group {
		if i > 0 && rv[len(rv)-1][0] == b {
			rv[len(rv)-1] = append(rv[len(rv)-1], b)
		} else {
			rv = append(rv, []byte{b})
		}

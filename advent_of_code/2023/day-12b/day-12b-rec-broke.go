...
func CountOptions(s *Spring) int {
	...
	Debugf("Counting: %s", s)
	return CountSubOptions(s.Record, s.Groups)
}

func CountSubOptions(target []byte, groups []int) int {
	defer FuncEnding(FuncStarting())
	...
	tarLen := len(target)
	if tarLen < needed {
		Debugf("Target length %d is less than needed length %d = %d + %d - 1", tarLen, needed, sum, len(groups))
		return 0
	}
	extra := tarLen - needed
	rv := 0
	for i := 0; i <= extra; i++ {
		...
		if CanBe(check, target, sum) {
			if len(groups) == 1 {
				Debugf("%d %s %d: Check: %s => Count It", tarLen, target, i, check)
				rv += 1
			} else {
				newTar := target[len(check):]
				if len(groups) == 2 && IsAllBz(newTar, Unknown) {
					nv := len(newTar) - groups[1] + 1
					Debugf("%d %s %d: Check %s => %s Shortcut = %d", tarLen, target, i, check, newTar, nv)
					rv += nv
				} else {
					Debugf("%d %s %d: Check %s => Recurse", tarLen, target, i, check)
					rv += CountSubOptions(target[len(check):], groups[1:])
					Debugf("%d %s %d: Groups: %v  Options: %d", tarLen, target, i, groups, rv)
				}
			}
		} else {
			Debugf("%d %s %d: Check: %s => nope", tarLen, target, i, check)
		}
	}

...
func Solve(params *Params) (string, error) {
	...
	for _, seed := range input.Seeds {
		loc := GetLocation(seed, input.Almanac)
		if loc < answer {
			answer = loc
		}
	}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "seeds: ") {
			rv.Seeds, err = SplitParseInts(strings.TrimPrefix(line, "seeds: "), " ")
			if err != nil {
		...
		rangeParts, err := SplitParseInts(line, " ")
		if err != nil {
			return nil, fmt.Errorf("unable to parse range line %q: %w", line, err)
		}
		if len(rangeParts) != 3 {
			return nil, fmt.Errorf("unable to parse range line %q: expected exactly 3 parts, found %v",
				line, rangeParts)
		}
...
func (e Entry) String() string {
	return fmt.Sprintf("%s => %s: [%s]", e.Source, e.Dest, strings.Join(MapSlice(e.Ranges, Range.String), "], ["))
}
...
func (r Range) String() string {
	return fmt.Sprintf("%d..%d => %d..%d", r.Source, r.Source+r.Length-1, r.Dest, r.Dest+r.Length-1)
}

func (r Range) Convert(n int) (int, bool) {
	if r.Source <= n && n < r.Source+r.Length {
		return n - r.Source + r.Dest, true
	}
	return 0, false
}

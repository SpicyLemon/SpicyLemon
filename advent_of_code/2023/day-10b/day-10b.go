...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	insides := FindInsides(input)
	answer := len(insides)
	return fmt.Sprintf("%d", answer), nil
}

func FindInsides(input *Input) []XY {
	// Initialize a matrix of the same size as the map with an i in each cell.
	vals := make([][]byte, len(input.Map))
	for r, row := range input.Map {
		vals[r] = []byte(strings.Repeat("i", len(row)))
	}

	// Copy all the actual loop chars.
	path1, path2 := FindPaths(input)
	path := make([]XY, 0, len(path1)+len(path2))
	path = append(path, path1...)
	path = append(path, path2...)
	for _, p := range path {
		x, y := p.GetXY()
		vals[y][x] = input.Map[y][x]
	}

	// Change all edge spots and all spots easily connectable to an edge with a space (for outside).
	keepGoing := true
	for keepGoing {
		keepGoing = false
		for y, row := range vals {
			for x := range row {
				if IsNewOutside(x, y, vals) {
					vals[y][x] = ' '
					keepGoing = true
				}
			}
	...
	// Check each remaining i. Set it to 'I' for inside, or 'x' for outside but not obviously.
	var fixed []XY
	for y, row := range vals {
		for x, cell := range row {
			if cell == 'i' {
				if IsInside(x, y, vals) {
					vals[y][x] = 'I'
				} else {
					vals[y][x] = 'x'
					fixed = append(fixed, Point{X: x, Y: y})
				}
			}
		}
	}

	// Anything that is an I is now an inside cell. Find all of them.
	var insides []XY
	for y, row := range vals {
		for x, cell := range row {
			if cell == 'I' {
				insides = append(insides, Point{X: x, Y: y})
			}
		}
	}

	if debug {
		// Output the vals matrix, but use the unicode box-art lines instead of the map chars.
		disp := make([][]string, len(vals))
		old := "|-LJ7Fx"
		pipes := []rune("|─└┘┐┌×")
		for y, row := range vals {
			disp[y] = make([]string, len(vals[y]))
			for x, cell := range row {
				nv := rune(cell)
				if ByteIn(cell, []byte(old)) {
					i := strings.Index(old, string(vals[y][x]))
					nv = pipes[i]
				}
				disp[y][x] = string(nv)
			}
		}
		// Draw the matrix, coloring the fixed ones and highlighting the inside parts.
		Stdoutf("Result:\n%s", CreateIndexedGridString(disp, fixed, insides))
	}

	return insides
}

func IsNewOutside(x, y int, vals [][]byte) bool {
	if vals[y][x] != 'i' {
		return false
	}
	if x <= 0 || y <= 0 || x >= len(vals[y])-1 || y >= len(vals)-1 {
		return true
	}
	return ByteIn(' ', []byte{
		vals[y-1][x-1], vals[y-1][x], vals[y-1][x+1],
		vals[y][x-1], vals[y][x], vals[y][x+1],
		vals[y+1][x-1], vals[y+1][x], vals[y+1][x+1],
	})
}

func IsInside(x, y int, vals [][]byte) bool {
	// This draws a ray from the provided (x, y) diagonally up and left until it reaches the edge.
	// We count the times we cross the path along the way.
	// If we cross an odd number of times, then (x, y) is inside.
	count := 0
	x--
	y--
	for x >= 0 && y >= 0 {
		// Don't need to worry about L or 7 because the ray would either miss it or touch it twice.
		if ByteIn(vals[y][x], []byte("|-JF")) {
			count++
		}
		x--
		y--
...
func FindPaths(input *Input) ([]XY, []XY) {
	path1, path2 := make([]XY, 1, len(input.Map)*3), make([]XY, 1, len(input.Map)*3)
	path1[0] = input.Start
	path2[0] = input.Start

	dx1, dy1, dx2, dy2 := GetDXY(input.Get(input.Start))
	path1 = append(path1, Point{X: input.Start.GetX() + dx1, Y: input.Start.GetY() + dy1})
	path2 = append(path2, Point{X: input.Start.GetX() + dx2, Y: input.Start.GetY() + dy2})

	for !EqualPoints(Last(path1), Last(path2)) {
		path1 = append(path1, input.GetNext(Last2(path1), Last(path1)))
		path2 = append(path2, input.GetNext(Last2(path2), Last(path2)))
	}

	return path1, path2
}

func GetDXY(b byte) (dx1, dy1, dx2, dy2 int) {
	switch b {
	case '|':
		dy1 = -1
		dy2 = 1
	...
	case 'F':
		...
		dy2 = 1
	default:
		panic(fmt.Errorf("unknown location tile %b", b))
	}
	return
}

func CanUp(b byte) bool {
	return ByteIn(b, []byte("|LJ"))
}

func CanDown(b byte) bool {
	return ByteIn(b, []byte("|7F"))
}

func CanLeft(b byte) bool {
	return ByteIn(b, []byte("-LF"))
}

func CanRight(b byte) bool {
	return ByteIn(b, []byte("-7J"))
}

func EqualPoints(a, b XY) bool {
...
func (i Input) DrawPath(path []XY, highlights ...XY) string {
	return CreateIndexedGridStringBz(i.Map, path, append([]XY{i.Start}, highlights...))
}

func (i Input) Get(p XY) byte {
	return i.Map[p.GetY()][p.GetX()]
}

func (i Input) FigureS() {
	x, y := i.Start.GetXY()
	canUp := y > 0 && ByteIn(i.Map[y-1][x], []byte("|7F"))
	canDown := y < len(i.Map)-1 && ByteIn(i.Map[y+1][x], []byte("|LJ"))
	canRight := x < len(i.Map[y])-1 && ByteIn(i.Map[y][x+1], []byte("-J7"))
	canLeft := x > 0 && ByteIn(i.Map[y][x-1], []byte("-LF"))
	nv := 'S'
	switch {
	case canUp && canDown && !canLeft && !canRight:
		nv = '|'
	case canUp && canRight && !canDown && !canLeft:
		nv = 'L'
	case canUp && canLeft && !canDown && !canRight:
		nv = 'J'
	case canLeft && canRight && !canUp && !canDown:
		nv = '-'
	case canDown && canRight && !canUp && !canLeft:
		nv = 'F'
	case canDown && canLeft && !canUp && !canRight:
		nv = '7'
	default:
		ymin := y - 1
		if ymin < 0 {
		...
		xmax := x + 1
		if xmax > len(i.Map[ymax])-1 {
			xmax = len(i.Map[ymax]) - 1
		}
		lines := make([]string, ymax-ymin+1)
		for r := ymin; r <= ymax; r++ {
			line := make([]byte, xmax-xmin+1)
			for c := xmin; c <= xmax; c++ {
				line[c-xmin] = i.Map[r][c]
			}
			lines[r-ymin] = string(line)
		}
		Stderrf("Failed to determin what was under the S.\ncanUp: %t, canDown: %t, canLeft: %t, canRight: %t\n%s",
			canUp, canDown, canLeft, canRight, strings.Join(lines, "\n"))
	}
	i.Map[y][x] = byte(nv)
}

func (i Input) GetNext(prev, cur XY) XY {
	dx1, dy1, dx2, dy2 := GetDXY(i.Get(cur))
	rv := Point{X: cur.GetX() + dx1, Y: cur.GetY() + dy1}
	if EqualPoints(prev, rv) {
		rv = Point{X: cur.GetX() + dx2, Y: cur.GetY() + dy2}
	}
	return rv
}

func ByteIn(val byte, opts []byte) bool {
	for _, b := range opts {
		if val == b {
			return true
		}
...
func BytesStringer(bz []byte) string {
	return string(bz)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Map: make([][]byte, len(lines))}
	for i, line := range lines {
		rv.Map[i] = []byte(line)
		if j := strings.Index(line, "S"); j >= 0 {
			rv.Start = Point{X: j, Y: i}
		}
	}
	rv.FigureS()
	return &rv, nil
...
func Last[S []E, E any](vals S) E {
	return vals[len(vals)-1]
}

func Last2[S []E, E any](vals S) E {
...
func CreateIndexedGridStringBz(vals [][]byte, colorPoints []XY, highlightPoints []XY) string {
	strs := make([][]string, len(vals))
	for y, row := range vals {
		strs[y] = make([]string, len(row))
		for x, cell := range row {
			strs[y][x] = string(cell)
		}
	}

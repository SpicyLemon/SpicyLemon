...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	parts := ReadSchematic(input.Schematic)
	answer := 0
...
func ReadSchematic(schematic [][]byte) []int {
	...
	for r, row := range schematic {
		...
		for c, cell := range row {
			...
			if !isDigitCell(cell) {
				if part > 0 {
					// Check this one, and the ones above/below for a part.
					isPart = isPart ||
						(!isTop && isPartCell(schematic[r-1][c])) ||
						isPartCell(cell) ||
						(!isBottom && isPartCell(schematic[r+1][c]))
					if isPart {
						rv = append(rv, part)
					}
					part = 0
			...
			part = part*10 + int(cell-'0')
			// Check above and below this for a part.
			isPart = isPart ||
				(!isTop && isPartCell(schematic[r-1][c])) ||
				(!isBottom && isPartCell(schematic[r+1][c]))
			// Check to the left. If it's not a digit, check it and the ones above and below for parts.
			if !isLeft && !isDigitCell(row[c-1]) {
				isPart = isPart ||
					(!isTop && isPartCell(schematic[r-1][c-1])) ||
					isPartCell(schematic[r][c-1]) ||
					(!isBottom && isPartCell(schematic[r+1][c-1]))
			}
		}
	}
...
func isPartCell(b byte) bool {
	return b != '.' && !isDigitCell(b)
}

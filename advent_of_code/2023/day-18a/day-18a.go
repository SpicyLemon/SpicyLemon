...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := DigPlanMatrix(input.Plan)
...
func DigPlanMatrix(plan []Step) int {
	defer FuncEnding(FuncStarting())
	trench := FollowDigPlan(plan)
	Debugf("Trench length: %d", len(trench))
	...
	pit := make([][]byte, height)
	for y := range pit {
		pit[y] = []byte(strings.Repeat(string(Empty), width))
	}

	shiftedTrench := make(ColorPath, 0, len(trench))
	for _, p := range trench {
		...
		shiftedTrench = append(shiftedTrench, newP)
	}

	if debug {
		Stderrf("Trench (%d):\n%s", len(shiftedTrench), CreateIndexedGridStringBz(pit, nil, shiftedTrench))
	}
	...
	var interior []ColorPoint
	for y, row := range pit {
		for x, cell := range row {
			if cell == Empty {
				pit[y][x] = Interior
				interior = append(interior, ColorPoint{X: x, Y: y})
			}
		}
	}

	if debug {
		Stderrf("Pit (%d):\n%s", len(trench)+len(interior), CreateIndexedGridStringBz(pit, interior, shiftedTrench))
	}

	return len(trench) + len(interior)
}

func IsOutside(matrix [][]byte, x, y int) bool {
	if x <= 0 || x >= len(matrix[0])-1 || y <= 0 || y >= len(matrix)-1 {
		return true
	}
	return matrix[y][x-1] == Outside ||
		matrix[y][x+1] == Outside ||
		matrix[y-1][x] == Outside ||
		matrix[y+1][x] == Outside
}

func IsInterior(matrix [][]byte, x, y int) bool {
	...
	dx, dy := 0, 0
	switch {
	case ld <= rd && ld <= ud && ld <= dd:
		dx = -1
	case rd <= ld && rd <= ud && rd <= dd:
		dx = 1
	case ud <= ld && ud <= rd && ud <= dd:
		dy = -1
	case dd <= ld && dd <= rd && dd <= ud:
		dy = 1
	default:
		panic(fmt.Errorf("unknown edge distances: L=%d, R=%d, U=%d, D=%d", ld, rd, ud, dd))
	}
	count := 0
	for x >= 0 && x < width && y >= 0 && y < height {
		if matrix[y][x] == Trench {
			count++
		}
		x += dx
		y += dy
...
func FollowDigPlan(plan []Step) ColorPath {
	defer FuncEnding(FuncStarting())
	var rv ColorPath
	cur := ColorPoint{X: 0, Y: 0}
	for _, step := range plan {
		newPath := step.Follow(cur)
		cur = newPath.Last()
...
func (p ColorPoint) String() string {
	return fmt.Sprintf("(%d,%d|%s)", p.X, p.Y, p.Color)
}

// GetX gets this Point's X value.
func (p ColorPoint) GetX() int {
	return p.X
}

// GetY gets this Point's Y value.
func (p ColorPoint) GetY() int {
	return p.Y
}

// GetXY gets this Point's (X, Y) values.
func (p ColorPoint) GetXY() (int, int) {
	return p.X, p.Y
}

type ColorPath []ColorPoint

func (c ColorPath) String() string {
	return strings.Join(MapSlice(c, ColorPoint.String), " ")
}

func (c ColorPath) Last() ColorPoint {
	return c[len(c)-1]
}

func (c ColorPath) MinMax() (xMin, yMin, xMax, yMax int) {
	if len(c) == 0 {
		return
	}
	xMin, xMax = c[0].X, c[0].X
	yMin, yMax = c[0].Y, c[0].Y
	for _, p := range c {
		xMin, xMax = MinMax(xMin, p.X, xMax)
		yMin, yMax = MinMax(yMin, p.Y, yMax)
	}
	return
}

func MinMax(min, v, max int) (int, int) {
	if v < min {
		return v, max
	}
	if v > max {
...
func (s Step) String() string {
	return fmt.Sprintf("%c %d (#%s)", s.Dir, s.Len, s.Col)
}

func (s Step) Follow(cur XY) ColorPath {
	dx, dy := 0, 0
	switch s.Dir {
	case Right:
		dx = 1
...
func ParseStep(line string) (*Step, error) {
	...
	return &Step{
		Dir: parts[0][0],
		Len: length,
		Col: strings.TrimSuffix(strings.TrimPrefix(parts[2], "(#"), ")"),
	}, nil
}

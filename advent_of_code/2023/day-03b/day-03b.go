...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	gears := ReadSchematic(input.Schematic)
	answer := 0
	Debugf("Gears: %v", gears)
...
func (i Input) String() string {
	lines := make([]string, len(i.Schematic))
	for l, line := range i.Schematic {
		lines[l] = string(line)
	}
	return fmt.Sprintf("Schematic:\n%s\n", strings.Join(AddLineNumbers(lines, 1), "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Schematic: make([][]byte, len(lines))}
	for i, line := range lines {
		rv.Schematic[i] = []byte(line)
	}
	return &rv, nil
}

func ReadSchematic(schematic [][]byte) []int {
	defer FuncEnding(FuncStarting())
	var rv []int
	for r, row := range schematic {
		isTop := r == 0
		isBottom := r == len(schematic)-1
		for c, cell := range row {
			...
			checkTL := !isTop && !isLeft
			checkTM := !isTop
			checkTR := !isTop && !isRight
			checkML := !isLeft
			checkMR := !isRight
			checkBL := !isBottom && !isLeft
			checkBM := !isBottom
			checkBR := !isBottom && !isRight
			var parts []int

			if checkTR {
				part, isStart := getPart(r-1, c+1, schematic)
				if part > 0 {
					parts = append(parts, part)
					checkTM = false
					checkTL = checkTL && isStart
				}
			}
			if checkTM {
				part, _ := getPart(r-1, c, schematic)
				if part > 0 {
					parts = append(parts, part)
					checkTL = false
				}
			...
			if checkTL {
				part, _ := getPart(r-1, c-1, schematic)
				if part > 0 {
					parts = append(parts, part)
				}
			}
			...
			if checkMR {
				part, _ := getPart(r, c+1, schematic)
				if part > 0 {
					parts = append(parts, part)
				}
			}
			...
			if checkBR {
				part, isFirst := getPart(r+1, c+1, schematic)
				if part > 0 {
					parts = append(parts, part)
					checkBM = false
					checkBL = checkBL && isFirst
				}
			}
			if checkBM {
				part, _ := getPart(r+1, c, schematic)
			...
			Debugf("Found gear at [%d][%d] = %d * %d = %d", r, c, parts[0], parts[1], gear)
			rv = append(rv, gear)
		}
	}
...
func isDigitCell(b byte) bool {
	return b >= '0' && b <= '9'
}

func isGearCell(b byte) bool {
	return b == '*'
}

// getPart returns the part value and whether r,c was the first digit in the number (true = first).
func getPart(r, c int, schematic [][]byte) (int, bool) {
	if !isDigitCell(schematic[r][c]) {
		return 0, false
	}
	isFirst := true
	part := string(schematic[r][c])
	// Go left first.
	for cc := c - 1; cc >= 0; cc-- {
		cell := schematic[r][cc]
		if !isDigitCell(cell) {
		...
		part = string(cell) + part
		isFirst = false
	}

	// Now go right!
	for cc := c + 1; cc < len(schematic[r]); cc++ {
		cell := schematic[r][cc]
		if !isDigitCell(cell) {
			break
		}
	...
	if err != nil {
		panic(fmt.Errorf("invalid part number %q: %w", part, err))
	}

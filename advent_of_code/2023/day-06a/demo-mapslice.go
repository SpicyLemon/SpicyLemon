...
func Solve(params *Params) (string, error) {
	...
	Stdoutf("Parsed Input:\n%s", input)
	answer := 1
	for _, race := range input.Races {
		answer *= race.CountWins()
	}
...
func (i Input) String() string {
	sections := []string{
		fmt.Sprintf("StringNumberJoin []Race : (%s)", StringNumberJoin(i.Races, 1, ") (")),
		fmt.Sprintf("StringNumberJoin []*Race: (%s)", StringNumberJoin(i.Paces, 1, ") (")),
		fmt.Sprintf("MapSlice  []Race     Race.String : %s", strings.Join(MapSlice(i.Races, Race.String), ", ")),
		fmt.Sprintf("MapSliceP []Race  (*Race).PString: %s", strings.Join(MapSliceP(i.Races, (*Race).PString), ", ")),
		fmt.Sprintf("MapPSlice []*Race    Race.String : %s", strings.Join(MapPSlice(i.Paces, Race.String), ", ")),
		fmt.Sprintf("MapSlice  []*Race (*Race).String : %s", strings.Join(MapSlice(i.Paces, (*Race).String), ", ")),
		fmt.Sprintf("MapSlice  []*Race (*Race).PString: %s", strings.Join(MapSlice(i.Paces, (*Race).PString), ", ")),
		fmt.Sprintf("MapSlice  []Race      RaceString : %s", strings.Join(MapSlice(i.Races, RaceString), ", ")),
		fmt.Sprintf("MapSliceP []Race      RacePString: %s", strings.Join(MapSliceP(i.Races, RacePString), ", ")),
		fmt.Sprintf("MapSlice  []*Race     RacePString: %s", strings.Join(MapSlice(i.Paces, RacePString), ", ")),
		fmt.Sprintf("MapPSlice []*Race     RaceString : %s", strings.Join(MapPSlice(i.Paces, RaceString), ", ")),
	}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "Time:") {
			times, err = SplitParseInts(strings.TrimSpace(strings.TrimPrefix(line, "Time:")), " ")
			if err != nil {
				return nil, fmt.Errorf("failed to parse times %q: %w", line, err)
			}
		}
		if strings.HasPrefix(line, "Distance:") {
			dists, err = SplitParseInts(strings.TrimSpace(strings.TrimPrefix(line, "Distance:")), " ")
			if err != nil {
				return nil, fmt.Errorf("failed to parse distances %q: %w", line, err)
			}
		}
	}
	if len(times) != len(dists) {
		return nil, fmt.Errorf("number of times (%d) does not equal number of distances (%d)", len(times), len(dists))
	}
	rv.Races = make([]Race, len(times))
	rv.Paces = make([]*Race, len(times))
	for i := range times {
		rv.Races[i] = Race{Time: times[i], Dist: dists[i]}
		rv.Paces[i] = &Race{Time: times[i], Dist: dists[i]}
	}
	return &rv, nil
}
...
func (r Race) String() string {
	return fmt.Sprintf("%d ms x %d mm", r.Time, r.Dist)
}

func (r *Race) PString() string {
	return fmt.Sprintf("%d mm in %d ms", r.Dist, r.Time)
}

func RaceString(r Race) string {
	return r.String()
}

func RacePString(r *Race) string {
	return r.PString()
}

func (r Race) Solve() (int, int) {
	sqrt := math.Sqrt(float64(r.Time*r.Time - 4*r.Dist))
	b1 := (float64(r.Time) + sqrt) / 2
	b2 := (float64(r.Time) - sqrt) / 2
	Debugf("%s: sqrt: %f, b1: %f, b2: %f", r, sqrt, b1, b2)
	rv1 := int(math.Floor(b1))
	if r.Dist == (r.Time-rv1)*rv1 {
		rv1--
	}
	rv2 := int(math.Ceil(b2))
	if r.Dist == (r.Time-rv2)*rv2 {
		rv2++
	}
	return rv1, rv2
}
...
func (r Race) CountWins() int {
	...
	rv := n2 - n1 + 1
	Debugf("%s Wins: [%d..(%d)..%d]", r, n1, rv, n2)
	return rv
}
...
// If the slice and mapper are either both concrete or both pointers, use MapSlice.
// If the slice is pointers, but the mapper takes in a concrete E, use MapPSlice.
// If the slice is concrete, but the mapper takes in a pointer to E, use MapSliceP.
...
func StringNumberJoin[S ~[]E, E Stringer](slice S, startAt int, delim string) string {
	return strings.Join(AddLineNumbers(MapSlice(slice, E.String), startAt), delim)
}

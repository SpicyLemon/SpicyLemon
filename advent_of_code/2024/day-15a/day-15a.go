...
func Solve(params *Params) (string, error) {
	...
	for i, move := range input.Moves {
		robot = MoveRobot(move, robot, warehouse)
		if params.Verbose {
			Stderrf("Move %d = %c:\n%s", i, move, MakeWarehouseString(warehouse, robot))
		}
	}
	answer := 0
	for y := range warehouse {
		for x := range warehouse {
			if warehouse[y][x].Value == Box {
				nv := x + 100*y
				answer += nv
...
func MakeWarehouseString(warehouse [][]*Node[byte], robot *Point) string {
	bz := MapGrid(warehouse, (*Node[byte]).GetValue)
	var pz []*Point
	if robot != nil && IsIn(bz, robot) {
		bz[robot.Y][robot.X] = Robot
		pz = append(pz, robot)
	}
	return CreateIndexedGridStringBz(bz, pz, pz)
}
...
func MoveRobot(move byte, robot *Point, warehouse [][]*Node[byte]) *Point {
	d := GetD(move)
	next := AddPoints(robot, d)
	w := Get(warehouse, next).Value
	switch w {
	case Wall:
		return robot
	case Open:
		return next
	case Box:
		if MoveBox(d, Get(warehouse, next), warehouse) {
			return next
		}
		return robot
	}
	panic(fmt.Errorf("unknown floor type %c at %s", w, next))
}

func MoveBox(d *Point, box *Node[byte], warehouse [][]*Node[byte]) bool {
	next := AddPoints(&box.Point, d)
	w := Get(warehouse, next)
	if w.Value == Open || (w.Value == Box && MoveBox(d, w, warehouse)) {
		w.Value = Box
		box.Value = Open
		return true
	}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		...
		case len(line) == 0:
			foundEmpty = true
			Debugf("empty line: %q", line)
		case foundEmpty:
			Debugf("moves line: %q", line)
			rv.Moves = append(rv.Moves, []byte(line)...)
		default:
			Debugf("grid line: %q", line)

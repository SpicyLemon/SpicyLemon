...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	head, tail := BuildDisk(input.DiskMap)
	if debug {
		Debugf("Initial Disk:\n%s", DiskString(head))
	}
	Compact(head, tail)
	...
	answer := Checksum(head)
	return fmt.Sprintf("%d", answer), nil
}

func DiskString(head *File) string {
	var lines []string
	i := 0
	for head != nil {
		lines = append(lines, fmt.Sprintf("[%d-%d]: %s", i, i+head.Length-1, head))
		i += head.Length
		head = head.Right
	}
...
func Compact(head, tail *File) {
	takeFrom := tail
	for takeFrom != nil {
		MoveLeft(head, takeFrom)
		takeFrom = takeFrom.Left
	}
}

func BuildDisk(dm []int) (*File, *File) {
	if len(dm) == 0 {
		return nil, nil
	}
	head := NewFile(0, dm[0])
	Debugf("head file: %s", head)
	id := 0
	cur := head
	for i := 1; i < len(dm); i++ {
		thisID := emptyID
		...
		cur = cur.Append(thisID, dm[i])
	}
	return head, cur
}

// MoveLeft will move the toMove file to the first empty location after head.
func MoveLeft(head, toMove *File) {
	defer FuncEnding(FuncStarting())
	if head == nil || toMove == nil || toMove.Length == 0 || toMove.ID == emptyID {
		Debugf("Nothing to move: %s, %s", head, toMove)
		return
	}
	i := -1
	for cur := head; cur != nil; cur = cur.Right {
		i++
		if cur == toMove {
			Debugf("[%d]: No spaces big enough to move %s", i, toMove)
			return
		}
		if cur.ID != emptyID || cur.Length < toMove.Length {
			continue
		}

		if cur.Length == toMove.Length {
			Debugf("found exact space: %s <= %s", cur, toMove)
			cur.ID = toMove.ID
			toMove.ID = emptyID
			return
		}

		// cur.Length > toMove.Length
		Debugf("found space thats bigger: %s <= %s", cur, toMove)
		newRight := cur.Split(toMove.Length)
		cur.ID = toMove.ID
		toMove.ID = emptyID
		Debugf("cur split into %s and %s, toMove is now %s", cur, newRight, toMove)
		return
	}
}
...
func NewFile(id, length int) *File {
	return &File{ID: id, Length: length}
}
...
func (f *File) String() string {
	...
	return fmt.Sprintf("%dx%d", f.ID, f.Length)
}

func (f *File) Checksum(first int) int {
	if f.ID == emptyID {
		return 0
	}
	return f.ID * f.Length * (first*2 + f.Length - 1) / 2
}

// Append will add f2 to this and return the next file to append to.
// If f1 and f2 have the same ids, this files length is updated and this is returned.
// Otherwise, f2 is linked after this and f2 is returned.
func (f *File) Append(id, length int) *File {
	if length == 0 {
		Debugf("length = 0, no change")
		return f
	}
	if f == nil {
		Debugf("this is nil, creating new unlinked file")
		return NewFile(id, length)
	}

	if f.ID == id {
		Debugf("this has same id, adding length")
		f.Length += length
		return f
	}

	Debugf("creating new file with id %d and length %d and adding it to the end", id, length)
	rv := NewFile(id, length)
	rv.Left = f
	rv.Right = f.Right
	if f.Right != nil {
		f.Right.Left = rv
...
// Split will shrink this file to the provided length and return a new file with the leftover length and same id.
// The returned file is linked correctly.
func (f *File) Split(newLen int) *File {
	if newLen <= 0 || newLen >= f.Length {
		panic(fmt.Errorf("cannot split file %s at length %d", f, newLen))
	}
	rightFile := NewFile(f.ID, f.Length-newLen)
	rightFile.Left = f
	rightFile.Right = f.Right
	if f.Right != nil {
		f.Right.Left = rightFile
	}
	f.Right = rightFile
	f.Length = newLen
	return rightFile
...
// CombineLeft will combine this file with the left file if they both have the same id.
func (f *File) CombineLeft() {
	...
	if f.Left.ID != f.ID {
		Debugf("left id is different, nothing to combine")
		return
	}

	Debugf("combining %s into %s", f.Left, f)
	f.Length += f.Left.Length
	if f.Left.Left != nil {
		f.Left.Left.Right = f
	}
	f.Left = f.Left.Left
...
// FillWith will fill the next available space from this File with the provided file.
// Returns the next File to put stuff in.
func (f *File) FillWith(f2 *File) *File {
	defer FuncEnding(FuncStarting())
	if f2 == nil {
		Debugf("f2 is nil")
		return f
	...
	if f2.ID == emptyID {
		Debugf("f2 is an empty block")
		return f
	}
	if f == f2 {
		Debugf("f2 is same as this, we're done.")
		return nil
	}

	cur := f
	Debugf("cur: %s", cur)
	for cur != nil && cur.ID != emptyID {
		cur = cur.Right
		Debugf("cur is filled, moving right: %s", cur)
	}
	if cur == nil {
		panic(fmt.Errorf("cannot fill %s: no space left", f2))
	}

	if cur.Length == f2.Length {
		Debugf("f2 %s fits exactly in cur %s", f2, cur)
		cur.ID = f2.ID
		f2.ID = emptyID
		return cur.Right
	}
	...
	if cur.Length > f2.Length {
		...
		f2.ID = emptyID
		cur.CombineLeft()
		Debugf("cur split into %s and %s", cur, newFile)
		return newFile
	}

	// cur.Length < f2.Length
	Debugf("f2 %s is larger than cur %s, splitting f2", f2, cur)
	cur.ID = f2.ID
	f3 := f2.Split(f2.Length - cur.Length)
	f3.ID = emptyID
	Debugf("cur now %s. f2 split into %s and %s, filling right with %s", cur, f2, f3, f2)
	return cur.Right.FillWith(f2)
}
...
func (i Input) String() string {
	...
	return strings.Join(MapSlice(i.DiskMap, strconv.Itoa), "")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	rv := Input{}
	if len(lines) != 1 {
		return &rv, fmt.Errorf("input can only have one line (%d): %q", len(lines), lines)
	}
	var err error
	rv.DiskMap, err = SplitParseIntsD(lines[0], "")
	if err != nil {
		return &rv, fmt.Errorf("could not split and parse input: %w", err)
	}
	return &rv, nil
}

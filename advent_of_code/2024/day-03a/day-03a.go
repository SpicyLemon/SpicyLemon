...
var mulRx = regexp.MustCompile(`mul\((\d\d?\d?),(\d\d?\d?)\)`)
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		...
		for j, match := range matches {
			Debugf("[%d]: %q => [%d]: %q", i, matches, j, match)
			l, err := strconv.Atoi(match[1])
			if err != nil {
				return &rv, fmt.Errorf("invalid first number in %q, %q: %w", match[0], match[1], err)
			}
			r, err := strconv.Atoi(match[2])
			if err != nil {
				return &rv, fmt.Errorf("invalid second number in %q, %q: %w", match[0], match[2], err)
			}

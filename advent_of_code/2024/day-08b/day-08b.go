...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	antinodes := GetAntinodes(params, input.Roof)
	if params.Verbose {
		params.Verbosef("Roof:\n%s", CreateIndexedGridStringBz(input.Roof, nil, antinodes))
	}
	answer := len(antinodes)
	return fmt.Sprintf("%d", answer), nil
}

func GetAntinodes(params *Params, roof [][]byte) []*Point {
	antennas := GetAntennas(roof)
	var rv []*Point
	known := make([][]*Point, len(roof))
	for y := range roof {
		known[y] = make([]*Point, len(roof[y]))
	}
	markPoint := func(p *Point) {
		if !IsIn(known, p) || known[p.Y][p.X] != nil {
			return
		}
		params.Verbosef("New Antinode: %s", p)
		known[p.Y][p.X] = p
		rv = append(rv, p)
	}
	markPoints := func(ps []*Point) {
		for _, p := range ps {
			markPoint(p)
		}
	}
	for _, points := range antennas {
		for i := 0; i < len(points)-1; i++ {
			for j := i + 1; j < len(points); j++ {
				Debugf("extending %s and %s", points[i], points[j])
				markPoints(ExtendPoints(known, points[i], points[j]))
			}
		}
	}
	return rv
...
// ExtendPoints starts at p1 and goes to p2, then goes the same distance beyond that.
func ExtendPoints[E any](known [][]E, p1, p2 *Point) []*Point {
	if p1.Equals(p2) {
		panic(fmt.Errorf("cannot extend equal points %s and %s", p1, p2))
	}
	rv := []*Point{p1, p2}
	d := SubPoints(p2, p1)
	next := AddPoints(p2, d)
...
func GetAntennas(roof [][]byte) map[byte][]*Point {
	rv := make(map[byte][]*Point)
	for y := range roof {
		for x := range roof[y] {
			b := roof[y][x]
			if IsAntenna(b) {
				rv[b] = append(rv[b], NewPoint(x, y))
			}
		}
	}
	return rv
...
func (i Input) String() string {
	...
	return CreateIndexedGridStringBz(i.Roof, []*Point(nil), nil)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{
		Roof: make([][]byte, len(lines)),
	}
...
func (p Point) Equals(p2 *Point) bool {
	return p2 != nil && p.X == p2.X && p.Y == p2.Y
}
...
// SubPoints returns a new point that is p1 - p2.

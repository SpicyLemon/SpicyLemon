...
// This solution has some extra parameter options.
//
// The --count option controls which exploration step to run.
//   --count 5  - Solution that doesn't require my attention.
//                This is also the default.
//             See Solution: go run day-14b-v2.go actual.input
//                       or: go run day-14b-v2.go actual.input -n 5
//   --count 4  - Output some statistical analysis on the counts of blocks.
//             See Solution: go run day-14b-v2.go actual.input -n 4 -v --lines 8258 8258
//              Investigate: go run day-14b-v2.go actual.input -n 4
//   --count 3  - Output all of the block counts.
//             See Solution: go run day-14b-v2.go actual.input -n 3 -v --lines 8256 8258
//              Investigate: go run day-14b-v2.go actual.input -n 3
//   --count 2  - Same as the original program.
//             See Solution: go run day-14b-v2.go actual.input -n 2 --lines 8000 8300 18 103
//              Investigate: go run day-14b-v2.go actual.input -n 2 --lines 0 10000 18 103
//   --count 1  - Same as --count 2, i.e. the original program.
//             See Solution: go run day-14b-v2.go actual.input -n 1 --lines 0 1000
//
// The --lines options are also used to provide various numbers.
//   Format: --lines <sMin> [<sMax> [<sKey> [<step>]]]
//   <sMin>  - The smallest seconds to consider.
//   <sMax>  - The largest seconds to consider.
//   <sKey>  - The number of seconds of a state of interest.
//   <step>  - The number of seconds to advance each step.
//   A second is only considered if  <second> = <sKey> + <step> * n  for any positive integer n.
//   The program will start at the smallest such value that is at least the <sMin>.
//   It will advance in <step> second intervals until the next would be more than <sMax>.
...
func Solve(params *Params) (string, error) {
	...
	switch params.Count {
	case 0, 5:
		answer := FindTree(input.Robots, cur, step, sMax)
		if answer > 0 {
			Stdoutf("After %d seconds:\n%s", answer, DrawGrid(input.Robots, answer))
		} else {
			Stdoutf("No solution found.")
		}
		return fmt.Sprintf("%d", answer), nil
	case 4:
		GoThroughStates4(params, input.Robots, cur, step, sMax)
	case 3:
		GoThroughStates3(params, input.Robots, cur, step, sMax)
	case 1, 2:
		GoThroughStates2(input.Robots, cur, step, sMax)
	default:
...
func FindTree(robots []*Robot, cur, step, sMax int) int {
...
func GoThroughStates4(params *Params, robots []*Robot, cur, step, sMax int) {
	MoveRobots(robots, cur)
	var allCounts [][5]int
	for cur <= sMax {
		counts := CountBlocks(robots)
		allCounts = append(allCounts, counts)
		params.Verbosef("After %d seconds: %d", cur, counts)
		cur += step
		if cur <= sMax {
			MoveRobots(robots, step)
		} else {
	...
	if debug {
		Stderrf("allCounts (%d):\n%s", len(allCounts), StringNumberJoinFunc(allCounts, CountsString, 0, "\n"))
	}
	if params.Verbose {
		Stderrf("After %d seconds:\n%s", cur, DrawGrid(robots, cur))
	}

	mins := make([]int, 5)
	maxs := make([]int, 5)
	totals := make([]int, 5)
	hists := make([]map[int]int, 5)
	for i, count := range allCounts {
		for j, c := range count {
			if i == 0 {
				mins[j] = c
				maxs[j] = c
				hists[j] = map[int]int{c: 1}
			} else {
				mins[j] = min(mins[j], c)
				maxs[j] = max(maxs[j], c)
				hists[j][c]++
	...
	for i, hist := range hists {
		Stdoutf("Histgram[%d]: %s", i, HistString(hist))
	}
}

func HistString(hist map[int]int) string {
	...
	slices.Reverse(keys)
	maxParts := 10
	parts := make([]string, min(maxParts+1, len(keys)))
	for i, k := range keys {
		if i >= maxParts {
			parts[i] = fmt.Sprintf("...(%d more)", len(keys)-maxParts)
			break
		}
		parts[i] = fmt.Sprintf("%dx%d", k, hist[k])
	}
	return "[" + strings.Join(parts, ",") + "]"
}

func CountsString(counts [5]int) string {
	return fmt.Sprintf("%d", counts)
}

func ScoreCounts(counts [5]int) int {
	return counts[2]*2 + counts[3]*3 + counts[4]*4
}

func GoThroughStates3(params *Params, robots []*Robot, cur, step, sMax int) {
...
func CountBlocks(robots []*Robot) [5]int {
	grid, _ := RobotGrid(robots)
	rv := [5]int{}
	for y := 0; y < len(grid)-1; y++ {
		for x := 0; x < len(grid[y])-1; x++ {
			count := CountIf(grid[y][x]) + CountIf(grid[y+1][x]) + CountIf(grid[y][x+1]) + CountIf(grid[y+1][x+1])
			rv[count]++
		}
	}
	return rv
...
// CountIf returns 1 if val > 1, or 0 if val is zero (or negative).
func CountIf(val int) int {
	if val > 0 {
		return 1
	}
	return 0
...
func GoThroughStates2(robots []*Robot, cur, step, sMax int) {
	stdin := bufio.NewReader(os.Stdin)
	MoveRobots(robots, cur)
	for cur <= sMax {
		Stdoutf("After %d seconds:\n%s"+
...
func RobotGrid(robots []*Robot) ([][]int, []*Point) {
	grid := make([][]int, Height)
	for y := range grid {
		grid[y] = make([]int, Width)
	}
	rs := make([]*Point, len(robots))
...
func (r *Robot) String() string {
	...
	return fmt.Sprintf("%s+%s", r.P, r.V)
}

var robotRx = regexp.MustCompile(`^p=(-?\d+),(-?\d+) v=(-?\d+),(-?\d+)$`)

func ParseRobot(line string) (*Robot, error) {
	parts := robotRx.FindStringSubmatch(line)
	if len(parts) != 5 {
		return nil, fmt.Errorf("invalid robot line %q: format does not match regex", line)
	}
	ints := make([]int, 4)
	for i, part := range parts[1:] {
		val, err := strconv.Atoi(part)
		if err != nil {
			return nil, fmt.Errorf("invalid robot line %q: [%d] invalid number %q: %w", line, i+1, part, err)
		}
		ints[i] = val
	}
	return &Robot{P: NewPoint(ints[0], ints[1]), V: NewPoint(ints[2], ints[3])}, nil
}
...
func (i Input) String() string {
	...
	return StringNumberJoin(i.Robots, 1, "\n")
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Robots: make([]*Robot, len(lines))}
	var err error
	for i, line := range lines {
		rv.Robots[i], err = ParseRobot(line)
		if err != nil {
			return &rv, nil
		}
	}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := CountXMAS(params, input.Chars)
	return fmt.Sprintf("%d", answer), nil
}

var xmasRx = regexp.MustCompile(`XMAS`)

type xformer struct {
	name   string
	runner func([][]byte) []string
}

var xformers = []xformer{
	{name: "Left To Right", runner: GetStringsLR},
	{name: "Right To Left", runner: GetStringsRL},
	{name: "Top Down", runner: GetStringsTD},
	{name: "Bottom Up", runner: GetStringsBU},
	{name: "Diagonal Down and Right", runner: GetStringsDDR},
	{name: "Diagonal Up and Right", runner: GetStringsDUR},
	{name: "Diagonal Up and Left", runner: GetStringsDUL},
	{name: "Diagonal Down and Left", runner: GetStringsDDL},
}

func CountXMAS(params *Params, chars [][]byte) int {
	rv := 0
	for i, f := range xformers {
		lines := f.runner(chars)
		Debugf("[%d] %s:\n%s\n%s\n", i, f.name, strings.Join(lines, "\n"), strings.Repeat("=", 20))
		for j, line := range lines {
			matches := xmasRx.FindAllString(line, -1)
			count := len(matches)
			params.Verbosef("[%d][%d]: %d <= %q", i, j, count, line)
			rv += count
		}
	}
	return rv
...
func DebugPrintAllAngles(chars [][]byte) {
	if !debug {
		return
	}
	for _, f := range xformers {
		Debugf("%s:\n%s\n%s\n", f.name, strings.Join(f.runner(chars), "\n"), strings.Repeat("=", 20))
	}
}

// GetStringsLR get strings of the line as read from Left to Right.
func GetStringsLR(chars [][]byte) []string {
	rv := make([]string, len(chars))
	for y := range chars {
		rv[y] = string(chars[y])
	}
	return rv
...
// GetStringsRL get strings of the line as read from Right to Left.
func GetStringsRL(chars [][]byte) []string {
	return ReverseStrings(GetStringsLR(chars))
}

// GetStringsTD get strings of the line as read from the Top to Down.
func GetStringsTD(chars [][]byte) []string {
	rv := make([]string, len(chars[0]))
	for y := range chars {
		for x := range chars[y] {
			rv[x] += string(chars[y][x])
		}
	}
	return rv
}

// GetStringsBU get strings of the line as read from the Bottom to Up.
func GetStringsBU(chars [][]byte) []string {
	return ReverseStrings(GetStringsTD(chars))
}

// GetStringsDDR get strings of the line as read from the Diagonal Down and Right.
func GetStringsDDR(chars [][]byte) []string {
	rv := make([]string, 0, len(chars)+len(chars[0])-1)
	for yy := len(chars) - 1; yy >= 0; yy-- {
		x := 0
		y := yy
		line := ""
		for y < len(chars) && x < len(chars[y]) {
			line += string(chars[y][x])
			x++
			y++
		}
		rv = append(rv, line)
	}
	for xx := 1; xx < len(chars[0]); xx++ {
		y := 0
		x := xx
		line := ""
		for y < len(chars) && x < len(chars[y]) {
...
// GetStringsDUL get strings of the line as read from the Diagonal Up and Left.
func GetStringsDUL(chars [][]byte) []string {
	return ReverseStrings(GetStringsDDR(chars))
}

// GetStringsDUR get strings of the line as read from the Diagonal Up and Right.
func GetStringsDUR(chars [][]byte) []string {
	rv := make([]string, 0, len(chars)+len(chars[0])-1)
	for yy := 0; yy < len(chars); yy++ {
		x := 0
		y := yy
		line := ""
		for y >= 0 && x < len(chars[y]) {
			line += string(chars[y][x])
			x++
			y--
		}
...
// GetStringsLR get strings of the line as read from the Diagonal Down and Left.
func GetStringsDDL(chars [][]byte) []string {
	return ReverseStrings(GetStringsDUR(chars))
}

// ReverseString reverses the bytes of a string.
...
// ReverseStrings reverses the bytes of each string provided (but the slice stays in the same order).
func ReverseStrings(strs []string) []string {
	rv := make([]string, len(strs))
	for y, str := range strs {
		rv[y] = ReverseString(str)
	}
	return rv
}
...
func (i Input) String() string {
	lines := make([]string, len(i.Chars))
	for l, line := range i.Chars {
		lines[l] = string(line)
	}

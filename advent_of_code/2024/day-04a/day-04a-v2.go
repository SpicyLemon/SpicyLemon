...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer := CountWord(params, input.Chars, "XMAS")
	return fmt.Sprintf("%d", answer), nil
}

// deltas are points representing each possible word direction.
var deltas = []*Point{
	NewPoint(0, 1),
	NewPoint(1, 1),
	NewPoint(1, 0),
...
// CountWord will count the number of times the given word appears in the provided word search grid.
func CountWord(params *Params, chars [][]byte, word string) int {
	rv := 0
	var xCells, xmasCells []*Point
	for y := range chars {
		for x := range chars[y] {
			if chars[y][x] != 'X' {
				continue
			...
			for _, d := range deltas {
				str, points := MakeWord(word, chars, x, y, d.X, d.Y)
				if str == word {
					rv++
					xCells = append(xCells, NewPoint(x, y))
					xmasCells = append(xmasCells, points...)
				}
			}
		}
	}
	...
	if params.Verbose {
		chars2 := MakeGrid(len(chars[0]), len(chars), byte('.'))
		for _, pt := range Join(xCells, xmasCells) {
			chars2[pt.Y][pt.X] = chars[pt.Y][pt.X]
		}
		params.Verbosef("Grid:\n%s", CreateIndexedGridStringBz(chars2, xmasCells, xCells))
	}
	return rv
}

// GetChar safely gets the character at (x,y) in the provided chars grid.
// Returns '.' if (x,y) is outside.
func GetChar(chars [][]byte, x, y int) byte {
...
func MakeWord(word string, chars [][]byte, x, y, dx, dy int) (string, []*Point) {
	if dx == 0 && dy == 0 {
		panic("Cannot call GetString with dx = 0 and dy = 0")
	}
	rv := make([]byte, 4)
	points := make([]*Point, 0, 4)
	for i := 0; i < 4; i++ {
		pt := NewPoint(x+dx*i, y+dy*i)
		c := GetChar(chars, pt.X, pt.Y)
		if word[i] != c {
			break
		}
		rv[i] = c
	...
	return string(rv), points
}

// Join will create a new slice by appending each slice together.
func Join[S ~[]E, E any](slices ...S) S {
	var rv S

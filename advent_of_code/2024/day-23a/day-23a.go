...
func Solve(params *Params) (string, error) {
	...
	for _, comp := range comps {
		if comp.HasT {
			newSets := FindLoops3(comp)
			Verbosef("Sets found from %q: %s", comp.Name, Connections(newSets))
			sets = append(sets, newSets...)
		}
	}
	Verbosef("Found %d sets (before de-duplication).", len(sets))
	sets = DeDupStrings(sets)
	answer := len(sets)
	Verbosef("Deduped: %s", Connections(sets))
...
func (c Connections) String() string {
	...
	for i, con := range c {
		x := i % 10
		if x == 0 {
			grid = append(grid, make([]string, 10))
			y = len(grid) - 1
		}
		grid[y][x] = " " + con
	}
...
func FindLoops3(comp1 *Computer) []string {
	var rv []string
	name1 := comp1.Name
	for name2, comp2 := range comp1.Connections {
		if name1 == name2 {
			continue
		}
		for name3, comp3 := range comp2.Connections {
			if name2 == name3 {
				continue
			}
			for name4 := range comp3.Connections {
				if name1 == name4 {
					rv = append(rv, CompsString(comp1, comp2, comp3))
				}
			}
		}
	}
...
func CompsString(comps ...*Computer) string {
	names := MapSlice(comps, (*Computer).GetName)
	slices.Sort(names)
	return strings.Join(names, "-")
}

func BuildNetwork(connections []*Connection) ([]*Computer, map[string]*Computer) {
	var rv []*Computer
	known := make(map[string]*Computer)

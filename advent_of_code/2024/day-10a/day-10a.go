...
func CountPathsToTop(heights [][]int) int {
	...
	if debug {
		Debugf("There are %d heads: %s", len(heads), strings.Join(MapSlice(heads, (*Node).PointString), "; "))
	}

	rv := 0
	for _, head := range heads {
		if debug {
			Debugf("Checking %s", head.FullString())
		}
		score := CountTops(head.FromTop)
		Debugf("head: %s has score %d", head, score)
		rv += score
	}
...
func CountTops(paths [][]*Node) int {
	have := make(map[int]map[int]bool)
	rv := 0
	for _, path := range paths {
		top := path[0]
		if _, ok := have[top.Y]; !ok {
			have[top.Y] = make(map[int]bool)
		}
		if !have[top.Y][top.X] {
			if debug {
				Debugf("Counting top %s from path %s", top.Point, pathString(path))
			}
			rv++
		}
		have[top.Y][top.X] = true
	}
	return rv
}

func BuildPaths(nodes [][]*Node) {
	nodesByHeight := make(map[int][]*Node)
	for y := range nodes {
		for _, node := range nodes[y] {
			nodesByHeight[node.Height] = append(nodesByHeight[node.Height], node)
		}
	}

	for height := 9; height >= 0; height-- {
		Debugf("There are %d nodes with height %d", len(nodesByHeight[height]), height)
		for _, node := range nodesByHeight[height] {
			switch {
			case height == 9:
				Debugf("top node: %s", node)
				node.FromTop = [][]*Node{{node}}
			case len(node.FromTop) == 0:
				Debugf("no paths from top: %s", node)
				continue
			default:
				Debugf("appending %s to its %d FromTop entries", node, len(node.FromTop))
				for p := range node.FromTop {
					node.FromTop[p] = append(node.FromTop[p], node)
				}
			}
			nextHeight := height - 1
			if node.Up != nil && node.Up.Height == nextHeight {
				node.Up.FromTop = append(node.Up.FromTop, node.FromTop...)
			}
			if node.Down != nil && node.Down.Height == nextHeight {
				node.Down.FromTop = append(node.Down.FromTop, node.FromTop...)
			}
			if node.Left != nil && node.Left.Height == nextHeight {
				node.Left.FromTop = append(node.Left.FromTop, node.FromTop...)
			}
			if node.Right != nil && node.Right.Height == nextHeight {
				node.Right.FromTop = append(node.Right.FromTop, node.FromTop...)
			}
		}
	}
}
...
func GetUDLR(p *Point) (*Point, *Point, *Point, *Point) {
	return p.Add(dUp), p.Add(dDown), p.Add(dLeft), p.Add(dRight)
}
...
func AsNodeMap(heights [][]int) [][]*Node {
	rv := make([][]*Node, len(heights))
	for y := range heights {
		rv[y] = make([]*Node, len(heights[y]))
		for x := range heights[y] {
			rv[y][x] = NewNode(x, y, heights[y][x])
		}
	}
...
func NewNode(x, y, height int) *Node {
	return &Node{Point: Point{X: x, Y: y}, Height: height}
}
...
func (n *Node) String() string {
	...
	return fmt.Sprintf("%s=%d:[%s](%d)", n.Point, n.Height, dirs, len(n.FromTop))
}

func (n *Node) FullString() string {
	return n.String() + ":\n" + StringNumberJoinFunc(n.FromTop, pathString, 1, "\n")
}

func (n *Node) PointString() string {
	if n == nil {
		return "<nil>"
	}
...
func pathString(path []*Node) string {
	return "[" + StringNumberJoinFunc(path, (*Node).PointString, 0, ",") + "]"
}
...
func (i Input) String() string {
	...
	return CreateIndexedGridStringNums(i.Heights, []*Point(nil), nil)
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		rv.Heights[i], err = SplitParseIntsD(line, "")
		if err != nil {

...
func Solve(params *Params) (string, error) {
	...
	bananaHist := make(map[string]int)
	priceHists := make([]map[string]int, len(input.Nums))
	for i, num := range input.Nums {
		ph := BuildPriceHist(num, 2000)
		for k, v := range ph {
			bananaHist[k] += v
		}
		Verbosef("%8d: (%d)", num, len(ph))
		priceHists[i] = ph
	}

	var best string
	var most int
	for seq, bananas := range bananaHist {
		if bananas > most {
			most = bananas
			best = seq
		}
	...
	Stdoutf("Sequence: %s", best)
	if verbose {
		for i, num := range input.Nums {
			count := priceHists[i][best]
			Stderrf("%8d: %d", num, count)
		}
	}
	...
	if len(params.Custom) > 0 {
		for _, line := range params.Custom {
			...
			seq, err := SplitParseIntsD(line, ",")
			if err != nil {
				return "", fmt.Errorf("invalid custom line %q: %w", line, err)
			}
			for i, num := range input.Nums {
				count, _ := FindSeq(num, 2000, seq)
				Stderrf("[%4d]: %8d: %d | %s = %d", i, num, count, line, priceHists[i][line])
			}
		}
		return "Stopping early because of custom input.", nil
	}
...
// FindSeq finds the given sequence in the evolution of the num and returns the price found and how many iterations in.
func FindSeq(num, count int, seq []int) (int, int) {
	// Dammit. I made this because the example wasn't working and I couldn't figure out why.
	// It's because the example changed from part 1 to part 2 and I was using the old one.
	cur := make([]int, 0, 5)
	lastPrice := Price(num)
	for i := 0; i < count; i++ {
		num = Evolve(num)
...
func SeqKey(vals []int) string {
	return strings.Join(MapSlice(vals, strconv.Itoa), ",")
}

func BuildPriceHist(num int, count int) map[string]int {
	rv := make(map[string]int)
	cur := make([]int, 0, 5)
	lastPrice := Price(num)
	Debugf("%8d: %d", num, lastPrice)
	for i := 0; i < count; i++ {
		num = Evolve(num)
		price := Price(num)
		d := price - lastPrice
...
func Price(num int) int {
	return num % 10
}

func EvolveFor(num, count int) int {
	for i := 0; i < count; i++ {
		num = Evolve(num)
	}
	return num
...
func Evolve(num int) int {
	// 16777216 = 2 ^ 11. So doing % 16777216 is the same masking on 16777215.
	num = ((num * 64) ^ num) & 16777215
	num = ((num / 32) ^ num) & 16777215
	return ((num * 2048) ^ num) & 16777215
}
...
func (i Input) String() string {
	...
	return fmt.Sprintf("Nums (%d):\n%s", len(i.Nums), StringNumberJoinFunc(i.Nums, IntString, 1, "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Nums: make([]int, len(lines))}
	var err error
	for i, line := range lines {
		rv.Nums[i], err = strconv.Atoi(line)
		if err != nil {
			return &rv, fmt.Errorf("invalid line %q: %w", line, err)
		}
	}
	return &rv, nil
}

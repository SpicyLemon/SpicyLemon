...
func Solve(params *Params) (string, error) {
	...
	answer := CountLoops(params, input.Grid, guard)
	return fmt.Sprintf("%d", answer), nil
}

func CountLoops(params *Params, grid [][]byte, guard *Point) int {
	normalPath := CopyGrid(grid)
	TrackGuard(normalPath, guard)
	possibles := FindChar(normalPath, X)
	count := len(possibles)
	params.Verbosef("Checking %d possible new obstical points.", count)

	rv := 0
	for i, obst := range possibles {
		newGrid := CopyGrid(grid)
		isLoop := TrackGuardWithObstacle(newGrid, guard, obst)
		if debug {
			pz := []*Point{obst}
			Debugf("[%d/%d]: %s = %t\n%s", i+1, count, obst, isLoop, CreateIndexedGridStringBz(newGrid, pz, pz))
		}
		if isLoop {
			rv++
			params.Verbosef("%s = loop", obst)
		}
	}
	return rv
}

func FindGuard(grid [][]byte) (*Point, error) {
	rvs := FindChar(grid, '^')
	if len(rvs) == 1 {
		return rvs[0], nil
	}
	...
	return nil, fmt.Errorf("multiple (%d) guards (^) found", len(rvs))
}

func FindChar(grid [][]byte, char byte) []*Point {
	var rv []*Point
...
func CountChar(grid [][]byte, char byte) int {
	rv := 0
	for y := range len(grid) {
		for x := range len(grid[y]) {
			if grid[y][x] == char {
...
var dirs = []*Point{NewPoint(0, -1), NewPoint(1, 0), NewPoint(0, 1), NewPoint(-1, 0)}

var dirNames = []byte{'u', 'r', 'd', 'l'}

func IsIn[E any](grid [][]E, p *Point) bool {
	return p.Y >= 0 && p.Y < len(grid) && p.X >= 0 && p.X < len(grid[0])
}

func IsOpen(grid [][]byte, p *Point) bool {
	return !IsIn(grid, p) || grid[p.Y][p.X] != '#'
}

func NewTrackingGrid(base [][]byte) [][]map[byte]bool {
	if base == nil {
		return nil
	}
	rv := make([][]map[byte]bool, len(base))
	for y := range base {
		rv[y] = make([]map[byte]bool, len(base[y]))
	}
	return rv
}

func TrackGuardWithObstacle(grid [][]byte, start *Point, obst *Point) bool {
	if !IsIn(grid, obst) || grid[obst.Y][obst.X] != '.' {
		return false
	}
	d := 0
	curDir := func() *Point {
	...
	grid[obst.Y][obst.X] = '#'
	cur := start
	tracker := NewTrackingGrid(grid)
	for IsIn(tracker, cur) {
		dirName := curDirName()
		if tracker[cur.Y][cur.X] != nil && tracker[cur.Y][cur.X][dirName] {
			return true
		}
		grid[cur.Y][cur.X] = X
		if tracker[cur.Y][cur.X] == nil {
			tracker[cur.Y][cur.X] = make(map[byte]bool)
		}
		tracker[cur.Y][cur.X][dirName] = true

		next := AddPoints(cur, curDir())
		if IsOpen(grid, next) {
			Debugf("Next = %s = open", next)
			cur = next
		} else {
			Debugf("Next = %s = obstacle. Turning right.", next)
			turnRight()
		}
	}
...
func TrackGuard(grid [][]byte, start *Point) {
	d := 0
	curDir := func() *Point {
		return dirs[d]
	}
	...
	isIn := func(p *Point) bool {
		return p.Y >= 0 && p.Y < len(grid) && p.X >= 0 && p.X < len(grid[0])
	}
	isOpen := func(p *Point) bool {
		return !isIn(p) || grid[p.Y][p.X] != '#'
	}

	cur := start
	Debugf("Starting at %s", cur)
	for isIn(cur) {
		grid[cur.Y][cur.X] = X
		next := AddPoints(cur, curDir())
		if isOpen(next) {
...
// AddPoints creates a new point that is the sum of all the provided points.
...
// CreateIndexedGridStringBz creates a string of the provided vals bytes matrix.
...
// CreateIndexedGridStringNums creates a string of the provided vals bytes matrix.
// The result will have row and column indexes and the desired cells will be colored and/or highlighted.

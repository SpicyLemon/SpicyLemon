...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	var safe, unsafe [][]int
	for i, report := range input.Reports {
		ok := IsSafe(report)
		params.Verbosef("[%d](%t): %v", i, ok, report)
		if ok {
	...
	for i, report := range unsafe {
		ok := false
		for j := range report {
			newRep := RemInd(report, j)
			okj := IsSafe(newRep)
			params.Verbosef("[%d][%d](%t): %v <= %v", i, j, okj, newRep, report)
			if okj {
				ok = true
				break
			}
	...
	return fmt.Sprintf("%d", len(safe)), nil
}
...
func RemInd(vals []int, ind int) []int {
	if ind >= len(vals) {
		panic(fmt.Errorf("cannot remove index %d from slice of length %d: %v", ind, len(vals), vals))
	}
	rv := make([]int, 0, len(vals)-1)
...
func (i Input) String() string {
	lines := make([]string, len(i.Reports))
	for i, rep := range i.Reports {
		lines[i] = fmt.Sprintf("[%d](%d): %v", i, len(rep), rep)
	}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		rv.Reports[i], err = SplitParseInts(line)
		if err != nil {
			return &rv, fmt.Errorf("invalid input line %d: %w", i, err)
		}
	}
	return &rv, nil
}

...
func Solve(params *Params) (string, error) {
	...
	cheats := FindCheats(path, track)
	if verbose || params.InputFile == DEFAULT_INPUT_FILE {
...
func FindCheats(path []*Node[int], track [][]*Node[int]) map[int][]*Cheat {
	rv := make(map[int][]*Cheat)
	for _, node := range path {
		for _, next := range GetCheatPoints(node, track) {
			Debugf("Node: %s   Next: %s", node, next)
			saved := next.Value - node.Value - 2
			if saved < 2 {
				continue
			}
...
var DCheats = []*Point{
	NewPoint(0, -2), NewPoint(-1, -1), NewPoint(-2, 0), NewPoint(-1, 1),
	NewPoint(0, 2), NewPoint(1, 1), NewPoint(2, 0), NewPoint(1, -1),
}

...
func Solve(params *Params) (string, error) {
	...
	availableRx, err := CreateAvailableRx(input.Available)
	if err != nil {
		return "", err
	}

	answer := 0
	for i, desired := range input.Desired {
		ok := availableRx.MatchString(desired)
		if ok {
			count := CountPossible(input.Available, desired)
			answer += count
			params.Verbosef("[%d]: %d => %q", i, count, desired)
		} else {
			params.Verbosef("[%d]: %t => %q", i, ok, desired)
		}
	}
	return fmt.Sprintf("%d", answer), nil
}

var known = map[string]int{"": 0}

func CountPossible(available []string, desired string) int {
	if rv, ok := known[desired]; ok {
		return rv
	}
	var prefixes []string
	for _, pattern := range available {
		if strings.HasPrefix(desired, pattern) {
			prefixes = append(prefixes, pattern)
		}
	}
	rv := 0
	for _, pre := range prefixes {
		theRest := strings.TrimPrefix(desired, pre)
		if len(theRest) == 0 {
			rv++
		} else {
			rv += CountPossible(available, theRest)
		}
	}
	known[desired] = rv
	return rv
...
func CreateAvailableRx(available []string) (*regexp.Regexp, error) {
	str := `^(` + strings.Join(available, "|") + `)+$`
	rv, err := regexp.Compile(str)
	if err != nil {
		return nil, fmt.Errorf("could not compile regex %q: %w", str, err)
	}
	return rv, nil
}
...
func (i Input) String() string {
	...
	return fmt.Sprintf("Available (%d): %q\nDesired (%d):\n%s",
		len(i.Available), i.Available, len(i.Desired), NumberJoin(i.Desired, 1, "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	if len(lines) < 3 {
		return nil, fmt.Errorf("invalid line count %d, must be at least 3", len(lines))
	}
	if len(lines[1]) != 0 {
		return nil, fmt.Errorf("invalid second line %q, must be empty", lines[1])
	}
	rv := Input{
...
func NumberJoin(vals []string, startAt int, sep string) string {
	return strings.Join(AddLineNumbers(vals, startAt), sep)
}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	good := FindValidEq(params, input.Equations)
	answer := 0
	for _, eq := range good {
		answer += eq.Total
	}
...
func FindValidEq(params *Params, eqs []*Eq) []*Eq {
	var rv []*Eq
	for i, eq := range eqs {
		ok := IsValidEq(eq.Total, eq.Nums)
		params.Verbosef("[%d]: %t <= %s", i, ok, eq)
		if ok {
			rv = append(rv, eq)
		}
	}
...
func IsValidEq(total int, nums []int) bool {
	if total < 0 {
		return false
	}
	switch len(nums) {
	...
	case 1:
		return total == nums[0]
	case 2:
		return total == nums[0]+nums[1] || total == nums[0]*nums[1] || total == ConcatNums(nums[0], nums[1])
	}
	i := len(nums) - 1
	if total%nums[i] == 0 && IsValidEq(total/nums[i], nums[:i]) {
		return true
	}
	if IsValidEq(total-nums[i], nums[:i]) {
		return true
	}
	newTotal, ok := TrimSuffix(total, nums[i])
	return ok && IsValidEq(newTotal, nums[:i])
}
...
func ConcatNums(a, b int) int {
	...
	if err != nil {
		panic(fmt.Errorf("concatenated number %d %d = %q could not be converted to int: %w", a, b, str, err))
	}
	return rv
}
...
func TrimSuffix(a, b int) (int, bool) {
	sa := strconv.Itoa(a)
	sb := strconv.Itoa(b)
	srv := strings.TrimSuffix(sa, sb)
	if srv == sa {
		return 0, false
	}
	rv, err := strconv.Atoi(srv)
	if err != nil {
		return 0, false
	}
	return rv, true
...
func (i Input) String() string {
	...
	return StringNumberJoin(i.Equations, 1, "\n")
}
...
func ParseEq(line string) (*Eq, error) {
	parts := strings.Split(line, ":")
	if len(parts) != 2 {
		return nil, fmt.Errorf("unknown line format %q: has %d parts", line, len(parts))
	}
	total, err := strconv.Atoi(parts[0])
	if err != nil {
		return nil, fmt.Errorf("unknown line format %q: cannot parse total %q: %w", line, parts[0], err)
	}
	rv := &Eq{Total: total}
	rv.Nums, err = SplitParseInts(strings.TrimSpace(parts[1]))
	if err != nil {
		return nil, fmt.Errorf("unknown line format %q: cannot parse numbers %q: %w", line, parts[1], err)
	}
	return rv, nil
}

func (e Eq) String() string {
	return fmt.Sprintf("%d: %d", e.Total, e.Nums)
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{
		Equations: make([]*Eq, len(lines)),
	}
	var err error
	for i, line := range lines {
		rv.Equations[i], err = ParseEq(line)
		if err != nil {

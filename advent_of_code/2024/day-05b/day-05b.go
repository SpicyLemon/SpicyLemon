...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	ctx := NewContext(params, input)
	updates := GetUnorderedUpdates(ctx)
	if params.Verbose {
		params.Verbosef("Bad Updates:\n%s", updatesString(updates))
	}
	fixed := FixUpdates(ctx, updates)
	answer := 0
	for _, update := range fixed {
...
type Context struct {
	Params        *Params
	Input         *Input
	WrongOrderMap map[int]map[int]bool
	RightOrderMap map[int]map[int]bool
}

func NewContext(params *Params, input *Input) *Context {
	rv := &Context{
		Params:        params,
		Input:         input,
		WrongOrderMap: ToWrongOrderMap(input.Orders),
		RightOrderMap: ToRightOrderMap(input.Orders),
	}
	if params.Verbose {
...
func GetUnorderedUpdates(ctx *Context) [][]int {
	var rv [][]int
	for u, update := range ctx.Input.Updates {
		wrong := FindWrongOrder(ctx, update)
		ctx.Params.Verbosef("[%d]: %s = %v", u, wrong, update)
		if wrong != nil {
			rv = append(rv, update)
		}
	}
...
func FindWrongOrder(ctx *Context, updates []int) *Point {
	for i := range updates {
		nope, known := ctx.WrongOrderMap[updates[i]]
		if !known {
			continue
		}
...
// ToWrongOrderMap creates a map from the orders such that rv[X][Y] = true when Y cannot be before X.
func ToWrongOrderMap(orders []*Point) map[int]map[int]bool {
	rv := make(map[int]map[int]bool)
	for _, order := range orders {
		if _, have := rv[order.Y]; !have {
			rv[order.Y] = make(map[int]bool)
		}
		rv[order.Y][order.X] = true
	}
	return rv
}

// ToRightOrderMap creates a map from the orders such that rv[X][Y] = true when X must come before Y.
func ToRightOrderMap(orders []*Point) map[int]map[int]bool {
	rv := make(map[int]map[int]bool)
	for _, order := range orders {
		if _, have := rv[order.X]; !have {
			rv[order.X] = make(map[int]bool)
		}
		rv[order.X][order.Y] = true
	}
	return rv
}

func orderMapString(m map[int]map[int]bool) string {
	pages := getKeys(m)
	lines := make([]string, len(pages))
	for p, page := range pages {
		lines[p] = fmt.Sprintf("[%d] %d: %v", p, page, getKeys(m[page]))
	}
	return strings.Join(lines, "\n")
}

func getKeys[M map[int]E, E any](m M) []int {
	rv := make([]int, 0, len(m))
	for key := range m {
		rv = append(rv, key)
	}
...
func updatesString(updates [][]int) string {
	return strings.Join(AddLineNumbers(MapSlice(updates, intSliceToString), 1), "\n")
}

func (i Input) String() string {
	orders := StringNumberJoin(i.Orders, 1, "\n")
	updates := updatesString(i.Updates)
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		...
		if inOrders {
			parts, err := SplitParseInts(line, "|")
			if err != nil {
				return &rv, fmt.Errorf("unknown order format [%d] %q: %w", i, line, err)
			}
			if len(parts) != 2 {
				return &rv, fmt.Errorf("unknown order format [%d] %q: parsed to %v", i, line, parts)
			}
			rv.Orders = append(rv.Orders, NewPoint(parts[0], parts[1]))
		} else {
			parts, err := SplitParseInts(line, ",")
			if err != nil {
				return &rv, fmt.Errorf("unknown update format [%d] %q: %w", i, line, err)
			}
			rv.Updates = append(rv.Updates, parts)
		}
	}
	return &rv, nil
}
...
// SplitParseInts splits a string on the provided delimiter and converts each part into an int.
...
func SplitParseInts(s string, d string) ([]int, error) {
	rv := []int{}

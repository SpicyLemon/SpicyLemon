...
func FasterSolve(input *Input) int {
	...
	if debug {
		for _, sum := range slices.Sorted(maps.Keys(locksBySum)) {
			locks := locksBySum[sum]
			Debugf("Locks with sum %d:\n%s", sum, StringNumberJoinFunc(locks, IntsString, 1, "\n"))
		}
		for _, sum := range slices.Sorted(maps.Keys(keysBySum)) {
			keys := keysBySum[sum]
			Debugf("Keys with sum %d:\n%s", sum, StringNumberJoinFunc(keys, IntsString, 1, "\n"))
		}
	}

	answer := 0
	for sum, locks := range locksBySum {
		keys := keysBySum[25-sum]
		if len(keys) == 0 {
			continue
		}
		for _, lock := range locks {
			Debugf("Checking lock: %d", lock)
			for _, key := range keys {
				Debugf("     with key: %d", key)
				if IsMatch(lock, key) {
					Debugf("Found match!")
					answer++
					break
...
func IsMatch(lock, key []int) bool {
	for i := range lock {
		if lock[i]+key[i] > 5 {
			return false
		}
...
func SumHist(vals [][]int) map[int][][]int {
	rv := make(map[int][][]int)
	for _, nums := range vals {
		sum := SumInts(nums)
		rv[sum] = append(rv[sum], nums)
	}
	return rv
}
...
func (i Input) String() string {
	...
	return fmt.Sprintf("Locks (%d):\n%s\nKeys (%d):\n%s",
		len(i.Locks), StringNumberJoinFunc(i.Locks, IntsString, 1, "\n"),
		len(i.Keys), StringNumberJoinFunc(i.Keys, IntsString, 1, "\n"),
	)
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for _, line := range lines {
		...
		if len(cur) == 7 {
			...
			if cur[0][0] == '.' {
				rv.Keys = append(rv.Keys, counts)
			} else {
				rv.Locks = append(rv.Locks, counts)
			}
			cur = make([][]byte, 0, 7)
		}
	}

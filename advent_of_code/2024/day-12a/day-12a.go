...
func CalcPrice(plots []*Node[byte]) int {
	...
	for _, plot := range plots {
		perim += oneIf(plot.Up == nil || plot.Value != plot.Up.Value) +
			oneIf(plot.Down == nil || plot.Value != plot.Down.Value) +
			oneIf(plot.Left == nil || plot.Value != plot.Left.Value) +
			oneIf(plot.Right == nil || plot.Value != plot.Right.Value)
	}
	rv := area * perim
	Debugf("%c: %d <= %d * %d", plots[0].Value, rv, area, perim)
	return rv
}

// oneIf returns 1 if test is true or 0 if not.
func oneIf(test bool) int {
	if test {
		return 1
	}
	return 0

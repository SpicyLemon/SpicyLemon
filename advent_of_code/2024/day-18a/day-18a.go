...
func Solve(params *Params) (string, error) {
	...
	params.Verbosef("Taking first %d points", count)

	points := input.Points
	if count > len(points) {
		return "", fmt.Errorf("invalid count %d with only %d points", count, len(points))
	}
	if count < len(points) {
		points = points[:count]
	}
	...
	grid, err := NewGrid(size, points)
	if err != nil {
		return "", err
	}
	if params.Verbose {
		Stderrf("Grid:\n%s", CreateIndexedGridString(MapGrid(grid, GetNodeValue), []*Point(nil), nil))
	}

	bestPath := FindPath(grid)
	answer := len(bestPath) - 1
	if params.Verbose {
		Stderrf("Best Path (%d):\n%s", len(bestPath), StringNumberJoin(bestPath, 0, " "))
		Stderrf("Grid:\n%s", CreateIndexedGridString(MapGrid(grid, GetNodeValue), nil, bestPath))
	}
...
func FindPath(grid [][]*Node[Cell]) []*Point {
	...
	Stderrf("No path found after checking %d of %d open spots.", checked, totalOpen)
	return nil
}

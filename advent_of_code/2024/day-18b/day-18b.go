...
func Solve(params *Params) (string, error) {
	...
	// Add one to the size since it's from 0 to <size> which is size + 1 wide.
	size++
	params.Verbosef("Grid size: %d x %d", size, size)

	points := input.Points
	Debugf("Points (%d):\n%s", len(points), points)

	answer, path, grid := FindCutoff(size, points)
	if params.Verbose {
		strs := MapGrid(grid, GetNodeValue)
		for _, p := range path {
			strs[p.Y][p.X] = "O"
		}
		badCount := 0
		for y := range strs {
			for x := range strs[y] {
				if strs[y][x] == string(Bad) {
					badCount++
				}
			}
		}
		Stderrf("Grid after %d drops:\n%s", badCount, CreateIndexedGridString(strs, path, path))
	}
	return fmt.Sprintf("%d,%d", answer.X, answer.Y), nil
}

func FindCutoff(size int, badSpots []*Point) (*Point, []*Point, [][]*Node[Cell]) {
	grid, err := NewGrid(size, badSpots)
	if err != nil {
		panic(fmt.Errorf("error creating new grid: %w", err))
	}
	for i := len(badSpots) - 1; i >= 0; i-- {
		path := FindPath(grid)
		if len(path) > 0 {
			return badSpots[i+1], path, grid
		}
		ResetGrid(grid)
		nowGood := Get(grid, badSpots[i])
		nowGood.Value.Value = Open
		nowGood.Next = make(map[Direction]*Node[Cell])
		for dir, next := range GetAdjacent(grid, nowGood) {
			if next.Value.Value == Open {
				next.Next[DirOpposites[dir]] = nowGood
				nowGood.Next[dir] = next
			}
		}
	}
	return nil, nil, grid
...
func ResetGrid(grid [][]*Node[Cell]) {
	for y := range grid {
		for x := range grid[y] {
			grid[y][x].Value.Visited = false
			grid[y][x].Value.Queued = false
			grid[y][x].Value.PathTo = nil
		}
	}
}

func FindPath(grid [][]*Node[Cell]) []*Point {
	queue := make([]*Node[Cell], 0, len(grid)*2) // Will probably grow a bit, but at least I tried.
	enqueue := func(node *Node[Cell]) {
		Debugf("Adding to queue: %s", node)
		if node.Value.Queued || node.Value.Visited {
			return
		}
	...
	Debugf("Start: %s", start)
	Debugf("  End: %s", end)
	enqueue(Get(grid, start))
	...
	for len(queue) > 0 {
		...
		nextPath := CopyAppend(cur.Value.PathTo, cur)

		for nextDir, nextNode := range cur.Next {
			Debugf("  [%c] Checking next = %s", nextDir, nextNode)
			if nextNode.Value.Visited {
				Debugf("      Next already visited.")
				continue
			}

			if !nextNode.Value.Queued || len(nextPath) < len(nextNode.Value.PathTo) {
				Debugf("      Updating the path to, now with length %d.", len(nextPath))
				nextNode.Value.PathTo = nextPath
			}
			if IsSameXY(nextNode, end) {
				Debugf("       Next is the end %s!", end)
				return AsPoints(CopyAppend(nextPath, end))
			}
			enqueue(nextNode)
		}
	}

	Debugf("No path found after checking %d of %d open spots.", checked, totalOpen)
	return nil
}
...
func (c Cell) String() string {
	return fmt.Sprintf("%c[%s%s](%d)", c.Value, Ternary(c.Visited, "V", "_"), Ternary(c.Queued, "Q", "_"), c.Cost())
}

// CompareCells returns -1 if a < b, 0 if a == b, or 1 if a > b.
func CompareCells(a, b *Node[Cell]) int {
	...
	return CmpInts(a.Point.X, b.Point.X)
}
...
func NewGrid(size int, points []*Point) ([][]*Node[Cell], error) {
	grid := make([][]*Node[Cell], size)
	for y := range grid {
		grid[y] = make([]*Node[Cell], size)
		for x := range grid[y] {
			grid[y][x] = NewCellNode(x, y, Open)
		}
	}
	LinkNodes(grid)
	Debugf("After linking the nodes: %s", grid[0][0])
	for _, p := range points {
		node, ok := GetB(grid, p)
		if !ok {
			return nil, fmt.Errorf("point out of range %s on grid with size %d", p, size)
		}
		node.Value.Value = Bad
		node.Unlink()
	}
	return grid, nil
}

func NewCellNode(x, y int, value byte) *Node[Cell] {
	return NewNode(x, y, Cell{Value: value})
}

func GetNodeValue(n *Node[Cell]) string {
	if n == nil {
		return " "
	}
	return string(n.Value.Value)
}
...
func PointsString(points []*Point) string {
	return strings.Join(SliceToStrings(points), " ")
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		vals, err := SplitParseIntsD(line, ",")
		if err != nil {
			return &rv, fmt.Errorf("[%d]: could not parse line %q: %w", i, line, err)
		}
		if len(vals) != 2 {
			return &rv, fmt.Errorf("[%d]: invalid amount of numbers. Expected 2: %q", i, line)
		}
		rv.Points[i] = NewPoint(vals[0], vals[1])
	}
	return &rv, nil
}

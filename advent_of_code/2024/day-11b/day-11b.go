...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	stones := BuildStoneMap(input.Vals)
	if params.Verbose {
		params.Verbosef("Initial: %s", stones)
	}
	for i := 1; i <= params.Count; i++ {
		stones = BlinkAtStones(stones)
		switch {
		case debug || i <= 6 && params.Verbose:
			Stderrf("blink %d: %s", i, stones)
		case params.Verbose:
			Stderrf("blink %d: (%d)", i, CountStones(stones))
		}
	}
	answer := CountStones(stones)
...
func (s StoneMap) String() string {
	...
	for i, val := range vals {
		count += s[val]
		parts[i] = fmt.Sprintf("%d(%d)", val, s[val])
	}
	return fmt.Sprintf("(%d): ", count) + strings.Join(parts, " ")
}

func BuildStoneMap(vals []int) StoneMap {
	rv := make(StoneMap)
...
func CountStones(stones StoneMap) int {
	rv := 0
...
func BlinkAtStones(stones StoneMap) StoneMap {
	rv := make(StoneMap)
	for value, count := range stones {
		lVal, rVal := BlinkValue(value)
		rv[lVal] += count
		if rVal != NoVal {
			rv[rVal] += count
		}
...
func BlinkValue(value int) (int, int) {
	if value == 0 {
		return 1, NoVal
	}
	...
	if len(valStr)%2 != 0 {
		return value * 2024, NoVal
	}

	pivot := len(valStr) / 2
	lValStr := valStr[:pivot]
	rValStr := valStr[pivot:]

	lVal, err := strconv.Atoi(lValStr)
	if err != nil {
		panic(fmt.Errorf("failed to split %q into %q* and %q: invalid left value: %w", valStr, lValStr, rValStr, err))
	}
	rVal, err := strconv.Atoi(rValStr)
	if err != nil {
		panic(fmt.Errorf("failed to split %q into %q and %q*: invalid right value: %w", valStr, lValStr, rValStr, err))
	}
	return lVal, rVal
}

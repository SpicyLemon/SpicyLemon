...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	updates := GetOrderedUpdates(params, input)
	if params.Verbose {
		params.Verbosef("Good Updates:\n%s", updatesString(updates))
	}
	answer := 0
...
func GetOrderedUpdates(params *Params, input *Input) [][]int {
	var rv [][]int
	wrongOrderMap := ToWrongOrderMap(input.Orders)

	for u, update := range input.Updates {
		wrongOrder := FindWrongOrder(wrongOrderMap, update)
		params.Verbosef("[%d]: %s = %v", u, wrongOrder, update)
		if wrongOrder == nil {
			rv = append(rv, update)
		}
	}
...
func FindWrongOrder(wrongOrderMap map[int]map[int]bool, updates []int) *Point {
	for i := range updates {
		nope, known := wrongOrderMap[updates[i]]
		if !known {
			continue
		}
		for j := i + 1; j < len(updates); j++ {
			if nope[updates[j]] {
				return NewPoint(updates[j], updates[i])
			}
		}
	}
	return nil

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	maxCost, grid := FindSmallestPath(input.Start, input.End, input.Maze)
	if params.Verbose {
		end := Get(grid, input.End)
		var path []*Point
		if end != nil {
			allPaths := MapSlice(end.Value.PathsTo, func(bz []byte) string { return fmt.Sprintf("(%d): %s", len(bz), string(bz)) })
			params.Verbosef("End Cell: %s\nPaths to (%d):\n%s", end, len(allPaths), strings.Join(AddLineNumbers(allPaths, 1), "\n"))
	...
	bestPaths := FindAllCheapPaths(maxCost, input.Start, input.End, grid, nil)
	switch {
	case debug:
		Stderrf("Found %d paths with cost %d or less:\n%s", len(bestPaths), maxCost, StringNumberJoin(bestPaths, 1, "\n"))
	case params.Verbose:
		Stderrf("Found %d paths with cost %d or less.", len(bestPaths), maxCost)
	}

...
func (i Input) String() string {
	return fmt.Sprintf("(%d): %v", len(i.Nums), i.Nums)
}

var mulRx = regexp.MustCompile(`(mul|do|don't)\(((\d\d?\d?),(\d\d?\d?))?\)`)
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		matches := mulRx.FindAllStringSubmatch(line, -1)
		Debugf("[%d]: %q", i, line)
		Debugf("[%d]: Matches: %q", i, matches)
		for j, match := range matches {
			Debugf("[%d][%d]: %q", i, j, match)
			switch match[1] {
			case "do":
				if len(match[2]) == 0 {
					on = true
			...
			case "mul":
				if on {
					...
					if err != nil {
						return &rv, fmt.Errorf("invalid first number in %q, %q: %w", match[0], match[3], err)
					}
					r, err := strconv.Atoi(match[4])
					if err != nil {
						return &rv, fmt.Errorf("invalid second number in %q, %q: %w", match[0], match[4], err)
					}
					rv.Nums = append(rv.Nums, []int{l, r})
				}
			default:
				panic(fmt.Errorf("unknown match: %q", match))
			}
		}
	}
	return &rv, nil

...
func Solve(params *Params) (string, error) {
	...
	botBottom := NewRobot("Bottom", NumPad, nil)
	botDir1 := NewRobot("Dir 1", DirPad, botBottom)
	botDir2 := NewRobot("Dir 2", DirPad, botDir1)
	botTop := NewRobot("Top", DirPad, botDir2)

	if len(params.Custom) > 0 {
		for _, line := range params.Custom {
			result := EnterSequence(botDir1, Sequence(line))
			Stderrf("%s  =>  %s", line, result)
			botTop.Reset()
		}
	...
	for _, code := range input.Codes {
		result := botDir2.EnterCode(code.Keys)
		v1 := len(result)
		v2 := code.Value
		a := v1 * v2
...
func NewKeypad(name string, layout [][]byte, buttons []byte, points map[byte]*Point, paths map[byte]map[byte][]Sequence) *Keypad {
	return &Keypad{
		Name:    name,
		Layout:  layout,
		Buttons: buttons,
...
func (k *Keypad) String() string {
	...
	return fmt.Sprintf("%s Layout:\n%s\nButtons: %s\nPoints: %s\nPaths:\n%s", k.Name, layout, buttons, points, strings.Join(paths, "\n"))
}
...
func InitStuff() {
	if debug {
		Stderrf("Direction Pad:\n%s", CreateIndexedGridStringFunc(DirPadLayout, GetButtonName, []*Point(nil), nil))
	}
	DirPadButtons = make([]byte, 0, 5)
	DirPadPoints = make(map[byte]*Point)
	for y := range DirPadLayout {
		for x, v := range DirPadLayout[y] {
			if v == Empty {
				continue
			}
			DirPadButtons = append(DirPadButtons, v)
			DirPadPoints[v] = NewPoint(x, y)
		}
	}
	if debug {
		Stderrf("Direction Pad Buttons: %s", strings.Join(MapSlice(DirPadButtons, GetButtonName), " "))
		Stderrf("Direction Pad Points: %s", PointMapString(DirPadPoints))
	}

	// Validate the DirPadPaths stuff.
	ValidatePaths("DirPadPaths", DirPadPaths, DirPadPoints)
	DirPad = NewKeypad("Direction Pad", DirPadLayout, DirPadButtons, DirPadPoints, DirPadPaths)

	if debug {
		Debugf("Number Pad:\n%s", CreateIndexedGridStringFunc(NumPadLayout, GetButtonName, []*Point(nil), nil))
	}
	NumPadButtons = make([]byte, 0, 11)
	NumPadPoints = make(map[byte]*Point)
	for y := range NumPadLayout {
		for x, v := range NumPadLayout[y] {
			if v == Empty {
				continue
			}
			NumPadButtons = append(NumPadButtons, v)
			NumPadPoints[v] = NewPoint(x, y)
		}
	}
	slices.Sort(NumPadButtons)
	if debug {
		Stderrf("Number Pad Buttons: %s", strings.Join(MapSlice(NumPadButtons, GetButtonName), " "))
		Stderrf("Number Pad Points: %s", PointMapString(NumPadPoints))
	}

	NumPadPaths = make(map[byte]map[byte][]Sequence)
	for _, from := range NumPadButtons {
		NumPadPaths[from] = make(map[byte][]Sequence)
		fromP := NumPadPoints[from]
		for _, to := range NumPadButtons {
			toP := NumPadPoints[to]
			d := toP.Sub(fromP)
			Debugf("Finding sequences from %c %s to %c %s, d = %s", from, fromP, to, toP, d)
			seqs := DeltaToSequences(toP.Sub(fromP))
			seqs = FilterIllegal(seqs, fromP, NumPadLayout)
			seqs = ReduceSequences(seqs)
			NumPadPaths[from][to] = seqs
		}
	}
	ValidatePaths("NumPadPaths", NumPadPaths, NumPadPoints)
	NumPad = NewKeypad("Number Pad", NumPadLayout, NumPadButtons, NumPadPoints, NumPadPaths)
}
...
seqLoop:
	for _, seq := range seqs {
		...
		for i, step := range seq {
			...
			if !isIn {
				Debugf("Omitting: From %s, sequence %s goes out of bounds at step %d = %s", from, seq, i+1, cur)
				continue seqLoop
			}
			if v == Empty {
...
func ReduceSequences(seqs []Sequence) []Sequence {
	...
	for _, dir := range Dirs {
		...
		for s, seq := range byDir[dir] {
			...
			dist := len(seq2)
			Debugf("%c[%d] seq %s (%d) expands to %s (%d)", dir, s, seq, len(seq), seq2, dist)
		...
		if debug {
			Stderrf("%c histogram:\n%s", dir, HistMapString(hist))
			Stderrf("Keeping only; %s", hist[best][0])
		}
...
func ExpandDirSequence(cur byte, seq Sequence) Sequence {
	...
	if known[curKey] != nil && known[curKey][seqKey] != nil {
		return known[cur][seqKey]
	}

	var rv Sequence
	for _, step := range seq {
		var best Sequence
		for _, next := range DirPad.Paths[cur][step] {
			if next[0] == cur {
				best = next
				break
			}
			if len(best) == 0 || len(best) < len(next) {
				best = next
			}
		}
		rv = append(rv, best...)

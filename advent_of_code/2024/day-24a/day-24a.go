...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	wireMap, err := BuildCircuit(input.Wires, input.Gates)
	if err != nil {
	...
	for name, wire := range wireMap {
		if strings.HasPrefix(name, "z") {
			keyWires = append(keyWires, wire)
		}
	}
	slices.SortFunc(keyWires, CompareWires)
	PropagateSignals(input.Gates, keyWires)
	if verbose {
		Stderrf("Key wires (%d):\n%s", len(keyWires), StringNumberJoin(keyWires, 1, "\n"))
	}
	zVals := strings.Join(MapSlice(keyWires, (*Wire).GetValue), "")
	Verbosef("z wires: %q", zVals)
	answer, err := strconv.ParseInt(zVals, 2, 64)
	if err != nil {
		return "", fmt.Errorf("could not convert %q into integer: %w", zVals, err)
	}
...
func BuildCircuit(wires []*Wire, gates []*Gate) (map[string]*Wire, error) {
	...
	for _, gate := range gates {
		...
		if gate.Out.Source != nil {
			return nil, fmt.Errorf("could not wire up %s: wire out already has a destination", gate)
		}
...
func ParseWire(line string) (*Wire, error) {
	...
	return &Wire{Name: parts[0], Value: parts[1]}, nil
}
...
func (g *Gate) String() string {
	return fmt.Sprintf("%s %s %s %q => %s", g.In1Name, g.Op, g.In2Name, g.Value, g.OutName)
}

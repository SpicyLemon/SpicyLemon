...
func Solve(params *Params) (string, error) {
	...
	if len(params.Custom) > 0 {
		Width, err = strconv.Atoi(params.Custom[0])
		if err != nil {
			return "", fmt.Errorf("invalid custom width %q: %w", params.Custom[0], err)
		}
	}
	if len(params.Custom) > 1 {
		Height, err = strconv.Atoi(params.Custom[1])
		if err != nil {
			return "", fmt.Errorf("invalid custom height %q: %w", params.Custom[1], err)
		}
	}
	params.Verbosef("Width=%d", Width)
	params.Verbosef("Height=%d", Height)
	Center = NewPoint(Width/2, Height/2)
	params.Verbosef("Center=%s", Center)
	movedRobots := make([]*Robot, len(input.Robots))
	quadrants := make([]int, 5)
	for i, robot := range input.Robots {
		movedRobots[i] = robot.Move(params.Count)
		q := GetQuadrant(movedRobots[i].P)
		quadrants[q]++
		params.Verbosef("%s moved to %s and is now in quadrant %d", robot, movedRobots[i], q)
	}
	if params.Verbose {
		Stderrf("Grid:\n%s", DrawGrid(movedRobots))
	}
	params.Verbosef("quadrants: %d", quadrants)
...
func DrawGrid(robots []*Robot) string {
	...
	centerPoints := make([]*Point, Height, Height+Width)
	for y := range grid {
		grid[y] = make([]int, Width)
		centerPoints[y] = NewPoint(Center.X, y)
	}
	for x := 0; x <= Width; x++ {
		centerPoints = append(centerPoints, NewPoint(x, Center.Y))
	}
	...
	return CreateIndexedGridStringNums(grid, nil, centerPoints)
}

// GetQuadrant returns 1 for upper left, 2 for upper right, 3 for lower left, 4 for lower right, 0 for a middle line.
...
// GetXSide returns -1 if x is in the left side, 0 if in the middle, 1 if on the right side.
func GetXSide(x int) int {
	switch {
	case x < Center.X:
		return -1
	case x == Center.X:
...
// GetYSide returns -1 if x is in the top half, 0 if in the middle, 1 if in the bottom half.
func GetYSide(y int) int {
	switch {
	case y < Center.Y:
		return -1
	case y == Center.Y:
...
func (r *Robot) Move(seconds int) *Robot {
	x := (r.P.X + r.V.X*seconds) % Width
	if x < 0 {
		x += Width
	}
	y := (r.P.Y + r.V.Y*seconds) % Height
	if y < 0 {
		y += Height
	}
	return &Robot{P: NewPoint(x, y), V: r.V}
}

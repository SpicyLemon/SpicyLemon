...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	answer1, answer2 := 0, 0
	goodNone, goodBoth, good1, good2 := 0, 0, 0, 0
	for i, m1 := range input.Machines {
		cost1, ok1 := GetCost(m1)
		if !ok1 {
			params.Verbosef("[%d]1: %s = no solution", i, m1)
		} else {
			params.Verbosef("[%d]1: %s = %d", i, m1, cost1)
			answer1 += cost1
		}

		m2 := Part2Machine(m1)
		cost2, ok2 := GetCost(m2)
		if !ok2 {
			params.Verbosef("[%d]2: %s = no solution", i, m2)
		} else {
			params.Verbosef("[%d]2: %s = %d", i, m2, cost2)
			answer2 += cost2
		}
		switch {
		case !ok1 && !ok2:
	...
	Stderrf("Lose both: %d", goodNone)
	Stderrf("Win both : %d", goodBoth)
	Stderrf("Win part1: %d", good1)
	Stderrf("Win part2: %d", good2)
	return fmt.Sprintf("%d and %d", answer1, answer2), nil
}

var dPrize = NewPoint(10000000000000, 10000000000000)

func Part2Machine(m *Machine) *Machine {
	rv := &Machine{}
	if m.A != nil {
		rv.A = NewPoint(m.A.X, m.A.Y)
	}
	if m.B != nil {
		rv.B = NewPoint(m.B.X, m.B.Y)
	}
	if m.Prize != nil {
		rv.Prize = AddPoints(dPrize, m.Prize)
	}
	return rv
}
...
var (
	ButtonARx = regexp.MustCompile(`^Button A: X\+(\d+), Y\+(\d+)$`)
	ButtonBRx = regexp.MustCompile(`^Button B: X\+(\d+), Y\+(\d+)$`)
	PrizeRx   = regexp.MustCompile(`^Prize: X=(\d+), Y=(\d+)$`)
)

func ParseMachine(lines []string) (*Machine, error) {
	rv := &Machine{}
	errs := make([]error, 3)
	rv.A, errs[0] = ParseLine(lines[0], ButtonARx, "button A")
	rv.B, errs[1] = ParseLine(lines[1], ButtonBRx, "button B")
	rv.Prize, errs[2] = ParseLine(lines[2], PrizeRx, "prize")
	return rv, errors.Join(errs...)
}

func ParseLine(line string, rx *regexp.Regexp, label string) (*Point, error) {
	parts := rx.FindStringSubmatch(line)
	if len(parts) != 3 {
		return nil, fmt.Errorf("unknown %s format %q", label, line)
	}
	x, err := strconv.Atoi(parts[1])
	if err != nil {
		return nil, fmt.Errorf("unknown %s x value %q from %q: %w", label, parts[1], line, err)
	}
	y, err := strconv.Atoi(parts[2])
	if err != nil {
		return nil, fmt.Errorf("unknown %s y value %q from %q: %w", label, parts[2], line, err)
	}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	head, _ := BuildStones(input.Vals)
	if params.Verbose {
		params.Verbosef("Initial: %s", StonesString(head))
	}
	for i := 1; i <= params.Count; i++ {
		BlinkAtStones(head)
		switch {
		case debug || (i <= 6 && params.Verbose):
			Stderrf("blink %d: %s", i, StonesString(head))
		case params.Verbose:
			Stderrf("blink %d: %s", i, StonesString(head)[:80])
		}
	}
	answer := StonesLen(head)
	return fmt.Sprintf("%d", answer), nil
}

func BlinkAtStones(head *Stone) {
	cur := head
	for cur != nil {
		cur = cur.Blink()
	}
...
func StonesVals(head *Stone) []int {
	cur := head
	var rv []int
	for cur != nil {
		rv = append(rv, cur.Value)
		cur = cur.Right
	}
	return rv
}

func StonesString(head *Stone) string {
	vals := StonesVals(head)
	return fmt.Sprintf("(%d): %d", len(vals), vals)
}

func BuildStones(vals []int) (*Stone, *Stone) {
	var head, cur *Stone
...
func NewStone(value int) *Stone {
	return &Stone{Value: value, ValStr: strconv.Itoa(value)}
}

func (s *Stone) Append(value int) *Stone {
	rv := NewStone(value)
	s.Right = rv
	rv.Left = s
	return rv
...
func (s *Stone) Blink() *Stone {
	...
	case s.Value == 0:
		...
		s.ValStr = "1"
		return s.Right
	case len(s.ValStr)%2 == 0:
		pivot := len(s.ValStr) / 2
		lValStr := s.ValStr[:pivot]
		rValStr := s.ValStr[pivot:]

		lVal, err := strconv.Atoi(lValStr)
		if err != nil {
			panic(fmt.Errorf("failed to split %q into %q and %q: invalid left value: %w", s.ValStr, lValStr, rValStr, err))
		}
		rVal, err := strconv.Atoi(rValStr)
		if err != nil {
			panic(fmt.Errorf("failed to split %q into %q and %q: invalid left value: %w", s.ValStr, lValStr, rValStr, err))
		}

		r := NewStone(rVal) // using NewStone because rValStr might have leading zeros to remove.
		r.Left = s
		r.Right = s.Right
		if s.Right != nil {
			s.Right.Left = r
		...
		s.Right = r
		s.Value = lVal
		s.ValStr = lValStr // Since lValStr is the start of the original number, it shouldn't have any leading zeros.
		return r.Right
	default:
		s.Value *= 2024
		s.ValStr = strconv.Itoa(s.Value)
		return s.Right
	}
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	rv := Input{}
	if len(lines) != 1 {
		return &rv, fmt.Errorf("only one line can be provided, found %d: %q", len(lines), lines)
	}
	var err error
	rv.Vals, err = SplitParseInts(lines[0])
	if err != nil {
		return &rv, fmt.Errorf("could not parse line %q: %w", lines[0], err)
	}
	return &rv, nil
}

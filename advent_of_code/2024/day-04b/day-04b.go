...
func getChar(chars [][]byte, y, x int) byte {
...
func CountXMAS(params *Params, chars [][]byte) int {
	...
	for y := range chars {
		for x := range chars[y] {
			...
			ul := getChar(chars, y-1, x-1)
			ur := getChar(chars, y-1, x+1)
			lr := getChar(chars, y+1, x+1)
			ll := getChar(chars, y+1, x-1)
			var ms, ss int
			for _, c := range []byte{ul, ur, lr, ll} {
				switch c {
				case 'M':
					ms++
				case 'S':
			...
			params.Verbosef("X-MAS Center A: (%d,%d)", x+1, y+1)
			rv++
			aCells = append(aCells, NewPoint(x, y))
			xCells = append(xCells,
				NewPoint(x, y),
				NewPoint(x-1, y-1),
				NewPoint(x-1, y+1),
				NewPoint(x+1, y-1),
	...
	if params.Verbose {
		onlyXMAS := make([][]byte, len(chars))
		for y := range chars {
			onlyXMAS[y] = make([]byte, len(chars[y]))
			for x := range chars[y] {
				onlyXMAS[y][x] = '.'
			}
		}
		for _, p := range aCells {
			onlyXMAS[p.Y][p.X] = chars[p.Y][p.X]
		}
		for _, p := range xCells {
			onlyXMAS[p.Y][p.X] = chars[p.Y][p.X]
		}
		params.Verbosef("Field:\n%s", CreateIndexedGridStringBz(onlyXMAS, xCells, aCells))
	}
	return rv
}

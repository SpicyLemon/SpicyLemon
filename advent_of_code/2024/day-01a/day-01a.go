...
func Solve(params *Params) (string, error) {
	...
	answer := 0
	for i := range input.L {
		answer += Abs(input.L[i] - input.R[i])
	}
...
func (i Input) String() string {
	return fmt.Sprintf("{%v,%v}", i.L, i.R)
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	for i, line := range lines {
		...
		var err error
		rv.L[i], err = strconv.Atoi(parts[0])
		if err != nil {
			return &rv, fmt.Errorf("invalid left number %q from line %d=%q: %w", parts[0], i+1, line, err)
		}
		rv.R[i], err = strconv.Atoi(parts[1])
		if err != nil {
			return &rv, fmt.Errorf("invalid right number %q from line %d=%q: %w", parts[1], i+1, line, err)
		}
	}
	return &rv, nil
}

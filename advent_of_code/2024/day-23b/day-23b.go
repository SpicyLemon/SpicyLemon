...
func Solve(params *Params) (string, error) {
	...
	if len(params.Custom) > 0 {
		for _, line := range params.Custom {
			names := strings.Split(line, ",")
			if len(names) == 1 {
				names = strings.Split(line, "-")
			}
			if len(names) < 3 {
				return "", fmt.Errorf("at least 3 names are required, e.g. co,de,ka,ta")
			}
			cluster := NewCompMap()
			Stderrf("Initial cluster: %s", cluster)
			for i, name := range names {
				comp := compMap[name]
				if comp == nil {
					return "", fmt.Errorf("computer %q does not exist", name)
				}
				newCluster := cluster.AddComputer(comp)
				if newCluster == nil {
					Stderrf("[%d/%d]: Could not add %q to %s", i+1, len(names), comp, cluster)
				} else {
					cluster = newCluster
					Stderrf("[%d/%d]: Added %q: %s", i+1, len(names), comp, cluster)
				}
			}
		}
		return "Stopped early due to custom flags.", nil
	}

	clusters := make([]CompMap, len(input.Connections))
	for i, conn := range input.Connections {
		clusters[i] = NewCompMap(conn.AComp, conn.BComp)
	}

	allClusters := CombineAllClusters(clusters, comps)
	if verbose {
		lines := slices.Sorted(maps.Keys(allClusters))
		Stderrf("All Clusters (%d):\n%s", len(allClusters), Connections(lines))
		// Stderrf("All Clusters (%d):\n%s", len(allClusters), strings.Join(AddLineNumbers(lines, 1), "\n"))
	}

	var answer string
	for result := range allClusters {
...
func CombineAllClusters(clusters []CompMap, comps []*Computer) map[string]CompMap {
	...
	triedKey := func(key1, key2 string) string {
		parts := strings.Split(key1, ",")
		parts = append(parts, strings.Split(key2, ",")...)
		parts = DeDupStrings(parts)
		slices.Sort(parts)
		return strings.Join(parts, ",")
	}
	alreadyTried := func(key1, key2 string) bool {
		return tried[triedKey(key1, key2)]
	}
	markTried := func(key1, key2 string) {
		key := triedKey(key1, key2)
		tried[key] = true
		Debugf("Trying %s = %s + %s", key, key1, key2)
	}

	rv := make(map[string]CompMap)
	for _, cluster := range clusters {
		rv[cluster.String()] = cluster
	}
	...
	for keepGoing {
		keepGoing = false
		newComps := make([]*Computer, 0, len(comps))
		newClusts := make(map[string]CompMap)
		clustKeys := slices.Sorted(maps.Keys(toCheck))
		Verbosef("Looking for connections among %d clusters and %d computers, %d are known.", len(toCheck), len(comps), len(rv))
		for _, comp := range comps {
			keepComp := false
			for _, key := range clustKeys {
				if alreadyTried(key, comp.Name) {
					continue
				}
				markTried(key, comp.Name)

				newClust := toCheck[key].AddComputer(comp)
				if newClust != nil {
					cName := newClust.String()
					rv[cName] = newClust
					newClusts[cName] = newClust
					keepGoing = true
					keepComp = true
				}
			}
...
type CompMap map[string]*Computer

func NewCompMap(comps ...*Computer) CompMap {
	rv := make(CompMap)
	for _, comp := range comps {
		rv[comp.Name] = comp
	}
...
func (m CompMap) Slice() []*Computer {
	keys := slices.Sorted(maps.Keys(m))
	rv := make([]*Computer, len(keys))
...
func (m CompMap) String() string {
	return strings.Join(MapSlice(m.Slice(), (*Computer).GetName), ",")
}

func (m CompMap) AddComputer(comp *Computer) CompMap {
	if m[comp.Name] != nil {
		return nil
	}
	for name := range m {
...
func (m CompMap) Copy() CompMap {
	if m == nil {
		return nil
	}
	rv := make(CompMap, len(m))
	for k, v := range m {
		rv[k] = v
	}
	return rv
...
func (c Connections) String() string {
	...
	if len(c) <= width {
		return fmt.Sprintf("(%d): %s", len(c), strings.Join(c, "  "))
	}
	var grid [][]string
	y := 0
	for i, con := range c {
		x := i % width
		if x == 0 {
			grid = append(grid, make([]string, width))
			y = len(grid) - 1
		}
		grid[y][x] = " " + con
	}
	return fmt.Sprintf("(%d):\n", len(c)) + CreateIndexedGridString(grid, []*Point(nil), nil)
}
...
func BuildNetwork(connections []*Connection) ([]*Computer, CompMap) {
	var rv []*Computer
	known := make(CompMap)
	for i, conn := range connections {
		acomp := known[conn.A]
		if acomp == nil {
			acomp = NewComputer(conn.A)
			known[conn.A] = acomp
			rv = append(rv, acomp)
			Debugf("[%d]: Created computer %q: %s", i, conn.A, acomp)
		}
		bcomp := known[conn.B]
		if bcomp == nil {
			bcomp = NewComputer(conn.B)
			known[conn.B] = bcomp
			rv = append(rv, bcomp)
			Debugf("[%d]: Created computer %q: %s", i, conn.B, bcomp)
		}
		acomp.Connections[bcomp.Name] = bcomp
		bcomp.Connections[acomp.Name] = acomp
		conn.AComp = acomp
		conn.BComp = bcomp
		Debugf("[%d]: Made Connection %s: %s %s", i, conn, acomp, bcomp)
	}
	return rv, known
}
...
func NewComputer(name string) *Computer {
	return &Computer{Name: name, HasT: strings.HasPrefix(name, "t"), Connections: make(map[string]*Computer)}
}

func (c *Computer) WithConnection(other *Computer) *Computer {
	c.Connections[other.Name] = other
	return c
}

func (c *Computer) String() string {
	return fmt.Sprintf("%s%s -> [%s]", c.Name, Ternary(c.HasT, "*", " "), strings.Join(slices.Sorted(maps.Keys(c.Connections)), ","))
}
...
func ParseConnection(line string) *Connection {
	...
	return &Connection{A: parts[0], B: parts[1]}
}
...
func (i Input) String() string {
	...
	return fmt.Sprintf("Connections (%d):\n%s", len(i.Connections), StringNumberJoin(i.Connections, 1, "\n"))
}

func ParseInput(lines []string) (*Input, error) {
	defer FuncEnding(FuncStarting())
	rv := Input{Connections: MapSlice(lines, ParseConnection)}
	return &rv, nil
}

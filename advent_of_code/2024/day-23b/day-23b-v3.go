...
func Solve(params *Params) (string, error) {
	...
	seen := make(map[string]bool)        // Computer names that have been clustered already.
	for i, comp := range comps {
		if seen[comp.Name] {
			continue
		}
		newCluster := BuildLargestCluster(comp)
		if newCluster != nil {
			result := newCluster.String()
			clusters[result] = newCluster
			for _, name := range newCluster.Names() {
				seen[name] = true
			}
...
func BuildLargestCluster(comp1 *Computer) CompMap {
	name1 := comp1.Name
	known := []CompMap{NewCompMap(comp1)}
	for _, comp2 := range comp1.GetConnections() {
		name2 := comp2.Name
		if name1 == name2 {
			continue
		}
		added := false
		for i, cluster := range known {
			newCluster := cluster.AddComputer(comp2)
			if newCluster != nil {
				known[i] = newCluster
				added = true
				break
		...
		if !added {
			known = append(known, NewCompMap(comp1, comp2))
		}
	}

	var best []CompMap
	bestLen := 0
	for _, cluster := range known {
		switch {
		case len(cluster) > bestLen:
			bestLen = len(cluster)
			best = []CompMap{cluster}
		case len(cluster) == bestLen:
			best = append(best, cluster)
		}
	}

	// Assuming there's only one largest, if there's two largest here, we just return nil, it's not an answer.
	if len(best) != 1 {
		return nil
	}
	return best[0]
...
func (m CompMap) Slice() []*Computer {
	return slices.SortedFunc(maps.Values(m), CompareComputers)
}

func (m CompMap) Names() []string {
...
// AddComputer tries to add the provided computer to this cluster. Computers can
// only be added if they have a connection to all other computers in this cluster.
// If successful, a new cluster is returned with all of this clusters's computers
// as well as the one provided. Returns nil otherwise.
func (m CompMap) AddComputer(comp1 *Computer) CompMap {
	if m[comp1.Name] != nil {
		return nil
	}
	for name, comp2 := range m {
		if comp1.Connections[name] == nil || comp2.Connections[comp1.Name] == nil {
			return nil
		}
	}
	rv := m.Copy()
...
// Copy copies only this map. The values will be the same computer references.
...
func BuildNetwork(connections []*Connection) ([]*Computer, CompMap) {
	...
	slices.SortFunc(rv, CompareComputers)
	return rv, known
}

// CompareComputers returns -1 if a < b, 0 if a == b, or 1 if a > b.
func CompareComputers(a, b *Computer) int {
...
func (c *Computer) String() string {
	return fmt.Sprintf("%s -> [%s]", c.Name, strings.Join(slices.Sorted(maps.Keys(c.Connections)), ","))
}
...
func (c *Computer) GetConnections() []*Computer {
	return slices.SortedFunc(maps.Values(c.Connections), CompareComputers)
}
...
func ParseConnection(line string) *Connection {
	...
	// Not setting the computers here so that I only create one for each computer.
	// They'll get populated when building the network.

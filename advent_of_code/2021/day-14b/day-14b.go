...
func Solve(input Input) (string, error) {
	...
	template := NewTemplatePairs(input.Template)
	Debugf("Template:      %6d %s", template.Total(), template)
	for i := 1; i <= input.Count; i++ {
		template = ApplyPairs(template, input.Pairs)
		Debugf("After step %2d: %6d %s", i, template.Total(), template)
	}
	chars := map[byte]int{}
	...
	char1 := input.Template[0]
	charN := input.Template[len(input.Template)-1]
	for k := range chars {
		chars[k] = chars[k] / 2
		if k == char1 || k == charN {
			chars[k]++
	...
	byCount := NewStringInts(len(chars))
	for char, count := range chars {
		byCount = append(byCount, StringInt{string(char), count})
	}
	sort.Sort(StringIntSorterInt(byCount))
	Stdout("Character counts:\n%s", byCount)
	minPair := byCount[0]
	maxPair := byCount[len(byCount)-1]
	Stdout("Min: %s", minPair)
	Stdout("Max: %s", maxPair)
	answer := maxPair.I - minPair.I
	return fmt.Sprintf("%d", answer), nil
}

// Note:
// Let L = initial template length.
// Define function l(L, n) to be the template length after n steps starting with a template length of L.
// l(L, n) = (L-1)*2^n + 1
// l(4, 10)  =  3*2^10 + 1 =              3,073
// l(20, 10) = 19*2^10 + 1 =             19,457
// l(4, 40)  =  3*2^40 + 1 =  3,298,534,883,329
// l(20, 40) = 19*2^40 + 1 = 20,890,720,927,745
// If iterating of pairs in a string, total iterations for each step = length of previous template - 1
// Define function k(L, n) to be the number of iterations to finish n steps starting with a template length of L.
// k(L, n) = sum from m = 0 to n-1 of (l(L, m) - 1).
// k(L, n) = sum from m = 0 to n-1 of (L-1)*2^m
// k(L, n) = (L-1) * (sum from m = 0 to n-1 of 2^m)
// but sum from m = 0 to n-1 of 2^m = 2^n - 1
// k(L, n) = (L-1) * (2^n - 1)
// k(L, n) = (L-1)*2^n - L + 1
// k(L, n) = (L-1)*2^n + 1 - L
// k(L, n) = l(L, n) - L
// l(4, 10)  =  3*2^10 + 1 =              3,069
// l(20, 10) = 19*2^10 + 1 =             19,437
// l(4, 40)  =  3*2^40 + 1 =  3,298,534,883,329
// l(20, 40) = 19*2^40 + 1 = 20,890,720,927,725
// At one operation per 0.00000 00000 01 seconds, that's 20,890,720,927,725 * 0.00000 00000 01 seconds =~ 20.9 seconds.
// At one operation per 0.00000 00000 1 seconds, that's 20,890,720,927,725 * 0.00000 00000 1 seconds =~ 208.9 seconds = 3 minutes 28.9 seconds.
// At one operation per 0.00000 00001 seconds, that's 20,890,720,927,725 * 0.00000 00001 seconds =~ 2,089 seconds = 34 minutes 49 seconds.
// At one operation per 0.00000 0001 seconds, that's 20,890,720,927,725 * 0.00000 0001 seconds =~ 20,890 seconds = 5 hours 48 minutes 10 seconds.
// At one operation per 0.00000 001 seconds, that's 20,890,720,927,725 * 0.00000 001 seconds =~ 208,907 seconds = 2 days 10 hours 1 minute 47 seconds.
// A little analysis from day-14a.go makes it look like a decent ops per second estimate is 0.00000 006 =~ 14 days 12 hours 10 minutes 42 seconds.

type TemplatePairs map[string]int

func (t TemplatePairs) String() string {
	...
	for _, k := range keys {
		rv.WriteString(fmt.Sprintf("  %s: %d", k, t[k]))
	}
	return rv.String()
}

func NewTemplatePairs(template string) TemplatePairs {
	rv := TemplatePairs{}
...
func (t TemplatePairs) Total() int {
	rv := 0
	for _, v := range t {
		rv += v
	}
...
func ApplyPairs(template TemplatePairs, pairs map[string]string) TemplatePairs {
	defer FuncEndingAlways(FuncStarting(template.Total() + 1))
	rv := TemplatePairs{}
	for k, v := range template {
		if p, ok := pairs[k]; ok {
			rv[string([]byte{k[0], p[0]})] += v
			rv[string([]byte{p[0], k[1]})] += v
		}
	}
	return rv
}
...
func (r StringInt) String() string {
	return fmt.Sprintf("%s: %d", r.S, r.I)
}

type StringInts []StringInt

func NewStringInts(l int) StringInts {
	return make([]StringInt, 0, l)
}

func (r StringInts) String() string {
	var rv strings.Builder
	for _, p := range r {
		rv.WriteString(fmt.Sprintf("  %s\n", p))
	}
	return rv.String()
}

type StringIntSorterInt []StringInt

func (s StringIntSorterInt) Less(i, j int) bool {
	if s[i].I != s[j].I {
		return s[i].I < s[j].I
	}
	return s[i].S < s[j].S
}
func (s StringIntSorterInt) Swap(i, j int) { s[i], s[j] = s[j], s[i] }
func (s StringIntSorterInt) Len() int      { return len(s) }
...
func (i Input) String() string {
	lineFmt := DigitFormatForMax(len(i.Pairs)) + ": %s -> %s\n"
	var rv strings.Builder
	rv.WriteString(fmt.Sprintf("Polymer Template: %s\n", i.Template))
	l := 0
	for k, v := range i.Pairs {
		l++
		rv.WriteString(fmt.Sprintf(lineFmt, l, k, v))
	}
	rv.WriteString(fmt.Sprintf("Steps to do: %d\n", i.Count))
	return rv.String()
}
...
func ParseInput(fileData []byte) (Input, error) {
	...
	rv.Template = lines[0]
	rv.Pairs = map[string]string{}
	for _, line := range lines[2:] {
		if len(line) > 0 {
			parts := strings.Split(line, " -> ")
			rv.Pairs[parts[0]] = parts[1]
		}
	}
	return rv, nil
}

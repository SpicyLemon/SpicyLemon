...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	BoardSize = params.Count
	P1 := NewPlayer(1, input.P1Start)
	P2 := NewPlayer(2, input.P2Start)
	ddie := NewDDie(100)
	loser := PlayTheGame(1000, ddie, P1, P2)
	answer := loser.Score * ddie.Rolls
	return fmt.Sprintf("%d * %d = %d", loser.Score, ddie.Rolls, answer), nil
}

func PlayTheGame(targetScore int, ddie *DDie, P1, P2 *Player) *Player {
	isP1Turn := true
	for {
		if isP1Turn {
			if P1.Play(ddie) >= targetScore {
				return P2
			}
			isP1Turn = false
		} else {
			if P2.Play(ddie) >= targetScore {
				return P1
			}
			isP1Turn = true
		}
	}
	panic("execution should not get to the end of func PlayTheGame")
}
...
func (d DDie) String() string {
	return fmt.Sprintf("D%d: Rolled %d times. Last roll: %d.", d.Sides, d.Rolls, d.Last)
}

func (d *DDie) Roll3() int {
	d.Last += 3
	d.Rolls += 3
	return d.Last + d.Last - 1 + d.Last - 2
}

...
func Solve(params *Params) (string, error) {
	...
	burrow := input.Start
	answer := 0
	answer += BurrowMustMove(burrow, RoomSpot(2, 0), HallSpot(2))
	answer += BurrowMustMove(burrow, RoomSpot(3, 0), HallSpot(3))
	answer += BurrowMustMove(burrow, RoomSpot(3, 1), HallSpot(6))
	answer += BurrowMustMove(burrow, RoomSpot(2, 1), RoomSpot(3, 1))
	answer += BurrowMustMove(burrow, HallSpot(3), RoomSpot(2, 1))
	answer += BurrowMustMove(burrow, RoomSpot(1, 0), RoomSpot(2, 0))
	answer += BurrowMustMove(burrow, RoomSpot(1, 1), HallSpot(5))
	answer += BurrowMustMove(burrow, HallSpot(2), RoomSpot(1, 1))
	answer += BurrowMustMove(burrow, RoomSpot(0, 0), HallSpot(1))
	answer += BurrowMustMove(burrow, RoomSpot(0, 1), RoomSpot(3, 0))
	answer += BurrowMustMove(burrow, HallSpot(5), RoomSpot(0, 1))
	answer += BurrowMustMove(burrow, HallSpot(1), RoomSpot(0, 0))
	answer += BurrowMustMove(burrow, HallSpot(6), RoomSpot(1, 0))
	Stdout("Ending Burrow:\n%s", burrow)
...
func (b *Burrow) MakeMove(from, to Spot) (int, error) {
	...
	amph := b.Get(from)
	if amph == '.' {
		return MIN_INT, fmt.Errorf("cannot move from %s to %s: does not have an amphipod.", from, to)
	}
	if toB := b.Get(to); toB != '.' {
		return MIN_INT, fmt.Errorf("cannot move from %s to %s: it is already occupied by %c.", from, to, toB)
	}
	if !from.IsHall && !to.IsHall && from.Ind == to.Ind {
		b.Set(from, '.')
		b.Set(to, amph)
		return Cost[amph], nil
	}
	if !from.IsHall && from.Ind2 == 1 && b.Rooms[from.Ind][0] != '.' {
		return MIN_INT, fmt.Errorf("cannot move from %s to %s: there's something in the way above it.", from, to)
	}
	if !to.IsHall && to.Ind2 == 1 && b.Rooms[to.Ind][0] != '.' {
		return MIN_INT, fmt.Errorf("cannot move to %s to %s: there's something in the room opening.", from, to)
	}
	if !to.IsHall {
		reqRoom := 0
		switch amph {
		...
		if to.Ind != reqRoom {
			return MIN_INT, fmt.Errorf("cannot move from %s to %s: %c cannot enter room %d", from, to, amph, to.Ind)
		}
		if from.IsHall && to.Ind2 == 0 && b.Rooms[to.Ind][1] != '.' && b.Rooms[to.Ind][1] != amph {
			return MIN_INT, fmt.Errorf("cannot move from %s to %s: room is not yet empty.", from, to)
		}
	}
	var fromi, toi, di, steps int
	switch {
	case from.IsHall && to.IsHall:
		return MIN_INT, fmt.Errorf("cannot move from %s to %s: illegal hall-to-hall move.", from, to)
	case from.IsHall && !to.IsHall:
	...
	for i := fromi; test(i); i += di {
		if b.Hall[i] != '.' {
			return MIN_INT, fmt.Errorf("cannot move from %s to %s: there's a %c in the way at Hall[%d].", from, to, b.Hall[i], i)
		}
		steps += 2
	...
	if !to.IsHall && to.Ind2 == 1 {
		steps++
		// Debugf("rt Steps: %d", steps)
	}
	b.Set(from, '.')
	b.Set(to, amph)
	Debugf("Moving %c from %s to %s: %d * %d = %d", amph, from, to, Cost[amph], steps, Cost[amph]*steps)
...
func (b Burrow) String() string {
	return fmt.Sprintf("#############\n#%c%c.%c.%c.%c.%c%c#\n###%c#%c#%c#%c###\n  #%c#%c#%c#%c#  \n  #########  ",
		b.Hall[0], b.Hall[1], b.Hall[2], b.Hall[3], b.Hall[4], b.Hall[5], b.Hall[6],
		b.Rooms[0][0], b.Rooms[1][0], b.Rooms[2][0], b.Rooms[3][0],
		b.Rooms[0][1], b.Rooms[1][1], b.Rooms[2][1], b.Rooms[3][1])
}
...
func NewBurrow() *Burrow {
	...
	for i := range rv.Rooms {
		rv.Rooms[i] = make([]byte, 2)
		for j := range rv.Rooms[i] {
			rv.Rooms[i][j] = '.'
		}
	}
	for i := range rv.Hall {
...
func ParseInput(lines []string) (*Input, error) {
	rv := Input{}
	rv.Start = NewBurrow()
	l1 := strings.ReplaceAll(lines[2], "#", "")
	l2 := strings.ReplaceAll(strings.TrimSpace(lines[3]), "#", "")
	for i := 0; i < 4; i++ {
		rv.Start.Rooms[i][0] = l1[i]
		rv.Start.Rooms[i][1] = l2[i]
	}
	return &rv, nil
}

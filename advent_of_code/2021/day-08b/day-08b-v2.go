...
func Solve(input Input) (string, error) {
	...
	for _, note := range input.Notes {
		answer += ReadDisplay(SortSignals(note.Signals), note.Display)
	}
	return fmt.Sprintf("%d", answer), nil
}

// SortSignals sorts the signals into their numerical display order.
func SortSignals(signals []string) []string {
	...
	rv := make([]string, 10)
	// Start with the easy ones. '1' has 2 segments, '4' has 4, '7' has 3, and '8' has 7.
	rv[1] = byLen[2][0]
	rv[4] = byLen[4][0]
	rv[7] = byLen[3][0]
	rv[8] = byLen[7][0]
	// Still left: '2', '3', '5' have 5 segments, '0', '6', '9' have 6 segments.
	// Segments b and d are the ones in '4' but not '1'
	segBD := StrMinus(rv[4], rv[1])
	// '5' is the entry with length 5 that contains both b and d ('2' and '3' will only contain one of them).
	for _, digit := range byLen[5] {
		if strings.ContainsRune(digit, segBD[0]) && strings.ContainsRune(digit, segBD[1]) {
			rv[5] = digit
			break
		}
	}
	// Segment c is the one in '1' but not '5'
	segC := StrMinus(rv[1], rv[5])[0]
	// '6' is the entry with length 6 that does not contain segment c (both '0' and '9' do).
	for _, digit := range byLen[6] {
		if !strings.ContainsRune(digit, segC) {
			rv[6] = digit
			break
		}
	}
	// Still left: '2', '3' have 5 segments, '0', '9' have 6 segments.
	// '9' is the entry with length 6 that isn't '6' and has all the segments in '5' (i.e. '5' - '6' has no segments).
	for _, digit := range byLen[6] {
		if rv[6] != digit && len(StrMinus(rv[5], digit)) == 0 {
			rv[9] = digit
			break
		}
	}
	// '0' is the entry with length 6 that isn't '6' or '9'.
	for _, digit := range byLen[6] {
		if rv[6] != digit && rv[9] != digit {
			rv[0] = digit
			break
		}
	}
	// Still left: '2', '3' have 5 segments
	// Segment e is the one not in '9' (i.e. in '8' but not '9').
	segE := StrMinus(rv[8], rv[9])[0]
	// '2' is the entry of length 5 that has segment e ('5' (already known) and '3' do not).
	for _, digit := range byLen[5] {
		if strings.ContainsRune(digit, segE) {
			rv[2] = digit
		}
	}
	// '3' is the entry of length 5 that isn't '5' or '2'.
	for _, digit := range byLen[5] {
		if rv[2] != digit && rv[5] != digit {
			rv[3] = digit
		}
	}
	return rv

...
func solve(input Input) (string, error) {
	...
	grid := MakeGrid(input.Lines)
	debugf("Grid:\n%s", grid)
	answer := grid.CountGe(2)
...
func MakeGrid(lines []Line) Grid {
	...
	grid := Grid{
		Width:  maxx + 1,
		Height: maxy + 1,
		Vents:  make([][]int, maxx+1),
	}
	for x := range grid.Vents {
		grid.Vents[x] = make([]int, maxy+1)
	}
	for _, line := range lines {
		coords := line.ToCoords()
		debugf("Adding line: %s: %v", line, coords)
		for _, coord := range coords {
			debugf("Vent coord: %s", coord)
			grid.Vents[coord.X][coord.Y]++
		}
	}
	return grid
}

func (g Grid) CountGe(i int) int {
	rv := 0
	...
	for y := 0; y < g.Height; y++ {
		for x := 0; x < g.Width; x++ {
			if g.Vents[x][y] == 0 {
				...
				rv.WriteString(fmt.Sprintf("%d", g.Vents[x][y]))
			}
		}
		rv.WriteByte('\n')
	}
...
func (l Line) ToCoords() []Coord {
	rv := []Coord{}
	switch {
	case l.IsHorizontal():
		for _, y := range fromTo(l.Start.Y, l.End.Y) {
			rv = append(rv, Coord{l.Start.X, y})
		}
	case l.IsVertical():
		for _, x := range fromTo(l.Start.X, l.End.X) {
			rv = append(rv, Coord{x, l.Start.Y})
		}
	default:
		xs := fromTo(l.Start.X, l.End.X)
		ys := fromTo(l.Start.Y, l.End.Y)
		for i := 0; i < len(xs); i++ {
			rv = append(rv, Coord{xs[i], ys[i]})
		}
	}
	return rv
}

func fromTo(start, end int) []int {
	if start < end {
		rv := make([]int, end-start+1)
		for i := 0; i < len(rv); i++ {
			rv[i] = start + i
		}
		debugf("fromTo(%d, %d) + = %v", start, end, rv)
		return rv
	}
	rv := make([]int, start-end+1)
...
func parseInput(fileData []byte) (Input, error) {
	...
	for _, line := range lines {
		if len(line) > 0 {
			...
			if len(matches) == 0 {
				return rv, fmt.Errorf("unable to parse line: [%s]", line)
			}
			ints := make([]int, 4)
			...
			rv.Lines = append(rv.Lines, Line{Start: Coord{ints[0], ints[1]}, End: Coord{ints[2], ints[3]}})
		}
	}
	return rv, nil

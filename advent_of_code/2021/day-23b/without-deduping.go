...
func Solve(params *Params) (string, error) {
	...
	Debugf("Looking for this burrow:\n%s", CompletedBurrow)
	Debugf("Parsed Input:\n%s", input)
	solver := NewSolver(input.Start)
	i := 0
	outputSolverInfo := func(i int) {
		Stderr("Calculated %d moves. Solver: %s", i, solver)
		last := solver.Visited[len(solver.Visited)-1]
		Stderr("Last State:\n%s", last.Ele.State)
		Stderr("Last Move: %s", last)
		Stderr("Moves to get there:\n%s", GetMoves(last))
		Stderr("Moves considered from there:\n%s", GetNextMoves(last.Ele.State))
	}
	for !solver.IsDone() {
		i++
		solver.CalculateNext()
		if params.Verbose && i%50000 == 0 {
			Stderr("Calculated %d moves. Solver: %s", i, solver)
			outputSolverInfo(i)
		}
		if params.Count != 0 && i >= params.Count {
			break
		}
	}
	Stdout("After %d moves. Solver: %s", i, solver)
	outputSolverInfo(i)
	answer := -9999999999999
	if solver.Solution != nil {
		burrow := solver.Solution.Ele.State
		answer = solver.Solution.MinCost
		if params.Verbose {
			Stderr("Final Burrow:\n%s", burrow)
			Stderr("Moves:\n%s", GetMoves(solver.Solution))
		}
	}

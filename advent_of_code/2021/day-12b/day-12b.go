...
func Solve(input Input) (string, error) {
	...
	caves := NewCaveSystem(input.Paths)
	Debugf("Cave System:\n%s\n", caves)
	_ = CaveTree(caves)
	answer := 0
...
type CaveNameSorter []string

func (s CaveNameSorter) Less(i, j int) bool {
	switch s[i] {
	case "start":
		return true
	case "end":
...
func (s CaveNameSorter) Swap(i, j int) { s[i], s[j] = s[j], s[i] }
func (s CaveNameSorter) Len() int      { return len(s) }
...
func (n Node) String() string {
	pathNames := make([]string, len(n.Path))
	...
	return strings.Join(pathNames, "->") + end
}

func (n Node) PathNames() []string {
	rv := make([]string, len(n.Path))
	for i, c := range n.Path {
		rv[i] = c.Name
	}
	return rv
...
func (n Node) IsEnd() bool {
	return len(n.Path) > 0 && n.Path[len(n.Path)-1].Name == "end"
}

func (n Node) IsDeadEnd() bool {
	return len(n.Next) == 0 && !n.IsEnd()
}

func CaveTree(caves CaveSystem) *Node {
	return AddCaveTreeNodes([]*Cave{}, caves["start"])
}

func AddCaveTreeNodes(path []*Cave, next *Cave) *Node {
	rv := NewNode()
	rv.Path = append(rv.Path, path...)
	rv.Path = append(rv.Path, next)
	if next.Name == "end" {
		return rv
	}
	for _, cave := range next.ConnectsTo {
		if CanGoTo(rv.Path, cave) {
			nextNode := AddCaveTreeNodes(rv.Path, cave)
...
func CanGoTo(path []*Cave, next *Cave) bool {
	if next.Name == "start" {
		return false
	}
	if next.IsBig {
	...
	if !HasDoubleSmall(path) {
		return true
	}
	for _, cave := range path {
		if cave.Name == next.Name {
			return false
		}
	}
	return true
...
func HasDoubleSmall(path []*Cave) bool {
	for i := 0; i < len(path)-1; i++ {
		for j := i + 1; j < len(path); j++ {
			if !path[i].IsBig && !path[j].IsBig && path[i].Name == path[j].Name {
				return true
			}
		}
	}
...
func (s CaveSystem) String() string {
	keys := make([]string, 0, len(s)-2)
	for k := range s {
		keys = append(keys, k)
	}
	sort.Sort(CaveNameSorter(keys))
	var rv strings.Builder
	for _, k := range keys {
		rv.WriteString(s[k].String())
		rv.WriteByte('\n')
	}
	return rv.String()
}
...
func (c Cave) String() string {
	connects := make([]string, len(c.ConnectsTo))
	for i, p := range c.ConnectsTo {
		connects[i] = p.Name
	}
	sort.Sort(CaveNameSorter(connects))
	var rv strings.Builder
	rv.WriteString(fmt.Sprintf("%5s", c.Name))
	if c.IsBig {
		rv.WriteString("  (BIG)  -> ")
	} else {
		rv.WriteString(" (small) -> ")
	}
	rv.WriteString(strings.Join(connects, ", "))
	return rv.String()
}

func (c *Cave) AddConnection(cave *Cave) {
	for _, c := range c.ConnectsTo {
		if c.Name == cave.Name {
			return
		}
	}
	c.ConnectsTo = append(c.ConnectsTo, cave)
}

func NewCaveSystem(paths []Path) CaveSystem {
	rv := CaveSystem{}
	for _, path := range paths {
		leftCave, lcFound := rv[path.Left]
		if !lcFound {
			leftCave = NewCave(path.Left)
			rv[path.Left] = leftCave
		}
		rightCave, rcFound := rv[path.Right]
		if !rcFound {
			rightCave = NewCave(path.Right)
			rv[path.Right] = rightCave
		}
...
func (i Input) String() string {
	lineFmt := DigitFormatForMax(len(i.Paths)) + ": %s\n"
	var rv strings.Builder
	for i, v := range i.Paths {
...
func ParseInput(fileData []byte) (Input, error) {
	...
	for _, line := range lines {
		if len(line) > 0 {
			...
			if len(parts) != 2 {
				return rv, fmt.Errorf("unable to parse line: %q", line)
			}
			rv.Paths = append(rv.Paths, Path{parts[0], parts[1]})
		}
	}
	return rv, nil
}

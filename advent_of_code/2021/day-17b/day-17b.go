...
func Solve(input Input) (string, error) {
	...
	hits := GetVelocitiesThatHit(input.Target)
	if debug {
		notHits := Points{}
		for _, hit := range hits {
			probe := NewProbe(hit.X, hit.Y)
			if _, isHit := probe.FireAt(input.Target); !isHit {
				notHits = append(notHits, hit)
			}
		}
		if len(notHits) != 0 {
			Stderr("GetVelocitiesThatHit returned the following points that are not hits:%s", notHits)
			return "", fmt.Errorf("your shit is broke.")
		}
	}
	answer := len(hits)
	if debug || input.Verbose {
		...
		dys := []int{}
		for _, hit := range hits {
			dxs = appendIfNew(dxs, hit.X)
			dys = appendIfNew(dys, hit.Y)
		}
		sort.Ints(dxs)
		sort.Ints(dys)
		Stderr("DX values: %v", dxs)
		Stderr("DY values: %v", dys)
	}
	return fmt.Sprintf("%d", answer), nil
}

func GetVelocitiesThatHit(target *TargetArea) Points {
	// The general functions for the coordinates of the probe after i steps are these:
	//     x(i) = i*(2*dx - i + 1)/2
	//     y(i) = i*(2*dy - i + 1)/2
	// where dx and dy are the initial velocity of the probe.
	// For this problem, we're only concerned with positive (non-zero) integer values.
	// The x(i) function has a maximum i, after which, x(i) = maxdx*(maxdx+1)/2.
	// The y(i) function also has a maximum i, after which the probe will always miss the target.
	//
	// To find the maximum I for the x function, we want to find the minimum number of
	// steps that it takes for a probe to stop increasing in the x direction, and still
	// reach the target.
	// When the probe stops moving, it has x = sum(1..dx) = dx*(dx+1)/2.
	// We want that to be maxx in order to allow for the most steps.
	// So we have that maxx = dx*(dx+1)/2.
	// Solving that for dx gives sqrt(2*maxx+0.25)-0.5 = dx.
	// Since we only care about integer values, we floor that.
	// So the dx that will allow for the most steps and still hit the target:
	//     maxidx = floor(sqrt(2*maxx+0.25)-0.5)
	// The probe's velocity after i steps is max(0, dx-i).
	// When i = dx, it stops moving right.
	// That means that after maxidx steps, there's no chance for new dx values to become available:
	//     maxIX = floor(sqrt(2*maxx+0.25)-0.5)
	//
	// To find the maximum I for the y function, we use a dy where the probe reaches it's
	// maximum height and still hit the target.
	// Similar to x, the probe will stop moving up at i = dy.
	// Another dy + 1 steps will bring the probe back to y = 0.
	// The step after that, the probe will have velocity 0 - dy - 1.
	// We want that to hit the very bottom of the target: miny.
	// That will give it the greatest initial velocity for maximum height.
	// So miny = 0 - maxdy - 1, or maxdy = 0 - miny - 1.
	// And we know it'll hit the target at 2 * dy + 2 steps (dy up, dy+1 down, 1 to target).
	// So i = 2 * (0 - miny - 1) + 2 = -2 * miny.
	//     maxIY = -2 * miny
	//
	// Lastly, for each i, we need to find the valid range of dx and dy values.
	// All combinations of them will be valid hits.
	// Solving the x(i) and y(i) functions for dx and dy gives:
	//     dv = (i - 1 + v*2/i)/2.
	// where dv is either dx or dy and v is x or y (respectively).
	// Since we only care about integer values, we ceiling the min and floor the max.
	// So:
	//     mindy(i) = ceil((i - 1 + miny*2/i)/2)
	//     maxdy(i) = floor((i - 1 + maxy*2/i)/2)
	// And if i <= maxIX:
	//     mindx(i) = ceil((i - 1 + minx*2/i)/2)
	//     maxdx(i) = floor((i - 1 + maxx*2/i)/2)
	// Otherwise:
	//     mindx(i) = ceil((maxIX - 1 + minx*2/maxIX)/2)
	//     maxdx(i) = floor((maxIX - 1 + maxx*2/maxIX)/2)
	// Care must also be taken to not add duplicate entries where the probe hits the
	// target a second time if left to keep moving another step.
	minX := float64(target.MinX)
	maxX := float64(target.MaxX)
	minY := float64(target.MinY)
	maxY := float64(target.MaxY)
	maxIX := math.Floor(math.Sqrt(2.0*maxX+0.25) - 0.5)
	maxI := -2.0 * minY
	Debugf("minX: %f, maxX: %f, minY: %f, maxY: %f, maxIX: %f, maxI: %f", minX, maxX, minY, maxY, maxIX, maxI)
	vals := func(i, min, max float64) []int {
		start := int(math.Ceil((i - 1 + min*2/i) / 2))
		stop := int(math.Floor((i - 1 + max*2/i) / 2))
		rv := make([]int, stop-start+1)
		for i := range rv {
			rv[i] = start + i
		}
		return rv
	...
	rv := Points{}
	var prevDXs, prevDYs []int
	for i := float64(1); i <= maxI; i++ {
		var dxs []int
		if i <= maxIX {
			dxs = vals(i, minX, maxX)
		} else {
			dxs = prevDXs
		}
		dys := vals(i, minY, maxY)
		Debugf("i: %f, dxs: %v, dys: %v", i, dxs, dys)
		for _, dy := range dys {
			for _, dx := range dxs {
				if !containsInt(prevDXs, dx) || !containsInt(prevDYs, dy) {
					rv = append(rv, &Point{dx, dy})
				}
			}
		}
		prevDXs = dxs
...
func containsInt(vals []int, val int) bool {
	for _, v := range vals {
		if v == val {
			return true
		}
...
func (p Points) String() string {
	var rv strings.Builder
	rv.WriteString(fmt.Sprintf("{%d}\n", len(p)))
	pts := make(Points, len(p))
	copy(pts, p)
	sort.Sort(pts)
	prevY := 0
	if len(pts) > 0 {
	...
	for i, pt := range pts {
		...
		rv.WriteString(fmt.Sprintf("%d%s", i, pt))
	}
	rv.WriteByte('\n')
	return rv.String()
}

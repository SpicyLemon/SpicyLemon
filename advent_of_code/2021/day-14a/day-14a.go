...
func Solve(input Input) (string, error) {
	...
	Debugf("Template:      %6d %s", len(template), template)
	for i := 1; i <= input.Count; i++ {
		template = ApplyPairs(template, input.Pairs)
		Debugf("After step %2d: %6d %s", i, len(template), template)
	}
	chars := map[rune]int{}
	for _, b := range template {
		chars[b]++
	}
	byCount := NewRuneInts(len(chars))
	for char, count := range chars {
		byCount = append(byCount, RuneInt{char, count})
	}
	sort.Sort(RuneIntSorterInt(byCount))
	Debugf("Character counts:\n%s", byCount)
	minPair := byCount[0]
	maxPair := byCount[len(byCount)-1]
	Debugf("Min: %s", minPair)
	Debugf("Max: %s", maxPair)
	answer := maxPair.I - minPair.I
	return fmt.Sprintf("%d", answer), nil
}

func ApplyPairs(template string, pairs map[string]string) string {
	defer FuncEndingAlways(FuncStarting(len(template)))
	var rv strings.Builder
	for i := 0; i < len(template)-1; i++ {
		rv.WriteByte(template[i])
		if v, ok := pairs[template[i:i+2]]; ok {
			rv.WriteString(v)
		}
	}
	rv.WriteByte(template[len(template)-1])
	return rv.String()
}
...
func (r RuneInt) String() string {
	return fmt.Sprintf("%q: %d", r.R, r.I)
}

type RuneInts []RuneInt

func NewRuneInts(l int) RuneInts {
	return make([]RuneInt, 0, l)
}

func (r RuneInts) String() string {
	var rv strings.Builder
	for _, p := range r {
...
type RuneIntSorterInt []RuneInt

func (s RuneIntSorterInt) Less(i, j int) bool { return s[i].I < s[j].I }
func (s RuneIntSorterInt) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s RuneIntSorterInt) Len() int           { return len(s) }

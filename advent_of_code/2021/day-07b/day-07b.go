...
func Solve(input Input) (string, error) {
	...
	stats := GetStats(input.Ints)
	Debugf("%s", stats)
	min := 9223372036854775807
	for h := 0; h < stats.Max; h++ {
		d := SumDist(input.Ints, h)
		if input.Extra {
			Stderr("H: %d, Total: %d", h, d)
		}
		if d < min {
			min = d
		}
...
func SumDist(from []int, to int) int {
	rv := 0
	for _, v := range from {
		d := MoveCost(v, to)
		Debugf("SumSeq(%d, %d) = %d", v, to, d)
		rv += d
	}
	return rv
}
...
func (s Stats) String() string {
	...
	rv.WriteString(fmt.Sprintf("Counts: %d\n", s.Counts))
	rv.WriteString(fmt.Sprintf("  Mode: %d\n", s.Mode))
	return rv.String()
}
...
func (i Input) String() string {
	lineFmt := DigitFormatForMax(len(i.Ints)) + ": %d\n"
	var rv strings.Builder
...
func ParseInput(fileData []byte) (Input, error) {
	...
	for _, line := range lines {
		if len(line) > 0 {
			ints, err := SplitParseInts(line, ",")
			if err != nil {
				return rv, err
			}
			rv.Ints = append(rv.Ints, ints...)
		}
	}
	return rv, nil
}

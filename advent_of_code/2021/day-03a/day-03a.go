...
func solve(input Input) (string, error) {
	...
	counts := []map[byte]int{}
	for _, diag := range input.Diags {
		for len(counts) < len(diag) {
	...
	gamma := make([]byte, len(counts))
	epsilon := make([]byte, len(counts))
	for i, m := range counts {
		maxc := 0
		maxb := byte('_')
		minc := len(input.Diags) + 1
		minb := byte('_')
		for b, c := range m {
			if c > maxc {
				maxc = c
	...
	gammai, err := strconv.ParseInt(string(gamma), 2, 64)
	if err != nil {
		return "", fmt.Errorf("gamma error: %w", err)
	}
	epsiloni, err := strconv.ParseInt(string(epsilon), 2, 64)
	if err != nil {
		return "", fmt.Errorf("epsilon error: %w", err)
	}
	answer := gammai * epsiloni
	return fmt.Sprintf("%d * %d = %d", gammai, epsiloni, answer), nil
}

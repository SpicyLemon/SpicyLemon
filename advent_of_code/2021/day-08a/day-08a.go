...
func Solve(input Input) (string, error) {
	...
	sevens := len(byLen[3])
	eights := len(byLen[7])
	answer := ones + fours + sevens + eights
	return fmt.Sprintf("1s: %d, 4s: %d, 7s: %d, 8s: %d = %d", ones, fours, sevens, eights, answer), nil
}
	...
	for _, line := range lines {
		if len(line) > 0 {
			...
			for _, str := range strings.Fields(parts[0]) {
				note.Signals = append(note.Signals, Display(str))
			}
			for _, str := range strings.Fields(parts[1]) {
				note.Digits = append(note.Digits, Display(str))
			}

...
func Solve(params *Params) (string, error) {
	...
	if len(params.Custom) > 0 {
		input, err = ParseInput(params.Custom)
	} else {
	...
	Debugf("Parsed Input:\n%s", input)
	maxRollLength := params.Count
	p1WinUs := 0
	p2WinUs := 0
	noWins := EnumerateRolls(2)
increaseRollLengthLoop:
	for {
		Debugf("Checking rolls of length %d.", len(noWins[0]))
		stillNoWins := ListListInt{}
		newP1RollWins := 0
		newP1WinUs := 0
		newP2RollWins := 0
		newP2WinUs := 0
		for _, rolls := range noWins {
			p1win, p2win := PlayTheGame(rolls, NewPlayer(1, input.P1Start), NewPlayer(2, input.P2Start))
			switch {
			case p1win:
				newP1RollWins++
				newP1WinUs += CountUniverses(rolls)
			case p2win:
				newP2RollWins++
				newP2WinUs += CountUniverses(rolls)
			default:
				stillNoWins = append(stillNoWins, rolls)
			}
		}
		p1WinUs += newP1WinUs
		p2WinUs += newP2WinUs
		Debugf("Player 1 wins with %d rolls in %d new universes for %d total.", newP1RollWins, newP1WinUs, p1WinUs)
		Debugf("Player 2 wins with %d rolls in %d new universes for %d total.", newP2RollWins, newP2WinUs, p2WinUs)
		Debugf("There are %d roll enumerations with length %d where no one wins.", len(stillNoWins), len(noWins[0]))
		switch {
		case len(stillNoWins) == 0:
			Debugf("No more no-wins left.")
			noWins = nil
			break increaseRollLengthLoop
		case len(stillNoWins[0]) >= maxRollLength:
			Debugf("Max roll length %d done.", maxRollLength)
			break increaseRollLengthLoop
		}
		noWins = AddToRollEnumeration(stillNoWins)
		Debugf("Now there are %d rolls to check, each with length %d (%d max)", len(noWins), len(noWins[0]), maxRollLength)
	}
	Stdout("Player 1 started at %d and won in %d universes.", input.P1Start, p1WinUs)
	Stdout("Player 2 started at %d and won in %d universes.", input.P2Start, p2WinUs)
	Stdout("There are %d roll enumerations with max length %d where no one wins.", len(noWins), maxRollLength)
	switch {
	case len(noWins) > 50:
		Debugf("First 10 no-win rolls:\n%s", noWins[:10])
		Debugf("Middle 10 no-win rolls:\n%s", noWins[len(noWins)/2-5:len(noWins)/2+5])
		Debugf("Last 10 no-win rolls:\n%s", noWins[len(noWins)-10:])
	case len(noWins) > 0:
		Debugf("No-win rolls:\n%s", noWins)
	}
	answer := p1WinUs
	...
	Stdout("Player 1 wins in %d universes.", p1WinUs)
	Stdout("Player 2 wins in %d universes.", p2WinUs)
	return fmt.Sprintf("%d", answer), nil
}

func PlayTheGame(rolls []int, p1, p2 *Player) (bool, bool) {
	for turn := 0; turn < len(rolls); turn++ {
		if turn%2 == 0 {
			p1.Move(rolls[turn])
...
func CountUniverses(rolls []int) int {
	rv := 1
	for _, roll := range rolls {
		rollU, ok := rollCounts[roll]
		if !ok {
			panic(fmt.Sprintf("No roll count found for %d", roll))
		}
		rv *= rollU
	}
	return rv
...
func AddToRollEnumeration(rollLists ListListInt) ListListInt {
	defer FuncEnding(FuncStarting(len(rollLists), len(rollLists[0])))
	rv := make(ListListInt, len(rollLists)*7)
	for r, rolls := range rollLists {
		for i := 3; i <= 9; i++ {
			rv[7*r+i-3] = make([]int, len(rolls)+1)
			copy(rv[7*r+i-3], rolls)
			rv[7*r+i-3][len(rolls)] = i
		}
	}
	return rv
...
func EnumerateRolls(turns int) ListListInt {
	defer FuncEnding(FuncStarting(turns))
	switch {
	case turns <= 0:
		return ListListInt{}
	case turns == 1:
		return ListListInt{{3}, {4}, {5}, {6}, {7}, {8}, {9}}
	}
	return AddToRollEnumeration(EnumerateRolls(turns - 1))
}

// ----------  ListListInt  ---------------------------------------------

type ListListInt [][]int

func (l ListListInt) String() string {
	lineFmt := DigitFormatForMax(len(l)) + ": %v"
	lines := make([]string, len(l))
	for i, line := range l {
		lines[i] = fmt.Sprintf(lineFmt, i, line)
	}
...
func NewPlayer(num, start int) *Player {
	return &Player{
		Num:   num,
		At:    start,
		Score: 0,
...
func (p Player) String() string {
	return fmt.Sprintf("Player %d: Score: %d, At: %d", p.Num, p.Score, p.At)
}

func (p *Player) Move(moves int) {
	p.At = (p.At-1+moves)%BoardSize + 1
	p.Score += p.At
}
...
func ParseInput(lines []string) (*Input, error) {
	...
	var err error
	if len(lines) < 2 {
		return nil, errors.New("not enough input lines")
	}
	rv.P1Start, err = strconv.Atoi(strings.TrimPrefix(lines[0], "Player 1 starting position: "))
	if err != nil {
		return nil, fmt.Errorf("error parsing %q: %w", lines[0], err)
	}
	rv.P2Start, err = strconv.Atoi(strings.TrimPrefix(lines[1], "Player 2 starting position: "))
	if err != nil {
		return nil, fmt.Errorf("error parsing %q: %w", lines[1], err)
	}
	return &rv, nil
}

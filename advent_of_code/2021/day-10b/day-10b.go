...
var EasyChunkRx *regexp.Regexp

func init() {
	EasyChunkRx = regexp.MustCompile(`\(\)|\[\]|\{\}|\<\>`)
}
...
func IncompleteChunkError(openedBy string) *ChunkError {
	return &ChunkError{OpenedBy: openedBy}
}

func CorruptChunkError(expected, found byte) *ChunkError {
	return &ChunkError{Expected: expected, Found: found}
}

func (e ChunkError) Error() string {
	if e.IsIncomplete() {
		return fmt.Sprintf("incomplete: opened by %s", e.OpenedBy)
	}
	return fmt.Sprintf("corrupt: expected %q, but found %q instead.", e.Expected, e.Found)
}

func (e ChunkError) IsIncomplete() bool {
	return len(e.OpenedBy) != 0
}

func (e ChunkError) IsCorrupt() bool {
	return e.Found != BYTE_ZERO
}

func (e ChunkError) Score() int {
	if e.IsCorrupt() {
		return Score[e.Found]
	}
	rv := 0
	for i := len(e.OpenedBy) - 1; i >= 0; i-- {
		rv = rv*5 + Score[e.OpenedBy[i]]
	}
	return rv
}

func ParseChunk(str string) *ChunkError {
	lastStr := ""
	for lastStr != str {
		lastStr = str
		str = EasyChunkRx.ReplaceAllLiteralString(str, "")
	}
	for i, c := range str {
		if strings.Contains(Closers, string(c)) {
			return CorruptChunkError(ClosedBy[str[i-1]], byte(c))
		}
	}
	return IncompleteChunkError(str)
}
...
func Solve(input Input) (string, error) {
	...
	for i, line := range input.Lines {
		...
		switch {
		case err == nil:
			return "", fmt.Errorf("%d: %q does not fail to parse", i, line)
		case err.IsIncomplete():
			score := err.Score()
			Debugf("%d: %q => %v = %d", i, line, err, score)
			incomplete = append(incomplete, score)
		case err.IsCorrupt():
			score := err.Score()
			Debugf("%d: %q => %v = %d", i, line, err, score)
			corrupt = append(corrupt, score)
		default:
			return "", fmt.Errorf("unkown error state %d: %q => %v", i, line, err)
		}
	}
	corruptAnswer := 0
	...
	incompleteAnswer := incomplete[len(incomplete)/2]
	return fmt.Sprintf("Corrupt: %d, Incomplete: %d", corruptAnswer, incompleteAnswer), nil
}

...
func Solve(input Input) (string, error) {
	...
	maxY := len(input.Octopus) - 1
	maxX := len(input.Octopus[0]) - 1
	doStep := func() int {
		flashes := make([][]bool, len(input.Octopus))
		for i := range flashes {
			flashes[i] = make([]bool, len(input.Octopus[0]))
		}
		for y, r := range input.Octopus {
			for x := range r {
				input.Octopus[y][x]++
			}
		}
	...
	answer := 0
	expected := (maxX + 1) * (maxY + 1)
	Debugf("Looking for %4d flashes.", expected)
	for i := 0; i < input.Count; i++ {
		v := doStep()
		Debugf("Step %6d: Flashes %4d", i, v)
		if v == expected {
			answer = i + 1
			break
		}
...
func (i Input) String() string {
	lineFmt := DigitFormatForMax(len(i.Octopus)) + ": "
	var rv strings.Builder
	for i, v := range i.Octopus {
		rv.WriteString(fmt.Sprintf(lineFmt, i))
		for _, o := range v {
			rv.WriteString(fmt.Sprintf("%d", o))
		}
		rv.WriteByte('\n')
	}
	return rv.String()
...
func ParseInput(fileData []byte) (Input, error) {
	...
	rv.Octopus = make([][]int, len(lines))
	var err error
	for y, line := range lines {
		if len(line) > 0 {
			rv.Octopus[y] = make([]int, len(line))
			for x, c := range line {
				rv.Octopus[y][x], err = strconv.Atoi(string(c))
				if err != nil {
					return rv, err
				}
			}

...
func solve(input Input) (string, error) {
	...
	countCol := func(diags [][]byte, col int) map[byte]int {
		rv := map[byte]int{}
	...
	filterDiags := func(diags [][]byte, col int, b byte) [][]byte {
		rv := [][]byte{}
		for _, diag := range diags {
			if diag[col] == b {
				rv = append(rv, diag)
	...
	oxgen := make([]byte, len(input.Diags[0]))
	oxgenDiags := append([][]byte{}, input.Diags...)
	for i := range oxgen {
		if debug {
			stderr("oxgenDiags:")
			for i, diag := range oxgenDiags {
				stderr("%5d: %v", i+1, diag)
			}
		}
		if len(oxgenDiags) == 1 {
			oxgen[i] = oxgenDiags[0][i]
			debugf("oxgen: %v", oxgen)
			continue
		}
		colCounts := countCol(oxgenDiags, i)
		debugf("colCounts: %v", colCounts)
		if colCounts['0'] > colCounts['1'] {
			oxgen[i] = '0'
		} else {
			oxgen[i] = '1'
		}
		oxgenDiags = filterDiags(oxgenDiags, i, oxgen[i])
		debugf("oxgen: %v", oxgen)
	}

	co2 := make([]byte, len(input.Diags[0]))
	co2Diags := append([][]byte{}, input.Diags...)
	for i := range co2 {
		if debug {
			stderr("co2Diags:")
			for i, diag := range co2Diags {
				stderr("%3d: %v", i+1, diag)
			}
		}
		if len(co2Diags) == 1 {
			co2[i] = co2Diags[0][i]
		...
		colCounts := countCol(co2Diags, i)
		debugf("colCounts: %v", colCounts)
		if colCounts['1'] < colCounts['0'] {
			co2[i] = '1'
		} else {
			co2[i] = '0'
		}
		co2Diags = filterDiags(co2Diags, i, co2[i])
		debugf("co2: %v", co2)
	}
	stdout("oxgen: [%s]", oxgen)
	stdout("  co2: [%s]", co2)

	oxgeni, err := strconv.ParseInt(string(oxgen), 2, 64)
	if err != nil {
		return "", fmt.Errorf("oxgen error: %w", err)
	}
	co2i, err := strconv.ParseInt(string(co2), 2, 64)
	if err != nil {
		return "", fmt.Errorf("co2 error: %w", err)
	}
	answer := oxgeni * co2i
	return fmt.Sprintf("%d * %d = %d", oxgeni, co2i, answer), nil
}
...
func (i Input) String() string {
	lineFmt := "%" + fmt.Sprintf("%d", len(fmt.Sprintf("%d", len(i.Diags)))) + "d: %s\n"
	var rv strings.Builder
	for i, v := range i.Diags {
		rv.WriteString(fmt.Sprintf(lineFmt, i, string(v)))
	}
	return rv.String()
...
func parseInput(fileData []byte) (Input, error) {
	...
	for _, line := range strings.Split(string(fileData), "\n") {
		if len(line) > 0 {
			rv.Diags = append(rv.Diags, []byte(line))
		}
	}
	return rv, nil

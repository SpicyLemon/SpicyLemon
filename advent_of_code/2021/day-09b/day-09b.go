...
func Solve(input Input) (string, error) {
	...
	maxY := len(input.Heights) - 1
	maxX := len(input.Heights[0]) - 1
	isLow := func(x, y int) bool {
		return (x == 0 || input.Heights[y][x] < input.Heights[y][x-1]) &&
			(y == 0 || input.Heights[y][x] < input.Heights[y-1][x]) &&
			(x == maxX || input.Heights[y][x] < input.Heights[y][x+1]) &&
			(y == maxY || input.Heights[y][x] < input.Heights[y+1][x])
	}
	lows := [][]int{}
	basinEdges := make([][]bool, maxY+1)
	basinMap := make([][]int, maxY+1)
	for y := 0; y <= maxY; y++ {
		basinEdges[y] = make([]bool, maxX+1)
		basinMap[y] = make([]int, maxX+1)
		for x := 0; x <= maxX; x++ {
			basinEdges[y][x] = input.Heights[y][x] == 9
			if isLow(x, y) {
				lows = append(lows, []int{y, x})
			}
		}
	}
	if debug {
		...
		Stderr("Basin Edges (%d x %d):\n%s", maxX+1, maxY+1, m.String())
	...
	growAll := func() bool {
		...
		for y := 0; y <= maxY; y++ {
			for x := 0; x <= maxX; x++ {
				if basinMap[y][x] == 0 && !basinEdges[y][x] {
					switch {
					case x > 0 && basinMap[y][x-1] > 0:
						basinMap[y][x] = basinMap[y][x-1]
						rv = true
					case y > 0 && basinMap[y-1][x] > 0:
						basinMap[y][x] = basinMap[y-1][x]
						rv = true
					case x < maxX && basinMap[y][x+1] > 0:
						basinMap[y][x] = basinMap[y][x+1]
						rv = true
					case y < maxY && basinMap[y+1][x] > 0:
						basinMap[y][x] = basinMap[y+1][x]
						rv = true
					}
				}
	...
	for i, low := range lows {
		basinMap[low[0]][low[1]] = i + 1
	}
	Debugf("Starting Growth")
	for i := 0; growAll(); i++ {
		Debugf("Growth round %d", i)
	}
	if debug {
		var m strings.Builder
		for y := 0; y <= maxY; y++ {
			m.WriteString(" ")
			for x := 0; x <= maxX; x++ {
				m.WriteString(fmt.Sprintf("%4d", basinMap[y][x]))
			}
			m.WriteByte('\n')
		}
		Stderr("Basin Map (%d x %d):\n%s", maxX+1, maxY+1, m.String())
	}
	counts := make([]int, len(lows))
	for y := 0; y <= maxY; y++ {
		for x := 0; x <= maxX; x++ {
			if !basinEdges[y][x] {
				counts[basinMap[y][x]-1]++
			}
		}
	}
	Debugf("Counts: %v", counts)
	sort.Ints(counts)
	Debugf("Sorted: %v", counts)
	top3 := counts[len(counts)-3:]
	answer := top3[0] * top3[1] * top3[2]
	return fmt.Sprintf("sum(%v) = %d", top3, answer), nil
}
...
func (i Input) String() string {
	lineFmt := DigitFormatForMax(len(i.Heights)) + ": "
	var rv strings.Builder
	for i, v := range i.Heights {
		rv.WriteString(fmt.Sprintf(lineFmt, i))
		for _, d := range v {
			rv.WriteString(fmt.Sprintf("%d", d))
		}
		rv.WriteByte('\n')
	}
	return rv.String()
...
func ParseInput(fileData []byte) (Input, error) {
	...
	for _, line := range lines {
		if len(line) > 0 {
			v := make([]uint8, len(line))
			for i, c := range line {
				d, err := strconv.ParseInt(string(c), 10, 8)
				if err != nil {
					return rv, err
				}
				v[i] = uint8(d)
			}
			rv.Heights = append(rv.Heights, v)
		}
	}
	return rv, nil
}

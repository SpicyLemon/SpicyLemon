...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	size := MinMax{-50, 50}
	cube := NewCube(size, size, size, false)
	for _, instr := range input.Instructions {
		cube.Set(instr.X, instr.Y, instr.Z, instr.Val)
	}
	answer := cube.CountVal(true)
...
func NewCube(x, y, z MinMax, val bool) *Cube {
	...
	rv.Values = make([][][]bool, rv.Depth)
	for zz := range rv.Values {
		rv.Values[zz] = make([][]bool, rv.Height)
		for yy := range rv.Values[zz] {
			rv.Values[zz][yy] = make([]bool, rv.Width)
			if val {
				for xx := range rv.Values[zz][yy] {
					rv.Values[zz][yy][xx] = val
				}
			}
		}
	}
...
func (c Cube) String() string {
	return fmt.Sprintf("x=%s (%d), y=%s (%d), z=%s (%d)", c.X, c.Width, c.Y, c.Height, c.Z, c.Depth)
}

func (c *Cube) Set(x, y, z MinMax, val bool) {
	xStart := Max(x.Min, c.X.Min) - c.X.Min
	xStop := Min(x.Max, c.X.Max) - c.X.Min
	yStart := Max(y.Min, c.Y.Min) - c.Y.Min
	yStop := Min(y.Max, c.Y.Max) - c.Y.Min
	zStart := Max(z.Min, c.Z.Min) - c.Z.Min
	zStop := Min(z.Max, c.Z.Max) - c.Z.Min
	for zz := zStart; zz <= zStop; zz++ {
		for yy := yStart; yy <= yStop; yy++ {
			for xx := xStart; xx <= xStop; xx++ {
				c.Values[zz][yy][xx] = val
			}
		}
	}
}

func (c *Cube) CountVal(val bool) int {
	rv := 0
	for z := range c.Values {
		for y := range c.Values[z] {
			for _, v := range c.Values[z][y] {
				if val == v {
					rv++
				}
			}

...
func Solve(input Input) (string, error) {
	...
	debugPaper := func(i int, p *Paper, c int) {
		if debug {
			if p.Width < 200 {
				Stderr("Paper after fold %d:\n%s", i, p)
			} else {
				Stderr("Paper after fold %d: Width: %d, Height: %d", i, p.Width, p.Height)
			}
		}
		Debugf("Dot count after fold %d: %d", i, c)
	}
	paper := NewPaper(input.Points)
	answer := paper.GetDotCount()
	debugPaper(0, paper, answer)
	for i := 0; i < input.Count && i < len(input.Folds); i++ {
		paper = paper.DoFold(input.Folds[i])
		answer = paper.GetDotCount()
...
func NewPaper(points []Point) *Paper {
	...
	rv := BlankPaper(maxY+1, maxX+1)
	for _, p := range points {
		rv.Dots[p.Y][p.X] = true
	}
	return rv
}

func BlankPaper(height, width int) *Paper {
	rv := Paper{
		Height: height,
		Width:  width,
	}
	rv.Dots = BoolMatrix(rv.Height, rv.Width)
	Debugf("New Paper size: %d x %d", rv.Width, rv.Height)
	return &rv
}

func (p Paper) GetDotCount() int {
	points := []Point{}
	rv := 0
	for y := 0; y < p.Height; y++ {
		for x := 0; x < p.Width; x++ {
	...
	Debugf("Found %d (%d) with maxX: %d, maxY: %d", len(points), rv, maxX, maxY)
	return rv
}

func (p Paper) DoFold(fold Fold) *Paper {
	Debugf("Applying fold %q", fold)
	if fold.X > 0 {
		return p.DoFoldX(fold.X)
	}
	return p.DoFoldY(fold.Y)
}

func (p Paper) DoFoldX(on int) *Paper {
	rv := BlankPaper(p.Height, on)
...
func (p Paper) DoFoldY(on int) *Paper {
	rv := BlankPaper(on, p.Width)
	for y := 0; y < rv.Height; y++ {
		for x := 0; x < rv.Width; x++ {
			switch {
			case p.Dots[y][x]:
...
func BoolMatrix(height, width int) [][]bool {
	rv := make([][]bool, height)
	for i := 0; i < height; i++ {
		rv[i] = make([]bool, width)
	}
	return rv
}
...
func (p Point) String() string {
	return fmt.Sprintf("(%4d, %4d)", p.X, p.Y)
}
...
func (f Fold) String() string {
	if f.X > 0 {
		return fmt.Sprintf("fold along x=%4d", f.X)
	}
	return fmt.Sprintf("fold along y=%4d", f.Y)
}
...
func (i Input) String() string {
	lineFmt := "  " + DigitFormatForMax(len(i.Points)) + ": %s\n"
	var rv strings.Builder
	rv.WriteString(fmt.Sprintf("Points %d:\n", len(i.Points)))
	for i, v := range i.Points {
		rv.WriteString(fmt.Sprintf(lineFmt, i, v))
	}
	rv.WriteString(fmt.Sprintf("Folds %d:\n", len(i.Folds)))
	for i, v := range i.Folds {
		rv.WriteString(fmt.Sprintf(lineFmt, i, v))
	}
	return rv.String()
}
...
func ParseInput(fileData []byte) (Input, error) {
	...
	for _, line := range lines {
		...
		case !pointsDone:
			...
			rv.Points = append(rv.Points, Point{parts[0], parts[1]})
		case strings.HasPrefix(line, "fold along x="):
			num := strings.TrimPrefix(line, "fold along x=")
			i, err := strconv.Atoi(num)
			if err != nil {
				return rv, err
			}
			rv.Folds = append(rv.Folds, Fold{i, 0})
		case strings.HasPrefix(line, "fold along y="):
			num := strings.TrimPrefix(line, "fold along y=")
			i, err := strconv.Atoi(num)
			if err != nil {
				return rv, err
			}
			rv.Folds = append(rv.Folds, Fold{0, i})
		default:
			return rv, fmt.Errorf("unknown line: %q", line)
		}
	}
	return rv, nil
}

...
func Solve(input Input) (string, error) {
	...
	minCosts := NewMinCostMatrix(input.Risks)
	minCosts.Set(0, 0, 0, nil)
	Stdout("Min Costs:\n%s", minCosts)
	for _, p := range input.Points {
		Stdout("Sums from %s:\n%s", p, input.Risks.SumFrom(p.X, p.Y).StringWithPath(input.Path, &p))
	}
	Stdout("Risks:\n%s", input.Risks.StringWithPath(input.Path, nil))
...
func (m MinCostMatrix) String() string {
	...
	rv.WriteString(CreateIndexedGridString(m.Risks.ToStringMatrix(), visited, nil))
	rv.WriteString("Come From:\n")
	rv.WriteString(CreateIndexedGridString(comeFromStrs, visited, nil))
	rv.WriteString("Min Costs:\n")
	rv.WriteString(CreateIndexedGridString(minCosts.ToStringMatrix(), visited, nil))
	return rv.String()
}
...
func (m Matrix) StringWithPath(points []Point, center *Point) string {
	toColor := make([]XY, len(points))
	for i, p := range points {
		toColor[i] = p
	}
	var toHighlight []XY
	if center != nil {
		toHighlight = append(toHighlight, center)
	}
	return CreateIndexedGridString(m.ToStringMatrix(), toColor, toHighlight)
}

func (m Matrix) SumFrom(cX, cY int) Matrix {
	w := m.Width()
	h := m.Height()
	rv := NewMatrix(w, h)
	// Fill in the center first.
	rv[cY][cX] = m[cY][cX]
	// Fill in straight up, straight left, and upper left quadrant
	for y := cY; y >= 0; y-- {
		for x := cX; x >= 0; x-- {
			switch {
			case x < cX && y < cY: // upper left quadrant
				rv[y][x] = rv[y][x+1] + rv[y+1][x] - rv[y+1][x+1] + m[y][x]
			case x < cX: // straight left
				rv[y][x] = rv[y][x+1] + m[y][x]
			case y < cY: // straight up
				rv[y][x] = rv[y+1][x] + m[y][x]
			}
		}
	}
	// Fill in straight right and upper right quadrant
	for y := cY; y >= 0; y-- {
		for x := cX + 1; x < w; x++ {
			switch {
			case y < cY: // upper right quadrant
				rv[y][x] = rv[y][x-1] + rv[y+1][x] - rv[y+1][x-1] + m[y][x]
	...
	// Fill in straight down and lower left quadrant
	for y := cY + 1; y < h; y++ {
		for x := cX; x >= 0; x-- {
			switch {
			case x < cX: // lower left quadrant
				rv[y][x] = rv[y][x+1] + rv[y-1][x] - rv[y-1][x+1] + m[y][x]
	...
	// Fill in the lower right quadrant
	for y := cY + 1; y < h; y++ {
		for x := cX + 1; x < w; x++ {
			rv[y][x] = rv[y][x-1] + rv[y-1][x] - rv[y-1][x-1] + m[y][x]
		}

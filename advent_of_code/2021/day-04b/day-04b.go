...
func solve(input Input) (string, error) {
	...
	winners := make([]*BingoBoard, 0, len(input.Boards))
	notWinners := make([]*BingoBoard, len(input.Boards))
	for b, board := range input.Boards {
		notWinners[b] = board
	}
	lastDraw := -1
	for d, draw := range input.Draws {
		if debug {
			sums := make([]int, len(notWinners))
			for b, board := range notWinners {
				sums[b] = board.SumUnhit()
			}
			stderr("Applying draw %d: [%d], with %d boards left: %v", d+1, draw, len(notWinners), sums)
		}
		stillNotWinners := make([]*BingoBoard, 0, len(notWinners))
		for _, board := range notWinners {
			board.ApplyDraw(draw)
			if board.IsWinner() {
				winners = append(winners, board)
			} else {
				stillNotWinners = append(stillNotWinners, board)
			}
		}
		lastDraw = draw
		notWinners = stillNotWinners
		if len(notWinners) == 0 {
			stdout("Draws: %v", input.Draws[:d+1])
			break
		}
	}
	if debug {
		...
		for b, board := range winners {
			stderr("Winner %d:\n%s\n", b, board)
		}
	}
	lastWinner := winners[len(winners)-1]
	sum := lastWinner.SumUnhit()
	stdout("Board:\n%s", lastWinner)
	return fmt.Sprintf("%d x %d = %d", lastDraw, sum, lastDraw*sum), nil
}
...
func NewBingoBoard() *BingoBoard {
	...
	for i := range rv.Cells {
		rv.Cells[i] = make([]int, 5)
	}
	rv.Hits = make([][]bool, len(rv.Cells))
	for i := range rv.Hits {
		rv.Hits[i] = make([]bool, len(rv.Cells[i]))
	}
	return &rv
}

func (b BingoBoard) ApplyDraw(draw int) {
	if !b.IsWinner() {
...
func (b BingoBoard) SumUnhit() int {
	rv := 0
	for i, line := range b.Cells {
		for j, cell := range line {
			if !b.Hits[i][j] {
...
func (b BingoBoard) String() string {
	...
	for i, line := range b.Cells {
		...
		for j, cell := range line {
			if b.Hits[i][j] {
				rv.WriteString(fmt.Sprintf(" \033[1m%2d\033[0m", cell))
			} else {
				rv.WriteString(fmt.Sprintf(" %2d", cell))
			}
		}
		rv.WriteByte('\n')
	}
...
func (i Input) String() string {
	lineFmt := "%" + fmt.Sprintf("%d", len(fmt.Sprintf("%d", len(i.Boards)))) + "d:\n%s"
	var rv strings.Builder
	rv.WriteString(fmt.Sprintf("Draws: %v\n", i.Draws))
	for i, v := range i.Boards {
...
func parseInput(fileData []byte) (Input, error) {
	...
	drawStrings := strings.Split(lines[0], ",")
	rv.Draws = make([]int, len(drawStrings))
	var err error
	for i, ds := range drawStrings {
		rv.Draws[i], err = strconv.Atoi(ds)
		if err != nil {
			return rv, err
		}
	}
	...
	for _, line := range lines[1:] {
		if len(line) == 0 {
			cur = NewBingoBoard()
			i = 0
			rv.Boards = append(rv.Boards, cur)
			continue
		}
		for j, cell := range strings.Fields(line) {
			cur.Cells[i][j], err = strconv.Atoi(cell)
			if err != nil {
				return rv, err
			}
		}

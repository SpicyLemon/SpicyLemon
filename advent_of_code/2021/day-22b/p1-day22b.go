...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	inputCubes := input.Instructions.ToCubes()
	Debugf("Input Cubes\n%s", inputCubes)
	initCube := NewCube(MinMax{-50, 50}, MinMax{-50, 50}, MinMax{-50, 50}, CUBE_ON)
	cubes := CubeList{}
	for _, cube := range inputCubes {
		c := initCube.GetIntersectionCube(cube)
		if c != nil {
	...
	for i := 0; i <= len(cubes)-1; i++ {
		for j := i + 1; j < len(cubes); j++ {
			cube := cubes[i].GetIntersectionCube(cubes[j])
			if cube != nil {
				finalCubes = append(finalCubes, cube)
			}
		}
...
func (c *Cube) GetIntersectionCube(d *Cube) *Cube {
	if c.Val == CUBE_MASK || d.Val == CUBE_MASK || (c.Val == CUBE_OFF && d.Val == CUBE_OFF) {
		return nil
	}
	rv := NewCube(
...
func (c Cube) String() string {
	return fmt.Sprintf("X=%s (% 6d), Y=%s (% 6d), Z=%s (% 6d) = % 15d cells * %s = % 15d", c.X, c.Width, c.Y, c.Height, c.Z, c.Depth, c.Volume(), c.Val, c.GetTotal())
}
...
func (m MinMax) String() string {
	return fmt.Sprintf("% 6d..% 6d", m.Min, m.Max)
}

...
func Solve(params *Params) (string, error) {
	...
	Debugf("Parsed Input:\n%s", input)
	cubes := CubeList{}
	for i, instr := range input.Instructions {
		cube := NewCube(instr.X, instr.Y, instr.Z, instr.Val)
		newCubes := CubeList{}
		for _, c := range cubes {
			newCubes = append(newCubes, c.CutAround(cube)...)
		}
		if cube.Val {
			newCubes = append(newCubes, cube)
		}
		cubes = newCubes
		Stdout("(%3d/%3d) After adding cube %s, there are %d cubes.", i+1, len(input.Instructions), cube, len(cubes))
		if i >= params.Count {
			break
		}
	}
	answer := cubes.CountOn()
...
func NewCube(x, y, z MinMax, val bool) *Cube {
	rv := Cube{
		Width:  x.Max - x.Min + 1,
...
func (c Cube) String() string {
	return fmt.Sprintf("x=%s (%d), y=%s (%d), z=%s (%d) = %d cells", c.X, c.Width, c.Y, c.Height, c.Z, c.Depth, c.Width*c.Height*c.Depth)
}
...
const OT_NONE OverlapType = 0
const OT_INSIDE OverlapType = 1
const OT_OUTSIDE OverlapType = 2
const OT_MUSHROOM_TOP OverlapType = 3
const OT_MUSHROOM_STEM OverlapType = 4
const OT_STEPS_WIDE OverlapType = 5
const OT_STEPS_THIN OverlapType = 6
const OT_CORNER OverlapType = 7
const OT_DONUT OverlapType = 8
const OT_SPEAR OverlapType = 9
const OT_KNOT OverlapType = 10
...
func (t OverlapType) String() string {
	...
	case 2:
		return "OT_OUTSIDE"
	case 3:
		return "OT_MUSHROOM_TOP"
	case 4:
		return "OT_MUSHROOM_STEM"
	case 5:
		return "OT_STEPS_WIDE"
	case 6:
		return "OT_STEPS_THIN"
	case 7:
		return "OT_CORNER"
	case 8:
		return "OT_DONUT"
	...
	case 10:
		return "OT_KNOT"
	default:
		return fmt.Sprintf("OverlapType(%d)", t)
	}
}

func (c Cube) GetOverlapType(d *Cube) OverlapType {
	if debug {
		Stderr("This: %s", c)
		Stderr("Corners:\n%s", c.GetCorners())
		Stderr("Edges:\n%s", c.GetEdges())
	...
	default:
		panic(fmt.Sprintf("Cube %s (that) has %d points inside cube %s (this).", d, pointsInThis, c))
	}
	pointsInThat := 0
	...
	case 0:
		// Do Nothing (keep going)
	default:
		panic(fmt.Sprintf("Cube %s (this) has %d points inside cube %s (that) (%d the other way).", c, pointsInThat, d, pointsInThis))
	}
	thatEdges := d.GetEdges()
	for _, s := range c.GetSides() {
		edgesThatIntersect := 0
		for _, e := range thatEdges {
			if e.Intersects(s) {
				edgesThatIntersect++
			}
		}
		switch edgesThatIntersect {
		case 4:
			return OT_DONUT
		case 2:
			return OT_KNOT
		case 0:
			// Do Nothing (keep going)
		default:
			panic(fmt.Sprintf("Cube %s (this) has side %s that intersects %d edges of %s (that).", c, s, edgesThatIntersect, d))
		}
	}
	thisEdges := c.GetEdges()
	for _, s := range d.GetSides() {
		edgesThisIntersect := 0
		for _, e := range thisEdges {
			if e.Intersects(s) {
				edgesThisIntersect++
			}
		}
		switch edgesThisIntersect {
		case 4:
			return OT_SPEAR
		case 0:
			// Do Nothing (keep going)
		default:
			panic(fmt.Sprintf("Cube %s (that) has side %s that intersects %d edges of %s (this).", d, s, edgesThisIntersect, c))
		}
	}
	return OT_NONE
}

func (c Cube) CutAround(d *Cube) CubeList {
	otype := c.GetOverlapType(d)
	Debugf("Overlap Type = %s", otype)
	var prv CubeList
	switch otype {
	case OT_NONE:
		prv = CubeList{&c}
	case OT_INSIDE:
		// This is completely inside the new one, it can go away.
		prv = CubeList{}
	case OT_OUTSIDE:
		prv = CubeList{
			NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
			NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
			NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
			NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
			NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, d.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
			NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
		}
	case OT_MUSHROOM_TOP:
		var keyEdge *Edge
		thatEdges := c.GetEdges()
		for _, s := range d.GetSides() {
			edgeCount := 0
			for _, e := range thatEdges {
				if e.Intersects(s) {
		...
		minInside := c.Contains(keyEdge.MinPoint())
		switch keyEdge.DVAxis {
		case 'x':
			if minInside {
				prv = CubeList{
					NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{d.X.Min, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				}
			} else {
				prv = CubeList{
					NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				}
			}
		case 'y':
			if minInside {
				prv = CubeList{
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Max + 1, c.Z.Max}, c.Val),
				}
			} else {
				prv = CubeList{
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Max + 1, c.Z.Max}, c.Val),
				}
			}
		case 'z':
			if minInside {
				prv = CubeList{
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
				}
			} else {
				prv = CubeList{
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
					NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
				}
			}
		}
	case OT_MUSHROOM_STEM:
		var keyEdge *Edge
		thatEdges := c.GetEdges()
		for _, s := range d.GetSides() {
			edgeCount := 0
		...
		case 'x':
			if minInside {
				prv = CubeList{NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val)}
			} else {
				prv = CubeList{NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val)}
			}
		case 'y':
			if minInside {
				prv = CubeList{NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val)}
			} else {
				prv = CubeList{NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val)}
			}
		case 'z':
			if minInside {
				prv = CubeList{NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val)}
			} else {
				prv = CubeList{NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val)}
			}
		}
	case OT_STEPS_WIDE:
		var keyEdge *Edge
		...
		case 'x':
			prv = CubeList{
				NewCube(MinMax{c.X.Min, keyEdge.DV.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{keyEdge.DV.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
			}
			thisYLess := c.Y.Contains(d.Y.Min)
			thisZLess := c.Z.Contains(d.Z.Min)
			if thisYLess {
				if thisZLess {
					prv = append(prv,
						NewCube(keyEdge.DV, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(keyEdge.DV, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(keyEdge.DV, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(keyEdge.DV, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			} else {
				if thisZLess {
					prv = append(prv,
						NewCube(keyEdge.DV, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(keyEdge.DV, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(keyEdge.DV, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(keyEdge.DV, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			}
		case 'y':
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, keyEdge.DV.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{keyEdge.DV.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
			}
			thisXLess := c.X.Contains(d.X.Min)
			thisZLess := c.Z.Contains(d.Z.Min)
			if thisXLess {
				if thisZLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, keyEdge.DV, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(MinMax{c.X.Min, d.X.Min - 1}, keyEdge.DV, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, keyEdge.DV, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(MinMax{c.X.Min, d.X.Min - 1}, keyEdge.DV, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			} else {
				if thisZLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, keyEdge.DV, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(MinMax{d.X.Max + 1, c.X.Max}, keyEdge.DV, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, keyEdge.DV, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(MinMax{d.X.Max + 1, c.X.Max}, keyEdge.DV, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			}
		case 'z':
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, keyEdge.DV.Min - 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{keyEdge.DV.Max + 1, c.Z.Max}, c.Val),
			}
			thisXLess := c.X.Contains(d.X.Min)
			thisYLess := c.Y.Contains(d.Y.Min)
			if thisXLess {
				if thisYLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, keyEdge.DV, c.Val),
						NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, c.Y.Max}, keyEdge.DV, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, keyEdge.DV, c.Val),
						NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, d.Y.Max}, keyEdge.DV, c.Val),
					)
				}
			} else {
				if thisYLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, keyEdge.DV, c.Val),
						NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, c.Y.Max}, keyEdge.DV, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, keyEdge.DV, c.Val),
						NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, d.Y.Max}, keyEdge.DV, c.Val),
					)
				}
			}
		}
	case OT_STEPS_THIN:
		var keyEdge *Edge
		thisSides := c.GetSides()
		for _, e := range d.GetEdges() {
			sideCount := 0
			for _, s := range thisSides {
				if e.Intersects(s) {
					sideCount++
					if sideCount == 2 {
						keyEdge = &e
		...
		case 'x':
			...
			if thisYLess {
				if thisZLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			} else {
				if thisZLess {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
					)
				} else {
					prv = append(prv,
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
						NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
					)
				}
			}
		case 'y':
	...
	case OT_CORNER:
		...
		thisZLess := c.Z.Contains(d.Z.Min)
		switch {
		case thisXLess && thisYLess && thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min + 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
			}
		case thisXLess && thisYLess && !thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
			}
		case thisXLess && !thisYLess && thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min + 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, d.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
			}
		case thisXLess && !thisYLess && !thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
			}
		case !thisXLess && thisYLess && thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min + 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
			}
		case !thisXLess && thisYLess && !thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
			}
		case !thisXLess && !thisYLess && thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min + 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, d.Y.Max}, MinMax{d.Z.Min, c.Z.Max}, c.Val),
			}
		case !thisXLess && !thisYLess && !thisZLess:
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Max}, c.Val),
			}
		}
	case OT_DONUT:
		var keySide *Side
		...
		case 'y':
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val),
			}
		case 'z':
			prv = CubeList{
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
				NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val),
			}
		}
	case OT_SPEAR:
		var keySide *Side
	...
	case OT_KNOT:
		...
		switch keyEdges[0].DVAxis {
		case 'x':
			if keyEdges[0].D1 == keyEdges[1].D1 {
				// same Y
				prv = CubeList{
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val),
					NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val),
				}
				if c.Y.Contains(d.Y.Min) {
					prv = append(prv, NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{d.Z.Min, d.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val))
				}
			} else {
				// same Z
				prv = CubeList{
				...
				if c.Z.Contains(d.Z.Min) {
					prv = append(prv, NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{c.X.Min, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val))
				}
			}
		case 'y':
			if keyEdges[0].D1 == keyEdges[1].D1 {
				...
				if c.X.Contains(d.X.Min) {
					prv = append(prv, NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Min, d.Z.Max}, c.Val))
				}
			} else {
				// Same Z
				prv = CubeList{
				...
				if c.Z.Contains(d.Z.Min) {
					prv = append(prv, NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{d.Z.Max + 1, c.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, c.Y.Max}, MinMax{c.Z.Min, d.Z.Min - 1}, c.Val))
				}
			}
		case 'z':
			if keyEdges[0].D1 == keyEdges[1].D1 {
				...
				if c.X.Contains(d.X.Min) {
					prv = append(prv, NewCube(MinMax{c.X.Min, d.X.Min - 1}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{d.X.Max + 1, c.X.Max}, MinMax{d.Y.Min, d.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val))
				}
			} else {
				// Same Y
				prv = CubeList{
				...
				if c.Y.Contains(d.Y.Min) {
					prv = append(prv, NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{c.Y.Min, d.Y.Min - 1}, MinMax{c.Z.Min, c.Z.Max}, c.Val))
				} else {
					prv = append(prv, NewCube(MinMax{d.X.Min, d.X.Max}, MinMax{d.Y.Max + 1, c.Y.Max}, MinMax{c.Z.Min, c.Z.Max}, c.Val))
				}
			}
		}
	default:
		panic(fmt.Sprintf("Unknown OverlapType: %s", otype))
	}
	rv := CubeList{}
	for _, v := range prv {
		if v.Volume() > 0 {
	...
	Debugf("Returning %d cubes.", len(rv))
	return rv
}

...
func Solve(params *Params) (string, error) {
	...
	for i := 1; i <= params.Count; i++ {
		if !Evolve(input.Floor) {
			answer = i
			Debugf("Final Floor:\n%s", input.Floor)
			break
		}
		CompareKnown(i, input.Floor)
	}
	CompareKnown(answer, input.Floor)
	return fmt.Sprintf("%d", answer), nil
}

func Evolve(floor *Matrix) bool {
	// Move Right
	toMoveR := []Point{}
	for y := 0; y < floor.Height; y++ {
		for x := 0; x < floor.Width; x++ {
			if floor.Vals[y][x] == BYTER {
				xx := x + 1
				if xx >= floor.Width {
					xx = 0
				}
				if floor.Vals[y][xx] == BYTEE {
					toMoveR = append(toMoveR, Point{x, y})
					x++
				}
			}
		}
	}
	for _, p := range toMoveR {
		xx := p.X + 1
		if xx >= floor.Width {
			xx = 0
		}
		floor.Vals[p.Y][p.X], floor.Vals[p.Y][xx] = floor.Vals[p.Y][xx], floor.Vals[p.Y][p.X]
	}
	// Move Left
	toMoveD := []Point{}
	for x := 0; x < floor.Width; x++ {
		for y := 0; y < floor.Height; y++ {
			if floor.Vals[y][x] == BYTED {
				yy := y + 1
				if yy >= floor.Height {
					yy = 0
				}
				if floor.Vals[yy][x] == BYTEE {
					toMoveD = append(toMoveD, Point{x, y})
					y++
				}
			}
		}
	}
	for _, p := range toMoveD {
		yy := p.Y + 1
		if yy >= floor.Height {
			yy = 0
		}
		floor.Vals[p.Y][p.X], floor.Vals[yy][p.X] = floor.Vals[yy][p.X], floor.Vals[p.Y][p.X]
	}
	return len(toMoveR) != 0 || len(toMoveD) != 0
}

func CompareKnown(i int, floor *Matrix) {
	if floor == nil || floor.Width != 10 || floor.Height != 9 {
		return
	}
	var lines []string
	switch i {
	...
	bad := []XY{}
	for y, row := range floor.Vals {
		if lines[y] != string(row) {
			for x, b := range []byte(lines[y]) {
				if b != row[x] {
					bad = append(bad, &Point{x, y})
				}
			}
		}
	}
	if len(bad) > 0 {
		act := strings.Split(CreateIndexedGridString(floor.ToStrings(), nil, bad), "\n")
		exp := strings.Split(CreateIndexedGridString(ParseMatrix(lines).ToStrings(), nil, bad), "\n")
		op := make([]string, len(act))
		for y := range op {
			op[y] = act[y] + "    " + exp[y]
		}
		Stderr("After %d Steps:\nThere are %d incorrect cells:\n%s", i, len(bad), strings.Join(op, "\n"))
	} else {
		Stderr("After %d Steps:\n%s", i, floor)
	}
}
...
func (m Matrix) String() string {
	return CreateIndexedGridString(m.ToStrings(), nil, nil)
}

func NewMatrix(width, height int, val byte) *Matrix {
	rv := Matrix{
		Width:  width,
		Height: height,
	}
	rv.Vals = make([][]byte, height)
	for y := range rv.Vals {
		rv.Vals[y] = make([]byte, width)
		if val != BYTE0 {
			for x := range rv.Vals[y] {
				rv.Vals[y][x] = val
			}
		}
	}
...
func (m Matrix) ToStrings() [][]string {
	rv := make([][]string, len(m.Vals))
	for y, row := range m.Vals {
		rv[y] = make([]string, len(row))
		for x, b := range row {
			rv[y][x] = string([]byte{b})
		}
	}
	return rv
}
...
func ParseMatrix(lines []string) *Matrix {
	rv := NewMatrix(len(lines[0]), len(lines), BYTE0)
	for y, line := range lines {
		copy(rv.Vals[y], []byte(line))
	}
	return rv
}
...
func (i Input) String() string {
	return fmt.Sprintf("Floor: %d x %d\n%s", i.Floor.Width, i.Floor.Height, i.Floor)
}

func ParseInput(lines []string) (*Input, error) {
	rv := Input{}
	keepers := make([]string, 0, len(lines))
	for _, line := range lines {
		if len(line) != 0 {
			keepers = append(keepers, line)
		}
	}
	rv.Floor = ParseMatrix(keepers)
	return &rv, nil
}

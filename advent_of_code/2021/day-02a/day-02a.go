...
func solve(input Input) (string, error) {
	...
	return fmt.Sprintf("%d x %d = %d", h, d, h*d), nil
}
...
func toDir(str string) (Dir, error) {
	...
	return unknown, fmt.Errorf("Unkown direction string: [%s]", str)
}
...
func (d Dir) String() string {
	...
	panic(fmt.Sprintf("Unknown direction vakue: [%d]", d))
}
...
func (i Instr) String() string {
	return fmt.Sprintf("%s %d", i.Dir, i.Units)
}
...
func (i Input) String() string {
	lineFmt := "%" + fmt.Sprintf("%d", len(fmt.Sprintf("%d", len(i.Instructions)))) + "d: %s\n"
...
func parseInput(fileData []byte) (Input, error) {
	...
	for _, line := range strings.Split(string(fileData), "\n") {
		if len(line) > 0 {
			...
			if len(fields) != 2 {
				return rv, fmt.Errorf("invalid input line: [%s]", line)
			}
			dir, err := toDir(fields[0])
			if err != nil {
				return rv, err
			}
			...
			rv.Instructions = append(rv.Instructions, Instr{Dir: dir, Units: int(u)})
		}
	}
	return rv, nil

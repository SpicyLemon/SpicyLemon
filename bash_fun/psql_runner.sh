#!/bin/bash
# This script can either be run directly, or sourced to add the psql_runner function to your environment.
...
if [[ "$sourced" == 'YES' ]]; then
    PSQL_RUNNER_SH="$( cd "$( dirname "$0" 2> /dev/null || dirname "$BASH_SOURCE" )"; pwd -P)/$( basename "$0" 2> /dev/null || basename "$BASH_SOURCE" )"
    psql_runner () {
        $PSQL_RUNNER_SH "$@"
    }
    return 0
...
DEFAULT_PORT='15551'
DEFAULT_KRAKEN_PORT='5434'
DEFAULT_DB_NAME='sofi_products'
DEFAULT_USER='products'
DEFAULT_PASS='sofi'
MY_EXIT_CODE=0
...
for THING_TO_DO in "${THINGS_TO_DO[@]}"; do
    CMD=(psql -U $DB_USER -h $DB_HOST -p $DB_PORT -d $DB_NAME -v AUTOCOMMIT=off -v ON_ERROR_STOP=on "${DB_OPTIONS[@]}")
    CMD_OUT="${CMD[@]} $THING_TO_DO"
    case "$THING_TO_DO" in
    '-f '*)
    ...
    *)
    ...
    if [[ -n "$ONLY_SHOW_CMD" ]]; then
        ...
        if [[ -n "$FILE" && "$( cat "$FILE" | grep -v ^[[:space:]]*$ | wc -l )" -eq "0" ]]; then
            ...
            if [[ -n "$FILE" || -n "$DB_CMD" ]]; then
                PGPASSWORD=$DB_PASS "${CMD[@]}" 2>&1 | to_stdout_and_strip_colors_log "$LOGFILE"
                PSQL_EXIT_CODE="${PIPESTATUS[0]}${pipestatus[1]}"
            else
                PSQL_EXIT_CODE=255
            fi
            ...
            if [[ "$PSQL_EXIT_CODE" -eq "0" ]]; then
                ...
                MY_EXIT_CODE=2
                RESULT="$( echo_bad " ERROR (code $PSQL_EXIT_CODE) " ) -"
                case "$PSQL_EXIT_CODE" in
                1) RESULT="$RESULT psql fatal error occured";;

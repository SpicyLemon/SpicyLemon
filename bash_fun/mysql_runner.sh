...
Usage: ./$script [($(echo_white '-h')|$(echo_white '--host')) <host>] [($(echo_white '-p')|$(echo_white '--port')) <port>] [($(echo_white '-d')|$(echo_white '--dbname')) <dbname>]
    ...
    The $(echo_white '-p') or $(echo_white '--port') option defines the port. Default is $( echo_green "$( getDefaultPort )" ).
        Setting and exporting the $( echo_yellow 'MYSQL_RUNNER_DEFAULT_PORT' ) environment variable will override this default.
...
getDefaultPort () {
    if [[ -n "$MYSQL_RUNNER_DEFAULT_PORT" ]]; then
        echo -E -n "$MYSQL_RUNNER_DEFAULT_PORT"
    else
